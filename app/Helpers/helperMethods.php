<?php

use Carbon\Carbon;
use Illuminate\Support\Facades\DB;
use Illuminate\Support\Facades\Validator;

/**
 * For setting active and inactive class on different <li> when clicked on side menu
 * @param name $path   Route Name
 * @param string $active active or inactive classes of css
 */

function getRoleRightsArray($roleId){
    $roleRights = DB::table('role_rights')
                ->select('role_rights.id as role_rights_id', 'role_rights.role_id', 'role_rights.right_id', 'rights.right_name', 'rights.right_code')
                ->join('rights', 'role_rights.right_id', '=', 'rights.id')
                ->where('role_id',$roleId)->get()->toArray();
    $rightsArray = array();
    if(!empty($roleRights)){
      foreach($roleRights as $right){
          $rightsArray[] = $right->right_code;
      }  
    }
    return $rightsArray;
}


function set_active($path, $active = 'nav-item nav-item-open') {

    return call_user_func_array('Request::routeIs', (array) $path) ? $active : 'nav-item';
}

function getConstant($key) {
    return \Config::get('constants.' . $key);
    exit;
}

function getCommissionValue($totalPrice, $percentage) {
    $mul = $totalPrice * $percentage;
    $commissionedValue = $mul / 100;
    return $commissionedValue;
}

//This Function checks the form validation and if fails return validation errors...

function checkValidation($request, $rules) {

    $validator = Validator::make($request->all(), $rules);
    if ($validator->fails()) {
        $errors = $validator->errors();
        return $errors;
    } else {
        return False;
    }
}

/* Setting Display Block or None property to each <ul> on side menu */

function set_none_block($path, $active = "block") {

    return call_user_func_array('Request::routeIs', (array) $path) ? $active : 'none';
}

/* For checking name if exist for side menu and so on... */

function page_name(string $name) {
    return array_key_exists($name, session('page'));
}

/* For checking link if exist for side menu and so on... */

function page_link(string $link) {
    return in_array($link, session('page'));
}

function db_table($table) {
    return DB::table($table);
}

//Calculate percent but if the percent value is negative and needs to be positive directly.
function calculate_percent($amount, $percent) {
    return (int) ($amount * abs($percent) / 100);
}

function array_exists($arr) {
    if (array_key_exists(0, $arr)) {
        $var = $arr;
    } else {
        $var[0] = $arr;
    }
    return $var;
}

/* Method for calculating insurance percentage */
/*
 * $num1 is the actual percentage value which is applied on buyer group.
 * $num2 is the total value.
 * $amount is the actual amount which is applied on the buyer group.
 */

function calculate_percent_record($num1, $num2, $amount) {
    $result = 0;
    $num3 = 0;

    if ($num1 == null) {
        $result = $num2;
    } elseif ($num1 < 0) {
        $vl = explode('-', $num1);
        $num3 = (float) $vl[1] * $num2 / 100;
        $result = $num2 - $num3;
    } else {
        $num3 = $num2 * $num1 / 100;
        $result = $num2 + $num3;
    }

    if ($amount == null) {
        $result = $result + $amount;
    } elseif ($amount < 0) {
        $result = $result + ($amount);
    } else {
        $result = $result + $amount;
    }

    return $result . '-' . $num3;
}

function dates_difference($date1, $date2) {
    //Our dates
// $date1 = "2013-03-01 06:01:00";
// $date2 = "2013-03-01 09:00:00";
//Convert them to timestamps.
    $date1Timestamp = strtotime($date1);
    $date2Timestamp = strtotime($date2);

//Calculate the difference.
    $difference = $date2Timestamp - $date1Timestamp;

    $diff_in_hr = $difference / 60 / 60;

    $whole = (int) $diff_in_hr;  // 5
    $frac = ($diff_in_hr - $whole) * 60;
    return $whole . 'h ' . $frac . 'm';
}

function api_to_normal_format($datec) {
    $timec = str_replace("T", " ", $datec);
    $new_dt = explode(".", $timec);
    return $new_dt[0];
}

function api_to_app_format($datec) {

    $timec = str_replace("T", " ", $datec);
    $new_dt = explode(".", $timec);
    return date('D , d M. H:i', strtotime($new_dt[0]));
}

//changing date to specific format for DB.
function change_date($date) {

    return date('Y-m-d', strtotime(str_replace('/', '-', $date)));
}

//changing date to specific format for PK Time like d-m-Y.
function pak_date($date) {

    return date('d-M-Y', strtotime(str_replace('/', '-', $date)));
}

function pak_date_time($date) {

    return date('d-M-Y h:i', strtotime(str_replace('/', '-', $date)));
}

function euro_date($date) {
    return \Carbon\Carbon::parse($date)->format('dMy');
    // return date('dMy',strtotime(str_replace ( '/' , '-' , $date)));
}

/* ----------  New Invoice number generated  ---------- */

// function invoice_number($type,$date)
function invoice_number($date) {
    $number = '';
    // $number.=$type.'-'.Carbon::parse($date)->format('dmY').'-';
    $number .= Carbon::parse($date)->format('dmY');

    if (\DB::table('invoice_numbers')->whereDate('inv_date', $date)->exists()) {

        $inserted_id = \DB::table('invoice_numbers')->insertGetId(['inv_date' => change_date($date)], 'inv_id');

        if (strlen((string) $inserted_id) >= 3) {

            // if (strlen((string)$inserted_id) >= 3)
            $number = $number . $inserted_id;
        } else {
            for ($i = strlen((string) $inserted_id); $i < 3; $i++) {
                $number = $number . '0';
            }

            $number = $number . $inserted_id;
        }
    } else {
        \DB::table('invoice_numbers')->whereDate('inv_date', $date)->truncate();

        $inserted_id = \DB::table('invoice_numbers')->insertGetId(['inv_date' => change_date($date)], 'inv_id');
        $number = $number . '00' . $inserted_id;
    }
    return $number;
}

/* Method for generating automatic invoice number
 *  $type is string
 *  $id is integer
 */

// function invoice_number($type,$id)
// {
//     $number='';
//     $number.=$type.'-';
//     if (strlen((string)$id) >= 8) {
//         if (strlen((string)$id) >= 8)
//             $number=$number.$id;
//     }
//     else
//     {
//         for($i=strlen((string)$id); $i<8;$i++)
//         {
//             $number=$number.'0';
//         }
//         $number=$number.$id;
//     }
//     return $number;
// }

/* Method for generating automatic deposit slip number
 *  $id is integer
 */
function deposit_slip_no($id) {
    $number = '';

    if (strlen((string) $id) >= 8) {

        if (strlen((string) $id) >= 8)
            $number = $number . $id;
    }
    else {
        for ($i = strlen((string) $id); $i < 8; $i++) {
            $number = $number . '0';
        }

        $number = $number . $id;
    }
    return $number;
}

/* Method for generating automatic Voucher Number
 *  $id is integer
 */

function voucher_number($id) {
    $number = 'PV-';

    if (strlen((string) $id) >= 8) {

        if (strlen((string) $id) >= 8)
            $number = $number . $id;
    }
    else {
        for ($i = strlen((string) $id); $i < 8; $i++) {
            $number = $number . '0';
        }

        $number = $number . $id;
    }
    return $number;
}

/* Method for generating automatic Receipt Voucher
 *  $id is integer
 */

// function receipt_voucher($id)
// {
//     $number='RV-';
//     if (strlen((string)$id) >= 8) {
//         if (strlen((string)$id) >= 8)
//             $number=$number.$id;
//     }
//     else
//     {
//         for($i=strlen((string)$id); $i<8;$i++)
//         {
//             $number=$number.'0';
//         }
//         $number=$number.$id;
//     }
//     return $number;
// }

/* Method for generating automatic Quotation Number
 *  $id is integer
 */
function quotation_number($id) {
    $number = 'QUO-';

    if (strlen((string) $id) >= 8) {

        if (strlen((string) $id) >= 8)
            $number = $number . $id;
    }
    else {
        for ($i = strlen((string) $id); $i < 8; $i++) {
            $number = $number . '0';
        }

        $number = $number . $id;
    }
    return $number;
}

function pr($arr) {
    echo "<pre>";
    print_r($arr);
    echo "</pre>";
    die();
}

function check_city_soto($orgion) {
    $pakistan_airports_code = array('BDN', 'BHV', 'BHW', 'BNP', 'CHB', 'CJL', 'DBA', 'DDU', 'DEA', 'DSK', 'GIL', 'GRT', 'GWD', 'HDD', 'ISB', 'JAG', 'JIW', 'KCF', 'KDD', 'KDU', 'KHI', 'LHE', 'LRG', 'LYP', 'MFG', 'MPD', 'MUX', 'UET');
    if (in_array($orgion, $pakistan_airports_code))
        return 'CITY';
    else
        return 'SOTO';
}

//Calcuate age from date of birth.
function calculate_age($birth) {

    return Carbon::parse($birth)->age;
}

/* For converting date into 19Aug19 format */

function convert_date($date) {
    return date('dMy', strtotime($date));
}

/* Convert date to dashes for frontend */

function date_dash($date) {

    return date('Y/m/d H:m', strtotime($date));
}

/* ----------  Date format for prints  ---------- */

function print_date($date) {

    return date('d/m/Y', strtotime($date));
}

function db_format_date($date) {
    return date('Y-m-d', strtotime(str_replace('/', '-', $date)));
}

function hitit__date_fomrat($date) {
    return date('Y-m-d', strtotime(str_replace('/', '-', $date)));
}

function convertNumberToWord($num = false) {
    $num = str_replace(array(',', ' '), '', trim($num));
    if (!$num) {
        return false;
    }
    $num = (int) $num;
    $words = array();
    $list1 = array('', 'One', 'Two', 'Three', 'Four', 'Five', 'Six', 'Seven', 'Eight', 'Nine', 'Ten', 'Eleven',
        'Twelve', 'Thirteen', 'Fourteen', 'Fifteen', 'Sixteen', 'Seventeen', 'Eighteen', 'Nineteen'
    );
    $list2 = array('', 'Ten', 'Twenty', 'Thirty', 'Forty', 'Fifty', 'Sixty', 'Seventy', 'Eighty', 'Ninety', 'Hundred');
    $list3 = array('', 'Thousand', 'Million', 'Billion', 'Trillion', 'Quadrillion', 'Quintillion', 'Sextillion', 'Septillion',
        'Octillion', 'Nonillion', 'Decillion', 'Undecillion', 'Duodecillion', 'Tredecillion', 'Quattuordecillion',
        'Quindecillion', 'Sexdecillion', 'Septendecillion', 'Octodecillion', 'Novemdecillion', 'Vigintillion'
    );
    $num_length = strlen($num);
    $levels = (int) (($num_length + 2) / 3);
    $max_length = $levels * 3;
    $num = substr('00' . $num, -$max_length);
    $num_levels = str_split($num, 3);
    for ($i = 0; $i < count($num_levels); $i++) {
        $levels--;
        $hundreds = (int) ($num_levels[$i] / 100);
        $hundreds = ($hundreds ? ' ' . $list1[$hundreds] . ' Hundred' . ' ' : '');
        $tens = (int) ($num_levels[$i] % 100);
        $singles = '';
        if ($tens < 20) {
            $tens = ($tens ? ' ' . $list1[$tens] . ' ' : '' );
        } else {
            $tens = (int) ($tens / 10);
            $tens = ' ' . $list2[$tens] . ' ';
            $singles = (int) ($num_levels[$i] % 10);
            $singles = ' ' . $list1[$singles] . ' ';
        }
        $words[] = $hundreds . $tens . $singles . ( ( $levels && (int) ( $num_levels[$i] ) ) ? ' ' . $list3[$levels] . ' ' : '' );
    } //end for loop
    $commas = count($words);
    if ($commas > 1) {
        $commas = $commas - 1;
    }
    return implode(' ', $words);
}

function readNumber($num, $depth = 0) {
    $num = (int) $num;
    $retval = "";
    if ($num < 0) // if it's any other negative, just flip it and call again
        return "negative " + readNumber(-$num, 0);
    if ($num > 99) { // 100 and above
        if ($num > 999) // 1000 and higher
            $retval .= readNumber($num / 1000, $depth + 3);

        $num %= 1000; // now we just need the last three digits
        if ($num > 99) // as long as the first digit is not zero
            $retval .= readNumber($num / 100, 2) . " hundred\n";
        $retval .= readNumber($num % 100, 1); // our last two digits                       
    }
    else { // from 0 to 99
        $mod = floor($num / 10);
        if ($mod == 0) { // ones place
            if ($num == 1)
                $retval .= "one";
            else if ($num == 0)
                $retval .= "zero";
            else if ($num == 2)
                $retval .= "two";
            else if ($num == 3)
                $retval .= "three";
            else if ($num == 4)
                $retval .= "four";
            else if ($num == 5)
                $retval .= "five";
            else if ($num == 6)
                $retval .= "six";
            else if ($num == 7)
                $retval .= "seven";
            else if ($num == 8)
                $retval .= "eight";
            else if ($num == 9)
                $retval .= "nine";
        }
        else if ($mod == 1) { // if there's a one in the ten's place
            if ($num == 10)
                $retval .= "ten";
            else if ($num == 11)
                $retval .= "eleven";
            else if ($num == 12)
                $retval .= "twelve";
            else if ($num == 13)
                $retval .= "thirteen";
            else if ($num == 14)
                $retval .= "fourteen";
            else if ($num == 15)
                $retval .= "fifteen";
            else if ($num == 16)
                $retval .= "sixteen";
            else if ($num == 17)
                $retval .= "seventeen";
            else if ($num == 18)
                $retval .= "eighteen";
            else if ($num == 19)
                $retval .= "nineteen";
        }
        else { // if there's a different number in the ten's place
            if ($mod == 2)
                $retval .= "twenty ";
            else if ($mod == 3)
                $retval .= "thirty ";
            else if ($mod == 4)
                $retval .= "forty ";
            else if ($mod == 5)
                $retval .= "fifty ";
            else if ($mod == 6)
                $retval .= "sixty ";
            else if ($mod == 7)
                $retval .= "seventy ";
            else if ($mod == 8)
                $retval .= "eighty ";
            else if ($mod == 9)
                $retval .= "ninety ";
            if (($num % 10) != 0) {
                $retval = rtrim($retval); //get rid of space at end
                $retval .= "-";
            }
            $retval .= readNumber($num % 10, 0);
        }
    }

    if ($num != 0) {
        if ($depth == 3)
            $retval .= " thousand\n";
        else if ($depth == 6)
            $retval .= " million\n";
        if ($depth == 9)
            $retval .= " billion\n";
    }
    return $retval;
}

function airport_name($code) {
    $airport_name = Array
        (
        "AAL" => "Aalborg, Denmark ",
        "AES" => "Aalesund, Norway ",
        "ZID" => "Aarhus, Denmark - Bus service ",
        "AAR" => "Aarhus, Denmark - Tirstrup ",
        "JEG" => "Aasiaat, Greenland ",
        "ABD" => "Abadan, Iran ",
        "ABA" => "Abakan, Russia ",
        "YXX" => "Abbotsford, BC ",
        "ABR" => "Aberdeen, SD ",
        "ABZ" => "Aberdeen, United Kingdom ",
        "AHB" => "Abha, Saudi Arabia ",
        "ABJ" => "Abidjan, Cote d'Ivoire ",
        "ABI" => "Abilene, TX ",
        "AUH" => "Abu Dhabi, United Arab Emirates ",
        "ABS" => "Abu Simbel, Egypt ",
        "ABV" => "Abuja, Nigeria ",
        "ACA" => "Acapulco, Mexico ",
        "AGV" => "Acarigua, Venezuela ",
        "ACC" => "Accra, Ghana ",
        "ADK" => "Adak Island, AK ",
        "ADA" => "Adana, Turkey ",
        "ADD" => "Addis Ababa, Ethopia ",
        "ADL" => "Adelaide, Australia ",
        "ADE" => "Aden, Yemen ",
        "AER" => "Adler/Sochi, Russia ",
        "AZR" => "Adrar, Algeria ",
        "AFT" => "Afutara, Soloman Islands ",
        "AGA" => "Agadir, Morocco ",
        "IXA" => "Agartala, India ",
        "AUP" => "Agaun, Papua New Guinea ",
        "AGF" => "Agen, France ",
        "AGR" => "Agra, India ",
        "AJI" => "Agri, Turkey ",
        "BQN" => "Aguadilla, Puerto Rico ",
        "AGU" => "Aguascalientes, Mexico ",
        "AGJ" => "Aguni, Japan ",
        "AMD" => "Ahmedabad, India ",
        "AWZ" => "Ahwaz, Iran ",
        "AIM" => "Ailuk Island, Marshall Islands ",
        "AEO" => "Aioun El Atrouss, Mauritania ",
        "AIC" => "Airok, Marshall Islands ",
        "AIT" => "Aitutaki, Cook Islands ",
        "AJL" => "Aizawl, India ",
        "AJA" => "Ajaccio, France ",
        "KKI" => "Akiachak, AK ",
        "AKI" => "Akiak, AK ",
        "AXT" => "Akita, Japan ",
        "CAK" => "Canton/Akron, OH ",
        "AKU" => "Aksu, China ",
        "AKX" => "Almaty, Kazakhstan ",
        "AKV" => "Akulivik, QC ",
        "AEY" => "Akureyri, Iceland ",
        "KQA" => "Akuton, AK ",
        "AAN" => "Al Ain, United Arab Emirates ",
        "AAC" => "Al Arish, Egypt ",
        "AAY" => "Al Ghaydah, Yemen ",
        "AHU" => "Al Hoceima, Morocco ",
        "AUK" => "Alakanuk, AK ",
        "ALM" => "Alamogordo, NM ",
        "ALS" => "Alamosa, CO ",
        "ABT" => "Al-Baha, Saudi Arabia ",
        "ALB" => "Albany, NY ",
        "CVO" => "Albany, OR - Bus service ",
        "QWY" => "Albany, OR - Bus service ",
        "ABQ" => "Albuquerque, NM ",
        "ABX" => "Albury, Australia ",
        "ACI" => "Alderney, United Kingdom ",
        "XLY" => "Aldershot, ON - Rail service ",
        "WKK" => "Aleknagik, AK ",
        "ALP" => "Aleppo, Syrian Arab Republic ",
        "ALJ" => "Alexander Bay, South Africa ",
        "ALY" => "Alexandria, Egypt ",
        "AEX" => "Alexandria, LA ",
        "XFS" => "Alexandria,ON - Rail service ",
        "AXD" => "Alexandroupolis, Greece ",
        "FJR" => "Al-Fujairah, United Arab Emirates ",
        "AHO" => "Alghero, Italy ",
        "ALG" => "Algiers, Algeria ",
        "ALC" => "Alicante, Spain ",
        "ASP" => "Alice Springs, Australia ",
        "AET" => "Allakaket, AK ",
        "ABE" => "Easton, PA ",
        "AIA" => "Alliance, NE ",
        "YTF" => "Alma, QC ",
        "LEI" => "Almeria, Spain ",
        "ARD" => "Alor Island, Indonesia ",
        "AOR" => "Alorsetar, Malaysia ",
        "GUR" => "Alotau, Papua New Guinea ",
        "APN" => "Alpena, MI ",
        "ALF" => "Alta, Norway ",
        "ATM" => "Altamira, Brazil ",
        "AAT" => "Altay, China ",
        "ACH" => "Altenrhein, Switzerland ",
        "ARR" => "Alto Rio Senguerr, Argentina ",
        "AOO" => "Martinsburg, PA ",
        "ASJ" => "Amami O Shima, Japan ",
        "AMA" => "Amarillo, TX ",
        "AZB" => "Amazon Bay, Papua New Guinea ",
        "IVA" => "Ambanja, Madagascar ",
        "AMY" => "Ambatomainty, Madagascar ",
        "WAM" => "Ambatondrazaka, Madagascar ",
        "ABL" => "Ambler, AK ",
        "AMQ" => "Ambon, Indonesia ",
        "ASV" => "Amboseli, Kenya ",
        "AMV" => "Amderma, Russia ",
        "ADJ" => "Amman, Jordan - Civil/Marka Airport ",
        "AMM" => "Amman, Jordan - Queen Alia International ",
        "ATQ" => "Amritsar, India ",
        "AMS" => "Amsterdam, Netherlands ",
        "DYR" => "Anadyr, Russia ",
        "YAA" => "Anahim Lake, BC ",
        "AKP" => "Anaktueuk, AK ",
        "HVA" => "Analalava, Madagascar ",
        "AAQ" => "Anapa, Russia ",
        "ANC" => "Anchorage, AK ",
        "AOI" => "Ancona, Italy ",
        "ANX" => "Andenes, Norway ",
        "AZN" => "Andizhan, Uzbekistan ",
        "AUY" => "Aneityum, Vanuatu ",
        "JHE" => "Angelholm/Helsingborg, Sweden ",
        "ANE" => "Angers, France - Marce ",
        "QXG" => "Angers, France - Rail service ",
        "AGD" => "Anggi, Indonesia ",
        "AQG" => "Anging, China ",
        "YAX" => "Angling Lake, ON ",
        "AGN" => "Angoon, AK ",
        "ANG" => "Angouleme, France ",
        "AXA" => "Anguilla, Anguilla ",
        "ANI" => "Aniak, AK ",
        "AWD" => "Aniwa, Vanuatu ",
        "AKA" => "Ankang, China ",
        "ESB" => "Ankara, Turkey - Esenboga ",
        "ANK" => "Ankara, Turkey - Etimesqut ",
        "JVA" => "Ankavandra, Madagascar ",
        "AAE" => "Annaba, Algeria ",
        "NCY" => "Annecy, France ",
        "ANM" => "Antalaha, Madagascar ",
        "AYT" => "Antalya, Turkey ",
        "TNR" => "Antaninvarivo, Madgascar ",
        "ANU" => "V.C. Bird International, Antigua & Barbuda ",
        "ANF" => "Antofagasta, Chile ",
        "WAQ" => "Antsalova, Madagascar ",
        "DIE" => "Antsiranana, Madagascar ",
        "WAI" => "Antsohihy, Madagascar ",
        "ZAY" => "Antwerp, Belgium - Bus service ",
        "ANR" => "Antwerp, Belgium - Deurne Airport ",
        "ANV" => "Anvik, AK ",
        "AOJ" => "Aomori, Japan ",
        "AOT" => "Aosta, Italy ",
        "APO" => "Apartado, Colombia ",
        "APW" => "Apia, Western Samoa ",
        "FGI" => "Apia, Western Samoa ",
        "ATW" => "Appleton, WI ",
        "AQJ" => "Aqaba, Jordan ",
        "AJU" => "Araca, Brazil ",
        "ARU" => "Aracatuba, Brazil ",
        "ARW" => "Arad, Romania ",
        "ARP" => "Aragip, Papua New Guinea ",
        "AUX" => "Araguaina, Brazil ",
        "AAG" => "Arapoti, Brazil ",
        "RAE" => "Arar, Saudi Arabia ",
        "AUC" => "Arauca, Colombia ",
        "AMH" => "Arba Mintch, Ethiopia ",
        "ACV" => "Eureka/Arcata, CA ",
        "YAB" => "Arctic Bay, NU ",
        "ARC" => "Arctic Village, AK ",
        "ADU" => "Ardabil, Iran ",
        "AQP" => "Arequipa, Peru ",
        "AGH" => "Argelholm/Helsingborg, Sweden ",
        "GYL" => "Argyle, Australia ",
        "ARI" => "Arica, Chile ",
        "ARH" => "Arkangelsk, Russia ",
        "AXM" => "Armenia, Colombia ",
        "ARM" => "Armidale, Australia ",
        "ATC" => "Arthur's Town, Bahamas ",
        "RUA" => "Arua, Uganda ",
        "AUA" => "Aruba, Aruba ",
        "ARK" => "Arusha, Tanzania ",
        "YEK" => "Arviat, NU ",
        "AJR" => "Arvidsjaur, Sweden ",
        "AKJ" => "Asahikawa, Japan ",
        "AVL" => "Hendersonville, NC ",
        "ASB" => "Ashgabat, Turkmenistan ",
        "HTS" => "Huntington, WV/Ashland, KY ",
        "ASM" => "Asmara, Eritrea ",
        "ASO" => "Asosa, Ethopia ",
        "ASE" => "Aspen, CO ",
        "ATZ" => "Assiut, Egypt ",
        "TSE" => "Astana, Kazakhstan ",
        "ASF" => "Astrakhan, Russia ",
        "OVD" => "Asturias, Spain and Canary Islands ",
        "ASU" => "Asuncion, Paraguay ",
        "ASW" => "Aswan, Egypt ",
        "AXK" => "Ataq, Yemen ",
        "AHN" => "Athens, GA ",
        "ATH" => "Athens, Greece ",
        "AIU" => "Atiu Island, Cook Islands ",
        "AKB" => "Atka, AK ",
        "ATL" => "Atlanta, GA ",
        "AIY" => "Atlantic City, NJ ",
        "ATD" => "Atoifi, Solomon Islands ",
        "ATK" => "Atqasuk, AK ",
        "YAT" => "Attawapiskat, ON ",
        "AUQ" => "Atuona, French Polynesia ",
        "GUW" => "Atyrau, Kazakhstan ",
        "AKL" => "Auckland, New Zealand ",
        "AGB" => "Augsburg/Munich, Germany ",
        "AGS" => "Augusta, GA ",
        "AUG" => "Augusta, ME ",
        "AKS" => "Auki, Solomon Islands ",
        "YPJ" => "Aupaluk, QC ",
        "AUL" => "Aur Island, Marshall Islands ",
        "IXU" => "Aurangabad, India ",
        "AUR" => "Aurillac, France ",
        "AUU" => "Aurukun, Australia ",
        "AUS" => "Austin, TX ",
        "AVN" => "Avignon, France ",
        "AYW" => "Ayawaki, Indonesia ",
        "AYQ" => "Ayers Rock, Australia ",
        "BXB" => "Babo, Indonesia ",
        "BCD" => "Bacolod, Philippines ",
        "BJZ" => "Badajcz, Spain ",
        "BXD" => "Bade, Indonesia ",
        "BDD" => "Badu Island, Australia ",
        "IXB" => "Bagdogra, India ",
        "YBG" => "Bagotville, QC ",
        "BJR" => "Baharpar, Ethiopia ",
        "BHV" => "Bahawalpur, Pakistan ",
        "BHI" => "Bahia Blanca, Argentina ",
        "BFQ" => "Bahia Pinas, Panama ",
        "BSC" => "Bahia Solano, Colombia ",
        "BAH" => "Bahrain, Bahrain ",
        "BAY" => "Baia Mare, Romania ",
        "YBC" => "Baie Comeau, QC ",
        "VMU" => "Baimuru, Papua New Guinea ",
        "YBK" => "Baker Lake, NU ",
        "BFL" => "Bakersfield, CA ",
        "GYD" => "Baku, Azerbaijan ",
        "BAS" => "Balalae, Solomon Islands ",
        "BZI" => "Balikesir, Turkey ",
        "BPN" => "Balikpapan, Indonesia ",
        "OPU" => "Balimo, Papua New Guinea ",
        "BNK" => "Ballina, Australia ",
        "BBA" => "Balmaceda, Chile ",
        "BWI" => "Baltimore, MD ",
        "BXR" => "Bam, Iran ",
        "ABM" => "Bamaga, Australia ",
        "BKO" => "Bamako, Mali ",
        "BTJ" => "Banda Aceh, Indonesia ",
        "BND" => "Bandar Abbas, Iran ",
        "TKG" => "Bandar Lampung, Indonesia - Branti ",
        "BDH" => "Bandar Lengeh, Iran ",
        "BWN" => "Bandar Seri Begawan, Brunei ",
        "BDO" => "Bandung, Indonesia ",
        "BLR" => "Bangalore, India ",
        "BPX" => "Bangda, China ",
        "BKK" => "Bangkok, Thailand ",
        "BGR" => "Bangor, ME ",
        "BNX" => "Banja Luka, Bosnia Herzegovina ",
        "BDJ" => "Banjarmasin, Indonesia ",
        "BJL" => "Banjul, Gambia ",
        "BMV" => "Banmethuot, Viet Nam - Phung-Doc ",
        "BNP" => "Bannu, Pakistan ",
        "BGF" => "Banqui, Central African Republic ",
        "BSD" => "Baoshan, China ",
        "BAV" => "Baotou, China ",
        "BHB" => "Bar Harbour, ME ",
        "BCA" => "Baracoa, Cuba ",
        "BCI" => "Barcaldine, Australia ",
        "BCN" => "Barcelona, Spain ",
        "BLA" => "Barcelona, Venezuela ",
        "BDU" => "Bardufoss, Norway ",
        "BRI" => "Bari, Italy ",
        "BNS" => "Barinas, Venezuela ",
        "BBN" => "Bario, Malaysia ",
        "BZL" => "Barisal, Bangladesh ",
        "BAX" => "Barnaul, Russia ",
        "BRM" => "Barquisimeto, Venezuela ",
        "BCL" => "Barra Colorado, Costa Rica ",
        "BRR" => "Barra, United Kingdom ",
        "EJA" => "Barran Cabermeja, Colombia ",
        "BAQ" => "Barranquilla, Colombia ",
        "BRA" => "Barreiras, Brazil ",
        "BRW" => "Barrow, AK ",
        "BTI" => "Barter Island, AK ",
        "BSO" => "Basco, Philippines ",
        "BSL" => "Basel, Switzerland ",
        "ZDH" => "Basel/Mulhouse Railway Station, Switzerland ",
        "BUZ" => "Bashehr, Iran ",
        "BIA" => "Bastia, France ",
        "BTH" => "Batam, Indonesia ",
        "ZBF" => "Bathhurst, NB ",
        "BRT" => "Bathurst Island, Australia ",
        "BHS" => "Bathurst, Australia ",
        "BAL" => "Batman, Turkey ",
        "BLJ" => "Batna, Algeria ",
        "BXM" => "Batom, Indonesia ",
        "BTR" => "Baton Rouge, LA ",
        "BJF" => "Batsfijord, Norway ",
        "BBM" => "Battambang, Cambodia ",
        "BUS" => "Batumi, Georgia ",
        "BPF" => "Batuna, Solomon Islands ",
        "BAU" => "Bauru, Brazil ",
        "MBS" => "Saginaw, MI ",
        "BYM" => "Bayamo, Cuba ",
        "BYU" => "Bayreuth, Germany ",
        "XBE" => "Bearskin Lake, ON ",
        "BPT" => "Port Arthur/Beaumont, TX ",
        "ZBV" => "Beaver Creek, CO - Van service ",
        "WBQ" => "Beaver, AK ",
        "CBH" => "Bechar, Algeria ",
        "BKW" => "Beckley, WV ",
        "BED" => "Bedford, MA ",
        "BEU" => "Bedourie, Australia ",
        "EIS" => "Beef Island, British Virgin Islands ",
        "BEI" => "Beica, Ethiopia ",
        "LAQ" => "Beida, Libya - La Braq ",
        "BHY" => "Beihai, China ",
        "PEK" => "Beijing, China ",
        "BEW" => "Beira, Mozambique ",
        "BEY" => "Beirut, Lebanon ",
        "BJA" => "Bejaia, Algeria ",
        "BLG" => "Belaga, Mozambique ",
        "BEL" => "Belem, Brazil ",
        "BMY" => "Belep Island, New Caledonia ",
        "BFS" => "Belfast, Northern Ireland, United Kingdom ",
        "BHD" => "Belfast, United Kingdom ",
        "EGO" => "Belgorod, Russia ",
        "BEG" => "Belgrade, Serbia and Montenegro - Beograd ",
        "BZE" => "Belize City, Belize - International ",
        "TZA" => "Belize City, Belize - Municipal ",
        "ZEL" => "Bella Bella, BC ",
        "QBC" => "Bella Coola, BC ",
        "BLV" => "Belleville, IL ",
        "XVV" => "Belleville, ON - Rail service ",
        "BLI" => "Bellingham, WA ",
        "BNY" => "Bellona, Solomon Islands ",
        "PLU" => "Belo Horizonte, Brazil - Pampulha ",
        "CNF" => "Blo Horizonte, Brazil ",
        "BMD" => "Belo, Madagascar ",
        "BJI" => "Bemidji, MN ",
        "BEB" => "Benbecula, United Kingdom ",
        "BEN" => "Benghazi, Libya ",
        "BKS" => "Bengkulu, Indonesia ",
        "BEH" => "Benton Harbor, MI ",
        "BEJ" => "Berau, Indonesia ",
        "BBO" => "Berbera, Somalia ",
        "YBV" => "Berens River, MB ",
        "BGO" => "Bergen, Norway ",
        "EGC" => "Bergerac, France ",
        "BVG" => "Berlevag, Norway ",
        "BER" => "Berlin, Germany - All airports ",
        "SXF" => "Berlin, Germany - Schoenefeld ",
        "TXL" => "Berlin, Germany - Tegel ",
        "THF" => "Berlin, Germany - Tempelhof ",
        "BDA" => "Hamilton, Bermuda ",
        "BRN" => "Berne, Switzerland - Belp Airport ",
        "ZDJ" => "Berne, Switzerland - Rail service ",
        "BPY" => "Besalampy, Madagascar ",
        "BET" => "Bethel, AK ",
        "BTT" => "Bettles, AK ",
        "BZR" => "Beziers, France ",
        "BDP" => "Bhadrapur, Nepal ",
        "BWA" => "Bhairawa, Nepal ",
        "BMO" => "Bhamo, Myanmar ",
        "BHR" => "Bharatpur, Nepal ",
        "BHU" => "Bhavnagar, India ",
        "BHO" => "Bhopal, India ",
        "BBI" => "Bhubaneswar, India ",
        "BHJ" => "Bhuj, India ",
        "BIK" => "Biak, Indonesia ",
        "BIQ" => "Biarritz, France ",
        "YTL" => "Big Trout, ON ",
        "BII" => "Bikini Atoll, Marshall Islands ",
        "BIO" => "Bilbao, Spain ",
        "BIL" => "Billings, MT ",
        "BLL" => "Billund, Denmark ",
        "GPT" => "Gulfport, MS ",
        "BMU" => "Bima, Indonesia ",
        "BIM" => "Bimini, Bahamas ",
        "NSB" => "Bimini, Bahamas ",
        "BGM" => "Johnson City, NY ",
        "BTU" => "Bintulu, Malaysia ",
        "NTI" => "Bintuni, Indonesia ",
        "BIR" => "Biratragar, Nepal ",
        "KBC" => "Birch Creek, AK ",
        "BVI" => "Birdsville, Australia ",
        "BHM" => "Birmingham, AL ",
        "BHX" => "Birmingham, United Kingdom ",
        "BHH" => "Bisha, Saudi Arabia ",
        "FRU" => "Bishkek, Kyrgyzstan ",
        "BSK" => "Biskra, Algeria ",
        "BIS" => "Bismarck, ND ",
        "OXB" => "Bissau, Guinea-Bissau ",
        "YBI" => "Black Tickle, NL ",
        "BKQ" => "Blackall, Australia ",
        "BLK" => "Blackpool, United Kingdom ",
        "BLT" => "Blackwater, Australia ",
        "BQS" => "Blagoveschensk, Russia ",
        "YBX" => "Blanc Sablon, QC ",
        "BLZ" => "Blantyre, Malawi ",
        "BHE" => "Blenheim, New Zealand ",
        "BID" => "Block Island, RI ",
        "BFN" => "Bloemfontein, South Africa ",
        "BMI" => "Bloomington, IL ",
        "BLF" => "Princeton, WV ",
        "BVC" => "Boa Vista, Cape Verde ",
        "BOV" => "Boang, Papua New Guinea ",
        "BVB" => "Boavista, Brazil ",
        "BOC" => "Bocas Del Toro, Panama ",
        "BOO" => "Bodo, Norway ",
        "BJV" => "Bodrum, Turkey ",
        "BOG" => "Bogota, Colombia ",
        "GIC" => "Boiju Island, Australia ",
        "BOI" => "Boise, ID ",
        "BJB" => "Bojnord, Iran ",
        "BUI" => "Bokondini, Indonesia ",
        "BWK" => "Bol, Croatia ",
        "BZO" => "Bolzano, Italy ",
        "BOA" => "Boma, Congo ",
        "BOM" => "Mumbai, India ",
        "BON" => "Bonaire, Netherlands Antilles ",
        "YVB" => "Bonaventure, QC ",
        "BNJ" => "Bonn, Germany ",
        "BOB" => "Bora Bora, French Polynesia ",
        "BOD" => "Bordeaux, France ",
        "HBE" => "Borg El Arab, Egypt ",
        "BMK" => "Borkum, Germany ",
        "BLE" => "Borlange, Sweden ",
        "RNN" => "Bornholm, Denmark ",
        "BOX" => "Borroloola, Australia ",
        "BSA" => "Bossaro, Somalia ",
        "BOS" => "Boston, MA ",
        "XHH" => "Boulder, CO - Bus service ",
        "WHH" => "Boulder, CO - Hiltons Har H ",
        "WBU" => "Boulder, CO - Municipal Airport ",
        "BQL" => "Boulia, Australia ",
        "BYA" => "Boundary, AK ",
        "BOJ" => "Bourgas, Bulgaria ",
        "BRK" => "Bourke, Australia ",
        "BOH" => "Bournemouth, United Kingdom ",
        "BWG" => "Bowling Green, KY ",
        "BZN" => "Bozeman, MT ",
        "BCQ" => "Brack, Libya ",
        "BFD" => "Bradford, PA ",
        "BRD" => "Brainerd, MN ",
        "BMP" => "Brampton Island, Australia ",
        "XPN" => "Brampton, ON - Rail service ",
        "YBR" => "Brandon, MB ",
        "XFV" => "Brantford, ON - Rail service ",
        "BSB" => "Brasilia, DF, Brazil ",
        "BTS" => "Bratislava, Slovakia ",
        "BTK" => "Bratsk, Russia ",
        "BWE" => "Braunschweig, Denmark ",
        "BWD" => "Brawnwood, TX ",
        "BZV" => "Brazzaville, Congo ",
        "QKB" => "Breckenridge, CO - Van service ",
        "BRE" => "Bremen, Germany ",
        "BES" => "Brest, France ",
        "BWQ" => "Brewarrina, Australia ",
        "BGI" => "Bridgetown, Barbados ",
        "BDS" => "Brindisi, Italy ",
        "BNE" => "Brisbane, Queensland, Australia ",
        "BRS" => "Bristol, United Kingdom ",
        "TRI" => "Kingsport, TN ",
        "BVE" => "Brive-La-Gaillarde, France - Laroche",
        "ZDN" => "Brno, Czech Republic - Bus service ",
        "BRQ" => "Brno, Czech Republic - Turany ",
        "YBT" => "Brochet, MB ",
        "XBR" => "Brockville, ON ",
        "BHQ" => "Broken Hill, Australia ",
        "BNN" => "Bronnoysund, Norway ",
        "BKX" => "Brookings, SD ",
        "RBH" => "Brooks Lodge, AK ",
        "BME" => "Broome, Australia ",
        "BRO" => "Brownsville, TX ",
        "BQK" => "Brunswick, GA ",
        "BHG" => "Brus Laguna, Honduras ",
        "BRU" => "Brussels, Belgium - National ",
        "ZYR" => "Brussels, Belgium - Rail service ",
        "CRL" => "Brussels, Belguim - Charleroi ",
        "BGA" => "Bucaramanga, Colombia ",
        "BBU" => "Bucharest, Romania - Baneasa ",
        "OTP" => "Bucharest, Romania - Otopeni International ",
        "BKC" => "Buckland, AK ",
        "BUD" => "Budapest, Hungary ",
        "AEP" => "Buenos Aires, Argentina - Jorge Newbery ",
        "EZE" => "Buenos Aires, Argentina - Ministro Pistarini ",
        "BUF" => "Buffalo, NY ",
        "UUA" => "Bulgulma, Russia ",
        "BJM" => "Bujumbura, Burundi ",
        "BUA" => "Buka, Papua New Guinea ",
        "BHK" => "Bukhara, Uzbekistan ",
        "BKZ" => "Bukoba, Malaysia ",
        "BUQ" => "Bulawayo, Zimbabwe ",
        "IFP" => "Bullhead City/Laughlin, AZ ",
        "BDB" => "Bundaberg, Australia ",
        "BXZ" => "Bunsil, Papua New Guinea ",
        "BUO" => "Burao, Somalia ",
        "BUR" => "Burbank, CA ",
        "LEV" => "Bureta, Fiji ",
        "BFV" => "Buri Ram, Thailand ",
        "BUC" => "Burketown, Australia ",
        "BRL" => "Burlington, IA ",
        "BTV" => "Burlington, VT ",
        "BWT" => "Burnie, Australia ",
        "YPZ" => "Burns Lake, BC ",
        "PUS" => "Busan, South Korea - Gimhae ",
        "BTM" => "Butte, MT ",
        "BXU" => "Butuan, Philippines ",
        "BZG" => "Bydgoszcz, Poland ",
        "SJD" => "Los Cabos, Mexico ",
        "CFR" => "Caen, France ",
        "CGY" => "Cagayan De Oro, Philippines - Lumbia ",
        "CAG" => "Cagliari, Italy ",
        "CNS" => "Cairns, Australia ",
        "CAI" => "Cairo, Egypt ",
        "CJA" => "Cajamarca, Peru ",
        "CBQ" => "Calabar, Nigeria ",
        "CJC" => "Calama, Chile ",
        "CCU" => "Kolkata, India ",
        "YYC" => "Calgary, AB ",
        "CLO" => "Cali, Colombia ",
        "CLY" => "Calvi, France ",
        "CMW" => "Camaguey, Cuba ",
        "YCB" => "Cambridge Bay, NU ",
        "CBG" => "Cambridge, United Kingdom ",
        "CRD" => "Camodoro, Argentina ",
        "YBL" => "Campbell River, BC ",
        "XAZ" => "Campbellton, NB - Rail service ",
        "CAL" => "Campbeltown, United Kingdom ",
        "CPE" => "Campeche, Mexico ",
        "CPV" => "Campina Grande, Brazil ",
        "CPQ" => "Campinas, Brazil ",
        "CGR" => "Campo Grande, Brazil ",
        "CAW" => "Campos, Brazil ",
        "CAS" => "Casablanca, Morocco - Anfa ",
        "CBR" => "Canberra, Australia ",
        "CUN" => "Cancun, Mexico ",
        "JCA" => "Cannes, France - Coisette Heliport ",
        "CEQ" => "Cannes, France - Mandelieu ",
        "QYW" => "Cannes, France - Vieux Port ",
        "CIW" => "Canouan, Saint Vincent and the Grenadines ",
        "CAP" => "Cap Haitien, Haiti ",
        "YTE" => "Cape Dorset, NU ",
        "CGI" => "Cape Girardeau, MO ",
        "LUR" => "Cape Lisburne, AK ",
        "EHM" => "Cape Newenham, AK ",
        "CPI" => "Cape Orford, Papua New Guinea ",
        "CPT" => "Cape Town, South Africa ",
        "CVL" => "Cape Vogel, Papua New Guinea ",
        "XAW" => "Capreol, ON - Rail service ",
        "CCS" => "Caracas, Venezuela ",
        "CKS" => "Carajas, Brazil ",
        "MDH" => "Carbondale, IL ",
        "CCF" => "Carcassonne, France ",
        "CWL" => "Cardiff, United Kingdom ",
        "CLD" => "Carlsbad, CA ",
        "CNM" => "Carlsbad, NM ",
        "MRY" => "Monterey, CA ",
        "CVQ" => "Carnarvon, Australia ",
        "RIK" => "Carrillo, Costa Rica ",
        "CTG" => "Cartagena, Colombia ",
        "YRF" => "Cartwright, NL ",
        "CUP" => "Carupani, Venezuela ",
        "CMN" => "Casablanca, Morocco - Mohamed V ",
        "CAC" => "Cascavel, Brazil ",
        "CSI" => "Casino, Australia ",
        "CPR" => "Casper, WY ",
        "XZB" => "Casselman, ON - Rail service ",
        "CST" => "Castaway, Fiji ",
        "YCG" => "Castlegar, BC ",
        "DCM" => "Castres, France ",
        "YAC" => "Cat Lake, ON ",
        "CTC" => "Catamarca, Argentina ",
        "CTA" => "Catania, Italy ",
        "CAQ" => "Caucasia, Colombia ",
        "CXJ" => "Caxias Do Sul, Brazil ",
        "CAY" => "Cayenne, French Guiana ",
        "CYB" => "Cayman Brac Is, Cambodia ",
        "CYO" => "Cayo Largo Del Sur, Cuba ",
        "CEB" => "Cebu, Philippines - Matan International ",
        "CDC" => "Cedar City, UT ",
        "CID" => "Cedar Rapids, IA ",
        "CED" => "Cedun, Australia ",
        "CEM" => "Central, AK ",
        "JCU" => "Ceuta, Spain and Canary Islands ",
        "CDR" => "Chadron, NE ",
        "ZBR" => "Chah-Bahar, Iran ",
        "CIK" => "Chalkyitsik, AK ",
        "CMF" => "Chambery, France ",
        "XCI" => "Chambord, QC - Rail service ",
        "CMI" => "Urbana/Champaign, IL ",
        "IXC" => "Chandigarh, India ",
        "XDL" => "Chandler, QC - Rail service ",
        "CGQ" => "Changchun, China ",
        "CGD" => "Changde, China ",
        "CHX" => "Changuinda, Panama ",
        "CZX" => "Changzhou, China ",
        "CHQ" => "Chania, Greece ",
        "CHG" => "Chaoyang, China ",
        "XAP" => "Chapeco, Brazil ",
        "YLD" => "Chapleau, ON ",
        "CHS" => "Charleston, SC ",
        "CRW" => "Charleston, WV ",
        "CTL" => "Charleville, Australia ",
        "CLT" => "Charlotte, NC ",
        "CHO" => "Charlottesville, VA ",
        "YHG" => "Charlottetown, NL ",
        "YYG" => "Charlottetown, PE ",
        "CHT" => "Chatham Island, New Zealand ",
        "XCM" => "Chatham, ON ",
        "CHA" => "Chattanooga, TN ",
        "CSY" => "Cheboksary, Russia ",
        "CYF" => "Chefornak, AK ",
        "CEK" => "Chelybinsk, Russia ",
        "XHS" => "Chemainus, BC - Rail service ",
        "MAA" => "Chennai, India ",
        "CJJ" => "Cheongju, South Korea ",
        "CEE" => "Chevepovets, Russia ",
        "CTU" => "Chergdu, China ",
        "CEG" => "Chester, United Kingdom ",
        "YCS" => "Chesterfield Inlet, NU ",
        "CTM" => "Chetumal, Mexico ",
        "VAK" => "Chevak, AK ",
        "YHR" => "Chevery, QC ",
        "CYS" => "Cheyenne, WY ",
        "CMJ" => "Chi Mei, Taiwan ",
        "CNX" => "Chiang Mai, Thailand ",
        "CEI" => "Chiang Rai, Thailand ",
        "CYI" => "Chiayi, Taiwan ",
        "YMT" => "Chibougamau, QC ",
        "CHI" => "Chicago, IL - All airports ",
        "MDW" => "Chicago, IL - Midway ",
        "ORD" => "Chicago, IL - O'Hare ",
        "CIX" => "Chicayo, Peru ",
        "CKX" => "Chicken, AK ",
        "CIC" => "Chico, CA ",
        "CIF" => "Chifeng, China ",
        "KCQ" => "Chignik, AK - ",
        "KCG" => "Chignik, AK - Fisheries ",
        "KCL" => "Chignik, AK - Lagoon ",
        "CUU" => "Chihuahua, Mexico ",
        "YAI" => "Chillan, Chile ",
        "CIP" => "Chipata, Zambia ",
        "CZN" => "Chisana, AK ",
        "YKU" => "Chisasibi, QC ",
        "HIB" => "Hibbing/Chisholm, MN ",
        "KIV" => "Chisinau, Republic of Moldova ",
        "HTA" => "Chita, Russia ",
        "CJL" => "Chitral, Pakistan ",
        "CTD" => "Chitre, Panama ",
        "CGP" => "Chittagong, Bangladesh ",
        "CHY" => "Choiseul Bay, Solomon Islands ",
        "CKG" => "Chongqing, China ",
        "CHC" => "Christchurch, New Zealand ",
        "XCH" => "Christmas Island, Christmas Island ",
        "CHU" => "Chuathbaluk, AK ",
        "ZUM" => "Churchill Falls, NL ",
        "YYQ" => "Churchill, MB ",
        "XAD" => "Churchill, MB - Rail service ",
        "ICI" => "Cicia, Fiji ",
        "AVI" => "Ciego De Avila, Cuba ",
        "CVG" => "Cincinnati, OH ",
        "CHP" => "Circle Hot Springs, AK ",
        "IRC" => "Circle, AK ",
        "CBL" => "Ciudad Bolivar, Venezuela ",
        "CME" => "Ciudad Del Carmen, Mexico ",
        "AGT" => "Ciudad del Este, Paraguay ",
        "CJS" => "Ciudad Juarez, Mexico ",
        "CEN" => "Ciudad Obregon, Mexico ",
        "CVM" => "Ciudad Victoria, Mexico ",
        "CLP" => "Clarks Point, AK ",
        "CKB" => "Clarksburg, WV ",
        "PIE" => "St Petersburg/Clearwater, FL ",
        "CFE" => "Clermont-ferrand, France ",
        "CVC" => "Cleve, Australia ",
        "CLE" => "Cleveland, OH ",
        "CNJ" => "Cloncurry, Australia ",
        "CVN" => "Clovis, NM ",
        "CMK" => "Club Makokola, Malawi ",
        "CLJ" => "Cluj, Romania ",
        "YCY" => "Clyde River, NU ",
        "CAZ" => "Cobar, Australia ",
        "CIJ" => "Cobija, Bolivia ",
        "XGJ" => "Cobourg, ON - Rail service ",
        "CBB" => "Cochabamba, Bolivia ",
        "COK" => "Cochin, India ",
        "CNC" => "Coconut Island, Australia ",
        "Kee" => "Cocos Islands, Cocos ",
        "COD" => "Yellowstone/Cody, WY ",
        "CUQ" => "Coen, Australia ",
        "CFA" => "Coffee Point, AK ",
        "KCC" => "Coffman Cove, AK ",
        "CFS" => "Coffs Harbor, Australia ",
        "CJB" => "Coimbatore, India ",
        "CDB" => "Cold Bay, AK ",
        "CLQ" => "Colima, Mexico ",
        "CLL" => "College Station, TX ",
        "CGN" => "Cologne, Germany - Cologne/Bonn ",
        "QKL" => "Cologne, Germany - Rail service ",
        "CMB" => "Colombo, Sri Lanka ",
        "ONX" => "Colon, Panama ",
        "COS" => "Colorado Springs, CO ",
        "COU" => "Columbia, MO ",
        "CAE" => "Columbia, SC ",
        "CSG" => "Columbus, GA ",
        "GTR" => "Columbus, MS ",
        "CMH" => "Columbus, OH ",
        "YCK" => "Colville Lake, NT ",
        "YQQ" => "Comox, BC ",
        "CKY" => "Conakry, Guinea ",
        "CCP" => "Concepcion, Chile ",
        "CCR" => "Concord, CA ",
        "COC" => "Cotarou, Benin ",
        "CNK" => "Concordia, KS ",
        "COG" => "Condoto, Colombia ",
        "CND" => "Constanta, Romania ",
        "CZL" => "Constantine, Algeria ",
        "OTD" => "Contadora, Panama ",
        "CPD" => "Coober Pedy, Australia ",
        "CTN" => "Cooktown, Australia ",
        "OOM" => "Cooma, NS, Australia ",
        "CNB" => "Coonamble, Australia ",
        "CPH" => "Copenhagen, Denmark ",
        "CPO" => "Copiapo, Chile ",
        "QCE" => "Copper Mountain, CO - Van service ",
        "YZS" => "Coral Harbour, NU ",
        "COR" => "Cordoba, Argentina ",
        "CDV" => "Cordova, AK ",
        "ORK" => "Cork, Ireland ",
        "YCC" => "Cornwall, ON ",
        "CZE" => "Coro, Venezuela ",
        "CZH" => "Corozal, Belize ",
        "CRP" => "Corpus Christi, TX ",
        "CNQ" => "Corrientes, Argentina ",
        "CEZ" => "Cortez, CO ",
        "CMG" => "Corumba, Brazil ",
        "CVU" => "Corvo Island, Portugal ",
        "CBO" => "Cotabato, Philippines ",
        "XGK" => "Coteau, QC - Rail service ",
        "YCA" => "Courtenay, BC ",
        "CXB" => "Cox's Bazar, Bangladesh ",
        "CZM" => "Cozumel, Mexico ",
        "CCV" => "Craig Cove, Vanuatu ",
        "CGA" => "Craig, AK ",
        "YXC" => "Cranbrook, BC ",
        "CEC" => "Crescent City, CA ",
        "CCM" => "Criciuma, Brazil ",
        "CKI" => "Croker Island, Australia ",
        "CKO" => "Crooked Creek, AK ",
        "CRI" => "Crooked Island, Bahamas ",
        "YCR" => "Cross Lake, MB ",
        "CRV" => "Crotone, Italy ",
        "CZS" => "Cruzeiro Do Sul, Brazil ",
        "CUW" => "Cube Cove, AK ",
        "CUC" => "Cucata, Colombia ",
        "CUE" => "Cuenca, Ecuador ",
        "CVJ" => "Cuernavaca, Mexico ",
        "CGB" => "Cuiaba, Brazil ",
        "CUL" => "Culiacan, Mexico ",
        "CUM" => "Cumana, Venezuela ",
        "CBE" => "Cumberland, MD ",
        "CMA" => "Cunnamulla, Australia ",
        "CUR" => "Curacao, Netherlands Antilles ",
        "CWB" => "Curitiba, Brazil ",
        "CUZ" => "Cuzco, Peru ",
        "DAD" => "Da Nang, Viet Nam ",
        "DRH" => "Dabra, Indonesia ",
        "TAE" => "Daegu, South Korea ",
        "DKR" => "Dakar, Senegal ",
        "VIL" => "Dakhla, Morocco ",
        "DLM" => "Dalaman, Turkey ",
        "" => "Dalat, Viet Nam - Lienkhang DLI)",
        "DLU" => "Dali City, China ",
        "DLC" => "Dalian, China ",
        "DFW" => "Fort Worth/Dallas, TX ",
        "DAL" => "Dallas, TX - Love Field ",
        "DAM" => "Damascus, Syrian Arab Republic ",
        "DMM" => "Dammam, Saudi Arabia ",
        "DGA" => "Dangriga, Belize ",
        "DAR" => "Dar Es Salaam, Tanzania ",
        "NLF" => "Darnley Island, QL, Australia ",
        "DAU" => "Daru, Papua New Guinea ",
        "DRW" => "Darwin, Northern Territory, Australia ",
        "DTD" => "Datadawai, Indonesia ",
        "YDN" => "Dauphin, MB ",
        "DVO" => "Davao, Philipines - Mati ",
        "DAV" => "David, Panama ",
        "YDI" => "Davis Inlet, NL ",
        "TVY" => "Dawe, Myanmar ",
        "YDA" => "Dawson City, YT ",
        "YDQ" => "Dawson Creek, BC ",
        "DAX" => "Daxian, China ",
        "DYG" => "Dayang, China ",
        "DDI" => "Daydream Is, Australia ",
        "DAY" => "Dayton, OH ",
        "DAB" => "Daytona Beach, FL ",
        "DOL" => "Deauville, France ",
        "DBM" => "Debra Marcos, Ethiopia ",
        "DBT" => "Debra Tabor, Ethiopia ",
        "DEC" => "Decatur, IL ",
        "YDF" => "Deer Lake, NL ",
        "YVZ" => "Deer Lake, ON ",
        "DRG" => "Deering, AK ",
        "DEZ" => "Deirezzor, Syria - Al Jafrah ",
        "DRT" => "Del Reo, TX ",
        "DEL" => "Delhi, India ",
        "YWJ" => "Deline, NT ",
        "DJN" => "Delta Junction, AK ",
        "DEM" => "Dembidollo, Ethiopia ",
        "DNM" => "Denham, Australia ",
        "DNZ" => "Denizli, Turkey ",
        "DPS" => "Denpasar Bali, Indonesia ",
        "DEN" => "Denver, CO - International ",
        "QWM" => "Denver, CO - Longmont Bus service ",
        "DEA" => "Dera Ghazi, Pakistan ",
        "DSK" => "Dera Ismail Khan, Pakistan ",
        "DRB" => "Derby, Australia ",
        "DER" => "Derim, Papua New Guinea ",
        "DSM" => "Des Moines, IA ",
        "DSE" => "Dessie, Ethiopia ",
        "DTT" => "Detroit, MI - All airports ",
        "DTW" => "Detroit, MI - Metro/Wayne County ",
        "DVL" => "Devil's Lake, ND ",
        "DPO" => "Devonport, Australia ",
        "DAC" => "Dhaka, Bangledesh - Zia International ",
        "DIB" => "Dibrugarn, India ",
        "DIK" => "Dickinson, ND ",
        "DIN" => "Dien Bien Phu, Viet Nam - Gialam ",
        "DIJ" => "Dijon, France ",
        "DIL" => "Dili, Indonesia ",
        "DLG" => "Dillingham, AK ",
        "DLY" => "Dillons Bay, Vanuata ",
        "DMU" => "Dimapur, India ",
        "DNR" => "Dinard, France ",
        "DPL" => "Dipolog, Philippines ",
        "DIR" => "Dire Dawa, Ethiopia ",
        "DIU" => "Div, India ",
        "DIY" => "Diyarbakir, Turkey ",
        "DJG" => "Djanet, Algeria ",
        "DJE" => "Djerba, Tunisia ",
        "JIB" => "Djibouti, Djibouti ",
        "DNK" => "Dnepropetrovsk, Ukraine ",
        "DOB" => "Dobo, Indonesia ",
        "DDC" => "Dodge City, KS ",
        "DDM" => "Dodoima, Papua New Guinea ",
        "DOD" => "Dodoma, Tanzania ",
        "DOH" => "Doha, Qatar ",
        "DCF" => "Dominica, Dominica - Cane Field ",
        "DOM" => "Dominica, Dominica - Melville Hall ",
        "CFN" => "Donegal, Ireland ",
        "DOK" => "Donetsk, Ukraine ",
        "DOG" => "Dongola, Sudan ",
        "DMD" => "Doomadgee, Australia ",
        "DTM" => "Dortmund, Germany ",
        "DHN" => "Dothan, AL ",
        "DOU" => "Dourados, Brazil ",
        "DLA" => "Dovala, Cameroon ",
        "DRS" => "Dresden, Germany ",
        "XDM" => "Drummondville, QC - Rail service ",
        "YHD" => "Dryden, ON ",
        "XNB" => "Dubai, United Arab Emirates - Bus Station ",
        "DXB" => "Dubai, United Arab Emirates - International ",
        "DBO" => "Dubbo, Australia ",
        "DUB" => "Dublin, Ireland ",
        "DUJ" => "Dubois, PA ",
        "DBV" => "Dubrovnik, Croatia ",
        "DBQ" => "Dubuque, IA ",
        "DLH" => "Duluth, MN ",
        "DGT" => "Dumaguete, Philippines ",
        "DUM" => "Dumai, Indonesia ",
        "DUQ" => "Duncan/Quam, BC ",
        "DND" => "Dundee, United Kingdom ",
        "DUD" => "Dunedin, New Zealand ",
        "DNH" => "Dunhuang, China ",
        "DKI" => "Dunk Island, Australia ",
        "DRO" => "Durango, CO ",
        "DGO" => "Durango, Mexico ",
        "DUR" => "Durban, South Africa ",
        "RDU" => "Raleigh/Durham, NC ",
        "DYU" => "Dushanbe, Tajikistan ",
        "DUS" => "Dusseldorf, Germany - International ",
        "MGL" => "Dusseldorf, Germany - Moenchen-Gl. ",
        "QDU" => "Dusseldorf, Germany - Rail service ",
        "DUT" => "Dutch Harbor, AK ",
        "DZA" => "Dzaoudzi, Mayotte ",
        "ELS" => "East London, South Africa ",
        "ZEM" => "East Main, QC ",
        "EAU" => "Eau Claire, WI ",
        "EBO" => "Ebon, Marshall Islands ",
        "EOI" => "Eday, United Kingdom ",
        "EDI" => "Edinburgh, United Kingdom ",
        "YEG" => "Edmonton, AB - International ",
        "XZL" => "Edmonton, AB - Rail service ",
        "EDA" => "Edna Bay, AK ",
        "EDO" => "Edremit, Turkey ",
        "EDR" => "Edward River, Australia ",
        "EEK" => "Eek, AK ",
        "EGS" => "Egilsstadir, Iceland ",
        "EIN" => "Eindhoven, Netherlands ",
        "EIB" => "Eisenach, Germany ",
        "SVX" => "Ekaterinburg, Russia ",
        "KKU" => "Ekuk, AK ",
        "KEK" => "Ekwok, AK ",
        "EHL" => "El Bolsan, Argentina ",
        "IPL" => "Imperial, CA ",
        "ELD" => "El Dorado, AR ",
        "ELF" => "El Fasher, Sudan ",
        "EMX" => "El Maiten, Argentina ",
        "EBD" => "El Obeid, Sudan ",
        "ELU" => "El Oved, Algeria ",
        "ELP" => "El Paso, TX ",
        "EPS" => "El Portillo/Samana, Dominician Republic - El Portillo",
        "ELE" => "El Real, Panama ",
        "ESR" => "El Salvador, Chile ",
        "VIG" => "El Vigia, Venezuela ",
        "EYP" => "El Yopal, Colombia ",
        "ETH" => "Elat, Italy ",
        "EZS" => "Elazig, Turkey ",
        "EBA" => "Elba Island, Italy ",
        "ELC" => "Elcho Island, Australia ",
        "EDL" => "Eldoret, Kenya ",
        "ELH" => "North Eleuthera, Bahamas ",
        "ELV" => "Elfin Cove, AK ",
        "ELI" => "Elim, AK ",
        "ESL" => "Elista, Russia ",
        "EKO" => "Elko, NV ",
        "ELM" => "Elmira, NY ",
        "LYU" => "Ely, MN ",
        "EAE" => "Emae, Vanuata ",
        "EMS" => "Embessa, Papua New Guinea ",
        "EMD" => "Emerald, Australia ",
        "EMK" => "Emmonak, AK ",
        "EMO" => "Emo, Papua New Guinea ",
        "EWI" => "Enarotali, Indonesia ",
        "ENE" => "Ende, Indonesia ",
        "ENT" => "Enewetak Island, Marshall Islands ",
        "WDG" => "Enid, OK ",
        "ENF" => "Enontekio, Finland ",
        "ENH" => "Enshi, China ",
        "EBB" => "Entebbe, Uganda ",
        "ENU" => "Enugu, Nigeria ",
        "EPL" => "Epinal, France ",
        "ECN" => "Ercan, Cyprus ",
        "ERF" => "Erfurt, Germany ",
        "ERI" => "Erie, PA ",
        "ERC" => "Erzincan, Turkey ",
        "ERZ" => "Erzurum, Turkey ",
        "EBJ" => "Esbjerg, Denmark - Esbjerg Airport ",
        "ZBB" => "Esbjerg, Denmark - Rail service ",
        "ESC" => "Escanaba, MI ",
        "ESM" => "Esmeraldas, Ecuador ",
        "EPR" => "Esperance, Australia ",
        "SON" => "Espiritu Santo, Vanuatu ",
        "EQS" => "Esquel, Argentina ",
        "YPF" => "Esquimalt, BC ",
        "EUG" => "Eugene, OR ",
        "EUE" => "Eureka, NV ",
        "EVV" => "Evansville, IN ",
        "EXT" => "Eveter, United Kingdom ",
        "EWE" => "Ewer, Indonesia ",
        "EXM" => "Exmouth Gulf, Australia ",
        "VDB" => "Fagernes, Norway ",
        "FIE" => "Fair Isle, United Kingdom ",
        "FAI" => "Fairbanks, AK ",
        "LYP" => "Faisalabad, Pakistan ",
        "FAJ" => "Fajard, Puerto Rico ",
        "FKQ" => "Fak Fak, Indonesia ",
        "FAV" => "Fakarava, French Polynesia ",
        "RVA" => "Farafangana, Madagascar ",
        "FAR" => "Fargo, ND ",
        "FMN" => "Farmington, NM ",
        "FAO" => "Faro, Portugal ",
        "FAE" => "Faroe Islands, Faroe Islands ",
        "FYV" => "Fayetteville, AR - Municipal/Drake ",
        "XNA" => "Fayetteville, AR - Northwest Arkansas Regional ",
        "FAY" => "Fayetteville, NC ",
        "FRE" => "Fera Island, Solomon Islands ",
        "FEG" => "Fergana, Uzbekistan ",
        "FEN" => "Fernando De Noronha, Brazil ",
        "FEZ" => "Fez, Morocco ",
        "WFI" => "Fianarantsoa, Madagascar ",
        "FSC" => "Figari, France ",
        "FZO" => "Filton, United Kingdom ",
        "XFW" => "Finkenwerder, Germany ",
        "FIZ" => "Fitzroy Crossing, Australia ",
        "FLG" => "Flagstaff, AZ ",
        "FLF" => "Flensburg, Germany ",
        "YFO" => "Flin Flon, MB ",
        "FNT" => "Flint, MI ",
        "PSA" => "Pisa, Italy - Gal Galilei ",
        "FLR" => "Florence, Italy - Peretola ",
        "FLO" => "Florence, SC ",
        "MSL" => "Sheffield/Florence/Muscle Shoals, AL ",
        "FLA" => "Florencia, Colombia ",
        "FLW" => "Flores Island, Portugal ",
        "FRS" => "Flores, Guatemala ",
        "FLN" => "Florianopolis, Brazil ",
        "FRO" => "Floro, Norway ",
        "FOG" => "Foggia, Italy ",
        "ZFD" => "Fond du Lac, SK ",
        "FDE" => "Forde, Norway ",
        "FMA" => "Formosa, Argentina ",
        "YFA" => "Fort Albany, ON ",
        "YPY" => "Fort Chipewyan, AB ",
        "QWF" => "Loveland/Fort Collins, CO - Bus service ",
        "FNL" => "Loveland/Fort Collins, CO - Municipal Airport ",
        "FTU" => "Fort Dauphin, Madagascar ",
        "FDF" => "Fort De France, Martinique ",
        "FOD" => "Fort Dodge, IA ",
        "YAG" => "Fort Frances, ON ",
        "YGH" => "Fort Good Hope, NT ",
        "YFH" => "Fort Hope, ON ",
        "FLL" => "Fort Lauderdale, FL ",
        "TBN" => "Fort Leonard Wood, MO ",
        "YMM" => "Fort Mcmurray, AB ",
        "RSW" => "Fort Myers, FL ",
        "YYE" => "Fort Nelson, BC ",
        "YER" => "Fort Severn, ON ",
        "YFS" => "Fort Simpson, NT ",
        "FSM" => "Fort Smith, AR ",
        "YSM" => "Fort Smith, NT ",
        "YXJ" => "Fort St John, BC ",
        "VPS" => "Valparaiso, FL ",
        "FWA" => "Fort Wayne, IN ",
        "FOR" => "Fortaleza, Brazil ",
        "YFX" => "Fox Harbour/St Lewis, NL ",
        "FRC" => "Franca, Brazil ",
        "MVB" => "Franceville, Gabon ",
        "FRW" => "Francistown, Botswana ",
        "HHN" => "Frankfurt, Germany - Hahn ",
        "FRA" => "Frankfurt, Germany - International ",
        "FKL" => "Franklin, PA ",
        "ZBJ" => "Fredericia, Denmark ",
        "XFC" => "Fredericton Junction, NB - Rail service ",
        "YFC" => "Fredericton, NB ",
        "FPO" => "Freeport, Bahamas ",
        "FNA" => "Freetown, Sierra Leone - Lungi Intl ",
        "FAT" => "Fresno, CA ",
        "FDH" => "Friedrichshafer, Germany ",
        "FUE" => "Puerto del Rosario, Spain ",
        "FUJ" => "Fukue, Japan ",
        "FUK" => "Fukuoka, Japan ",
        "FKS" => "Fukushima, Japan ",
        "FUN" => "Funafuti Atol, Tuvalu ",
        "FNC" => "Funchal, Portugal ",
        "FTA" => "Futuna Island, Vanuatu ",
        "FUT" => "Futuna Island, Wallis and Futuna Islands ",
        "FUG" => "Fuyang, China ",
        "FOC" => "Fuzhou, China ",
        "GBE" => "Gaborone, Botswana ",
        "GAF" => "Gafsa, Tunisia ",
        "GGN" => "Gagnoa, Cote D'Ivoire ",
        "GNV" => "Gainesville, FL ",
        "GPS" => "Galapagos, Ecuador ",
        "GEV" => "Gallivare, Sweden ",
        "GUP" => "Gallup, NM ",
        "GWY" => "Galway, Ireland ",
        "GAX" => "Gamba, Gabon ",
        "GMB" => "Gambela, Ethiopia ",
        "GAN" => "Gan Island, Maldives ",
        "YQX" => "Gander, NL ",
        "KAG" => "Gangneung, South Korea ",
        "GHE" => "Garachine, Panama ",
        "GAR" => "Garaina, Papua New Guinea ",
        "GRL" => "Garasa, Papua New Guinea ",
        "GCK" => "Garden City, KS ",
        "GPN" => "Garden Point, Australia ",
        "GOV" => "Gove, Australia ",
        "GYY" => "Gary, IN ",
        "YGP" => "Gaspe, QC ",
        "XDD" => "Gaspe, QC - Rail service ",
        "ELQ" => "Gassim, Saudi Arabia ",
        "ZGU" => "Gaua, Vanuatu ",
        "GAU" => "Gawahati, India ",
        "GZA" => "Gaza City, Occupied Palestinian Territory ",
        "GZT" => "Gaziatep, Turkey ",
        "GDN" => "Gdansk, Poland ",
        "GEB" => "Gebe, Indonesia ",
        "GDZ" => "Gelendzik, Russia ",
        "EGN" => "Geneina, Sudan ",
        "GES" => "General Santos, Philippines ",
        "GVA" => "Geneva, Switzerland ",
        "GOA" => "Genoa, Italy ",
        "GGT" => "George Town, Bahamas ",
        "GRJ" => "George, South Africa ",
        "GEO" => "Georgetown, Guyana ",
        "XHM" => "Georgetown, ON - Rail service ",
        "GET" => "Geraldton, Australia, ",
        "GRO" => "Gerona, Spain ",
        "ZGS" => "Gethsemani, QC ",
        "LTD" => "Ghadames, Libya ",
        "GHA" => "Ghardala, Algeria ",
        "GHT" => "Ghat, Libya ",
        "GIB" => "Gibraltar, Gibraltar ",
        "GIL" => "Gilgit, Pakistan ",
        "YGX" => "Gillam, MB ",
        "GCC" => "Gillette, WY ",
        "YGB" => "Gillies Bay, BC ",
        "GIS" => "Gisborne, New Zealand ",
        "GIZ" => "Gizan, Saudi Arabia ",
        "GZO" => "Gizo, Solomon Islands ",
        "YHK" => "Gjoa Haven, NU ",
        "GGG" => "Longview, TX ",
        "GLT" => "Gladstone, Australia ",
        "GGW" => "Glasgow, MT ",
        "GLA" => "Glasgow, United Kingdom - Glasgow International ",
        "PIK" => "Glasgow, United Kingdom - Prestwick ",
        "GLI" => "Glen Innes, Australia ",
        "XZC" => "Glencoe, ON - Rail service ",
        "GDV" => "Glendive, MT ",
        "GOI" => "Goa, India ",
        "GOB" => "Goba, Ethiopia ",
        "GGS" => "Gobernador Gregores, Argentina ",
        "GDE" => "Gode/Iddidole, Ethopia ",
        "YGO" => "Gods Narrows, MB ",
        "ZGI" => "Gods River, MB ",
        "GYN" => "Goiania, Brazil ",
        "OOL" => "Gold Coast, QL, Australia ",
        "GLF" => "Golfito, Costa Rica ",
        "GOQ" => "Golmud, China ",
        "GLV" => "Golovin, AK ",
        "GOE" => "Gonalia, Papua New Guinea ",
        "GDQ" => "Gondari, Ethiopia ",
        "GNU" => "Goodnews Bay, AK ",
        "YYR" => "Goose Bay, NL ",
        "GOR" => "Gore, Ethiopia ",
        "GKA" => "Goroka, Papua New Guinea ",
        "GTO" => "Gorontalo, Indonesia ",
        "GOT" => "Gothenburg, Sweden - Landvetter ",
        "GSE" => "Gothenburg, Sweden - Saeve ",
        "GBL" => "Goulburn Island, Australia ",
        "GUD" => "Goundam, Mali ",
        "GVR" => "Governador Valadares, Brazil ",
        "GHB" => "Governors Harbour, Bahamas ",
        "OYA" => "Goya, CR, Argentina ",
        "GZM" => "Gozo, Malta ",
        "GRW" => "Graciosa Island, Portugal ",
        "GFN" => "Grafton, Australia ",
        "GRX" => "Granada, Spain ",
        "JGC" => "Grand Canyon, AZ - Heliport ",
        "GCN" => "Grand Canyon, AZ - National Park ",
        "GCM" => "Grand Cayman, Cayman Islands ",
        "GFK" => "Grand Forks, ND ",
        "GRI" => "Grand Island, NE ",
        "GJT" => "Grand Junction, CO ",
        "GRR" => "Grand Rapids, MI ",
        "GPZ" => "Grand Rapids, MN ",
        "GDT" => "Grand Turk Island, Turks and Caicos Islands ",
        "YQU" => "Grande Prairie, AB ",
        "KGX" => "Grayling, AK ",
        "GRZ" => "Graz, Austria ",
        "GTF" => "Great Falls, MT ",
        "GRB" => "Green Bay, WI ",
        "GSO" => "High Point, NC ",
        "GLH" => "Greenville, MS ",
        "PGV" => "Greenville, NC ",
        "GSP" => "Spartanburg/Greenville, SC ",
        "GND" => "Grenada, Grenada, ",
        "GNB" => "Grenoble, France ",
        "GFF" => "Griffith, Australia ",
        "XGY" => "Grimsby, ON ",
        "GRY" => "Grimsey, Iceland ",
        "YGZ" => "Grise Fiord, NU ",
        "JGR" => "Groennedal, Greenland ",
        "GRQ" => "Groningen, Netherlands ",
        "GTE" => "Groofe Eylandt, Australia ",
        "GON" => "New London/Groton ",
        "GDL" => "Guadalajara, Mexico ",
        "GUM" => "Guam ",
        "GJA" => "Guanaja, Honduras ",
        "BJX" => "Leon, Mexico ",
        "CAN" => "Guangzhou, China ",
        "GAO" => "Guantanamo, Cuba ",
        "GUA" => "Guatemala City, Guatemala ",
        "GYE" => "Guayaquil, Ecuador ",
        "GYA" => "Guayaramerin, Bolivia ",
        "GYM" => "Guaymas, Mexico ",
        "XIA" => "Guelph, ON - Rail service ",
        "GCI" => "Guernsey, United Kingdom ",
        "GUB" => "Guerrero Negro, Mexico ",
        "KWL" => "Guilin, China ",
        "GUI" => "Guiria, Venezuela ",
        "ULU" => "Gulu, Uganda ",
        "KWE" => "Gulyang, China ",
        "GUC" => "Gunnison, CO ",
        "KUV" => "Gunsan, South Korea ",
        "URY" => "Gurayat, Saudi Arabia ",
        "GST" => "Gustavus, AK ",
        "GWD" => "Gwadar, Pakistan ",
        "GWL" => "Gwalior, India ",
        "KWJ" => "Gwangju, South Korea ",
        "KVD" => "Gyandzha, Azerbaijan ",
        "LWN" => "Gyourmri, Armenia ",
        "HPA" => "HaApa, Tonga ",
        "HAC" => "Hachijo Jima, Japan ",
        "HGR" => "Hagerstown, MD ",
        "HFS" => "Hagfors, Sweden ",
        "HFA" => "Haifa, Israel ",
        "HAK" => "Haikou, China ",
        "HAS" => "Hail, Saudi Arabia ",
        "HLD" => "Hailar, China ",
        "SUN" => "Sun Valley, ID ",
        "HNS" => "Haines, AK ",
        "HPH" => "Haiphong, Viet Nam - Catbi ",
        "HKD" => "Hakodate, Japan ",
        "ZHQ" => "Halberstadt, Germany ",
        "YHZ" => "Halifax, NS - International ",
        "XDG" => "Halifax, NS - Rail service ",
        "YUX" => "Hall Beach, NU ",
        "HCQ" => "Halls Creek, Australia ",
        "HAD" => "Halmstad, Sweden ",
        "HAM" => "Hamburg, Germany - Fuhisbuettel ",
        "LBC" => "Hamburg, Germany - Luebeck ",
        "HTI" => "Hamilton Island, Australia ",
        "HLZ" => "Hamilton, New Zealand ",
        "YHM" => "Hamilton, ON ",
        "HFT" => "Hammerfest, Norway ",
        "PHF" => "Williamsburg, VA ",
        "HNM" => "Hana, HI - Island of Maui ",
        "PAK" => "Hanapepe, HI ",
        "CMX" => "Hancock, MI ",
        "HGH" => "Hangzhou, China ",
        "HAQ" => "Hanimaadhoo, Maldives ",
        "HAN" => "Hanoi, Viet Nam - Noibai ",
        "HAJ" => "Hanover, Germany ",
        "LEB" => "White River, VT ",
        "HZG" => "Hanzhang, China ",
        "HRE" => "Harare, Zimbabwe ",
        "HRB" => "Harbin, China ",
        "HGA" => "Hargeisa, Somolia ",
        "HRL" => "Harlingen, TX ",
        "MDT" => "Harrisburg, PA ",
        "HRO" => "Harrison, AR ",
        "EVE" => "Harstad-Narvik, Norway ",
        "BDL" => "Windsor Locks, CT ",
        "HME" => "Hassi Messaoud, Algeria ",
        "HAA" => "Hasvik, Norway ",
        "HDY" => "Hat Yai, Thailand ",
        "HTR" => "Hateruma, Japan ",
        "HAU" => "Haugesund, Norway ",
        "HAV" => "Havana, Cuba ",
        "HAE" => "Havasupai, AZ ",
        "YGV" => "Havre St Pierre, QC ",
        "HVR" => "Havre, MT ",
        "YHY" => "Hay River, NT ",
        "HDN" => "Hayden, CO ",
        "HIS" => "Hayman Island, Australia ",
        "HYS" => "Hays, KS ",
        "HKB" => "Healy Lake, AK ",
        "HFE" => "Hefei, China ",
        "HDB" => "Heidelberg, Germany ",
        "HLN" => "Helena, MT ",
        "HGL" => "Helgoland, Germany ",
        "HEL" => "Helsinki, Finland ",
        "HEH" => "Heno, Myanmar ",
        "HER" => "Heraklian, Greece ",
        "HDF" => "Heringsdorf, Germany ",
        "HMV" => "Hermavan, Sweden ",
        "HMO" => "Hermosillo, Mexico ",
        "XAK" => "Herning, Denmark ",
        "HVB" => "Hervey Bay, Australia ",
        "XDU" => "Hervey, QC - Rail service ",
        "HKY" => "Hickory, NC ",
        "YOJ" => "High Level, AB ",
        "ITO" => "Hilo, HI - Island of Hawaii ",
        "HHH" => "Hilton Head, SC ",
        "HIW" => "Hiroshima, Japan - Hiroshima West ",
        "HIJ" => "Hiroshima, Japan - International ",
        "HIT" => "Hivaro, Papua New Guinea ",
        "SGN" => "Ho Chi Minh City, Viet Nam ",
        "HBA" => "Hobart, Australia ",
        "HBB" => "Hobbs, NM ",
        "HOD" => "Hodeidah, Yemen ",
        "HDS" => "Hoedspruit, South Africa ",
        "HOQ" => "Hof, Germany ",
        "HOF" => "Hofuf, Saudi Arabia ",
        "HET" => "Hohhot, China ",
        "HKK" => "Hokitika, New Zealand ",
        "HOG" => "Holguin, Cuba ",
        "HYL" => "Hollis, AK ",
        "YHI" => "Holman, NT ",
        "HCR" => "Holy Cross, AK ",
        "HOM" => "Homer, AK ",
        "HKG" => "Hong Kong, Hong Kong ",
        "HIR" => "Honiara, Solomon Islands ",
        "HVG" => "Honningsvag, Norway ",
        "HNL" => "Honolulu, HI - Island of Oahu ",
        "HOK" => "Hooker, Australia ",
        "MKK" => "Hoolehua, HI - Island of Molokai ",
        "HNH" => "Hoonah, AK ",
        "HPB" => "Hooper Bay, AK ",
        "YHO" => "Hopedale, NL ",
        "HID" => "Horn Island Australia ",
        "HFN" => "Hornafjordur, Iceland ",
        "HOR" => "Horta, Portugal ",
        "HKN" => "Hoskins, Papua New Guinea ",
        "HOT" => "Hot Springs, AR ",
        "HTN" => "Hotan, China ",
        "HOE" => "Houeisay, Laos ",
        "HUQ" => "Houn, Libya ",
        "ZHO" => "Houston, BC - Bus station ",
        "HOU" => "Houston, TX - Hobby ",
        "IAH" => "Houston, TX - Intercontinental ",
        "HUH" => "Huahine, French Polynesia ",
        "HUN" => "Hualien, Taiwan - Phi Bai ",
        "HHQ" => "Hualtin, Thailand ",
        "HUU" => "Huanuco, French Polynesia ",
        "HYN" => "Huargyan, China ",
        "HUX" => "Huatulco, Mexico ",
        "HUV" => "Hudiksvall, Sweden ",
        "YHB" => "Hudson Bay, SK ",
        "HUI" => "Hue, Viet Nam ",
        "HGD" => "Hughenden, Australia ",
        "HUS" => "Hughes, AK ",
        "HLF" => "Hultsfred, Sweden ",
        "HUY" => "Humberside, United Kingdom ",
        "HSV" => "Huntsville, AL ",
        "HRG" => "Hurghada, Egypt ",
        "HON" => "Huron, SD ",
        "HSL" => "Huslia, AK ",
        "HWN" => "Hwange Nat Park, Zimbabwe ",
        "HYA" => "Hyannis, MA ",
        "HYG" => "Hydaburg, AK ",
        "HYD" => "Hyderabad, India ",
        "IAS" => "Iasi, Romania ",
        "IBE" => "Ibague, Colombia ",
        "IBZ" => "Ibiza, Spain ",
        "IDA" => "Idaho Falls, ID ",
        "IAA" => "Igarka, Russia ",
        "IGG" => "Igiugig, AK ",
        "YGT" => "Igloolik, NU ",
        "IGU" => "Iguassu Falls, PR, Brazil ",
        "IGR" => "Iguazu, Argentina ",
        "IHU" => "Ihu, Papua New Guinea ",
        "ILP" => "Ile Des Pins, New Caledonia ",
        "YGR" => "Iles De La Madeleine, QC ",
        "ILF" => "Ilford, MB ",
        "IOS" => "Ilheus, Brazil ",
        "ILI" => "Iliamna, AK ",
        "ILA" => "Illaga, Indonesia ",
        "ILO" => "Iloilo, Philippines - Mandurriao ",
        "IUL" => "Ilu, Indonesia ",
        "JAV" => "Ilulissat, Greenland ",
        "IMP" => "Imperatriz, Brazil ",
        "IMF" => "Imphal, India ",
        "IAM" => "In Amenas, Algeria ",
        "IGA" => "Inagua, Bahamas ",
        "INX" => "Inanwatan, Indonesia ",
        "IDN" => "Indagen, Papua New Guinea ",
        "IND" => "Indianapolis, IN ",
        "IDR" => "Indore, India ",
        "XIB" => "Ingersoll, ON - Rail service ",
        "INN" => "Innsbruck, Austria ",
        "INA" => "Inta, Russia ",
        "INL" => "International Falls, MN ",
        "YPH" => "Inukjuak, QC ",
        "YEV" => "Inuvik, NT ",
        "IVC" => "Invercargill, New Zealand ",
        "IVR" => "Inverell, Australia ",
        "INV" => "Inverness, United Kingdom ",
        "IYK" => "Inyokern, CA ",
        "IOA" => "Ioannina, Greece ",
        "IOP" => "Ioma, Papua New Guinea ",
        "IPN" => "Ipatinga, Brazil ",
        "IPI" => "Ipiales, Colombia ",
        "IPE" => "Ipil, Philippines ",
        "IPH" => "Ipoh, Malaysia ",
        "IPA" => "Ipota, Vanuatu ",
        "YFB" => "Iqaluit, NU ",
        "IQQ" => "Iquique, Chile ",
        "IQT" => "Iquitos, Peru ",
        "IKT" => "Irkutsk, Russia ",
        "IMT" => "Iron Mountain, MI ",
        "IWD" => "Ironwood, MI ",
        "IFJ" => "Isafjordur, Iceland ",
        "IFN" => "Isfahan, Iran ",
        "ISG" => "Ishigakij, Japan ",
        "ISB" => "Islamabad, Pakistan ",
        "YIV" => "Island Lake/Garden Hill, Canada ",
        "ILY" => "Islay, United Kingdom ",
        "IOM" => "Isle of Man, United Kingdom ",
        "ISC" => "Isles of Scilly, United Kingdom - St Marys ",
        "TSO" => "Isles of Scilly, United Kingdom - Tresco ",
        "ISP" => "Islip, NY ",
        "IST" => "Istanbul, Turkey ",
        "ITB" => "Itaituba, Brazil ",
        "ITH" => "Ithaca, NY ",
        "ITK" => "Itokama, Papua New Guinea ",
        "IVL" => "Ivalo, Finland ",
        "IFO" => "Ivano-Frankovsk, Ukraine ",
        "YIK" => "Ivujivik, QC ",
        "IWJ" => "Iwami, Japan ",
        "ZIH" => "Zihuatanejo/Ixtapa, Mexico ",
        "IZT" => "Ixtepec, Mexico ",
        "ADB" => "Izmir, Turkey ",
        "IZO" => "Izumo, Japan ",
        "JAT" => "Jabor, Marshall Islands ",
        "JCR" => "Jacareacanga, Brazil ",
        "JAC" => "Jackson Hole, WY ",
        "JAN" => "Jackson, MS ",
        "MKL" => "Jackson, TN ",
        "JAX" => "Jacksonville, FL ",
        "OAJ" => "Jacksonville, NC ",
        "JAG" => "Jacobabad, Pakistan ",
        "JAQ" => "Jacquinot Bay, Papua New Guinea ",
        "JAI" => "Jaipur, India ",
        "CGK" => "Jakarta, Indonesia ",
        "JAL" => "Jalapa, Mexico ",
        "UIT" => "Jaluit Island, Marshall Islands ",
        "DJB" => "Jambi. Indonesia ",
        "JMS" => "Jamestown, ND ",
        "JHW" => "Jamestown, NY ",
        "JGA" => "Jamnagar, India ",
        "JKR" => "Janakpur, Nepal ",
        "JVL" => "Janesville, WI ",
        "JQE" => "Jaque, Panama ",
        "XDH" => "Jasper, AB - Rail service ",
        "DJJ" => "Jayapura, Indonesia ",
        "JED" => "Jeddah, Saudi Arabia ",
        "JEJ" => "Jeh, Marshall Islands ",
        "CJU" => "Jeju, South Korea - Jeju Airport, metro area ",
        "XRY" => "Jerez De La Frontere, Spain ",
        "JER" => "Jersey, United Kingdom ",
        "JSR" => "Jessore, Bangladesh ",
        "JMU" => "Jiamusi, China ",
        "JGN" => "Jiayuguan, China ",
        "GJL" => "Jijel, Algeria ",
        "JIJ" => "Jijiga, Ethiopia ",
        "JIM" => "Jimma, Ethiopia ",
        "TNA" => "Jinan, China ",
        "JDZ" => "Jingdezhen, China ",
        "JHG" => "Jinghong, China ",
        "JIN" => "Jinja, Uganda ",
        "JJN" => "Jinjiang, China ",
        "HIN" => "Jinju, South Korea - Sancheon ",
        "BCO" => "Jinka, Ethiopia ",
        "JNZ" => "Jinzhou, China ",
        "JPR" => "Ji-Parana, Brazil ",
        "JIW" => "Jiwani, Pakistan ",
        "JPA" => "Joao Pessoa, Brazil ",
        "JDH" => "Jodhpur, India ",
        "JOE" => "Joensuu, Finland ",
        "JNB" => "Johannesburg, South Africa ",
        "JON" => "Johnston Island, US Minor Outlying Islands ",
        "JST" => "Johnstown, PA ",
        "JHB" => "Johor, Malaysia ",
        "JOI" => "Joinville, Brazil ",
        "XJL" => "Joliette, QC - Rail service ",
        "IXJ" => "Jommu, India ",
        "JMO" => "Jomsom, Nepal ",
        "JBR" => "Jonesboro, AR ",
        "JKG" => "Jonkoping, Sweden ",
        "XJQ" => "Jonquiere, QC - Rail service ",
        "JLN" => "Joplin, MO ",
        "JRH" => "Jorhat, India ",
        "JSM" => "Jose De San Martin, Argentina ",
        "AJF" => "Jouf, Saudi Arabia ",
        "JDO" => "Juazeiro Do Norte, Brazil ",
        "JUI" => "Juist, Germany ",
        "JDF" => "Juiz De Fora, Brazil ",
        "JUJ" => "Jujuy, Argentina ",
        "JCK" => "Julia Creek, Australia ",
        "JUL" => "Juliaca, Peru ",
        "JNU" => "Juneau, AK ",
        "JUZ" => "Juzha, China ",
        "JYV" => "Jyvaskyla, Finland ",
        "KDM" => "Kaadedhdhoo, Maldives ",
        "KBT" => "Kaben, Marshall Islands ",
        "ABK" => "Kabri Dar, Ethiopia ",
        "KBL" => "Kabul, Afghanistan ",
        "KBM" => "Kabwun, Papua New Guinea ",
        "KCF" => "Kadanwari, Pakistan ",
        "KDO" => "Kadhonoo, Maldives ",
        "KCM" => "Kahramanmaras, Turkey ",
        "OGG" => "Maui, HI ",
        "KZF" => "Kaintiba, Papua New Guinea ",
        "KAT" => "Kaitaia, New Zealand ",
        "KAJ" => "Kajaani, Finland ",
        "KAE" => "Kake, AK ",
        "KNK" => "Kakhonak, AK ",
        "AZO" => "Kalamazoo, MI ",
        "LUP" => "Kalaupapa, HI - Island of Molokai, ",
        "KAX" => "Kalbarri, Australia ",
        "KGD" => "Kaliningrad, Russia ",
        "KLG" => "Kalskag, AK ",
        "KAL" => "Kaltag, AK ",
        "KBX" => "Kambuaya, Indonesia ",
        "KAC" => "Kameshli, Syrian Arab Republic ",
        "YKA" => "Kamloops, BC ",
        "MUE" => "Kamuela, HI - Island of Hawaii, ",
        "KCD" => "Kamur, Indonesia ",
        "KUY" => "Kamusi, Papua New Guinea ",
        "XGR" => "Kangiqsualujjuaq, QC ",
        "YWB" => "Kangiqsujuaq, QC ",
        "YKG" => "Kangirsuk, QC ",
        "KAN" => "Kano, Nigeria ",
        "MCI" => "Kansas City, MO ",
        "JHM" => "Kapalua, HI - Island of Maui, ",
        "YYU" => "Kapuskasing, ON ",
        "KHI" => "Karachi, Pakistan - Quaid-E-Azam International ",
        "KDL" => "Kardia, Estonia ",
        "KAB" => "Kariba, Zimbabwe ",
        "FKB" => "Karlsruhe/Badern Baden, Germany ",
        "AOK" => "Karpathos, Greece ",
        "KBF" => "Karubaga, Indonesia ",
        "KXA" => "Kasaan, AK ",
        "ZKB" => "Kasaba Bay, Zambia ",
        "XKS" => "Kasabonika, ON ",
        "KAA" => "Kasama, Zambia ",
        "BBK" => "Kasane, Botswana ",
        "ZKE" => "Kaschechewan, ON ",
        "KUK" => "Kasigluk, AK ",
        "KTR" => "Katherine, NT, Australia ",
        "KTM" => "Kathmandu, Nepal ",
        "KTW" => "Katowice, Poland ",
        "LIH" => "Lihue, HI - Island of Kaui ",
        "KUN" => "Kaunas, Lithuania ",
        "KVA" => "Kavala, Greece ",
        "KVG" => "Kavieng, Papua New Guinea ",
        "KAW" => "Kawthaung, Myanmar ",
        "ASR" => "Kayseri, Turkey ",
        "KZN" => "Kazan,, Russia ",
        "EAR" => "Kearney, NE ",
        "EEN" => "Keene, NH ",
        "KEW" => "Keewaywin, ON ",
        "EFL" => "Kefallinia, Greece ",
        "ZKG" => "Kegaska, QC ",
        "YLW" => "Kelowna, BC ",
        "ENA" => "Kenai, AK ",
        "KDI" => "Kendari, Indonesia ",
        "YQK" => "Kenora, ON ",
        "CFU" => "Kerkyra, Greece ",
        "KTN" => "Ketchikan, AK ",
        "EYW" => "Key West, FL ",
        "QKS" => "Keystone, CO - Van service ",
        "HJR" => "Khajuraho, India ",
        "UVL" => "Kharga, Egypt ",
        "HRK" => "Kharkov, Ukraine ",
        "LBD" => "Khudzhand, Tajikistan ",
        "KDD" => "Khuzdar, Pakistan ",
        "IAN" => "Kiana, AK ",
        "KBP" => "Kiev, Ukraine - Borispol ",
        "IEV" => "Kiev, Ukraine - Zhulhany ",
        "KGL" => "Kigali, Rwanda ",
        "TKQ" => "Kigoma, Tanzania ",
        "JRO" => "Kilimanjaro, Tanzania ",
        "ILE" => "Killeen, TX ",
        "YLC" => "Kimmirut/Lake Harbour, Canada ",
        "KVC" => "King Cove, AK ",
        "AKN" => "King Salmon, AK ",
        "KIF" => "Kingfisher Lake, ON ",
        "IGM" => "Kingman, AZ ",
        "KIN" => "Kingston, Jamaica - Norman Manley ",
        "KTP" => "Kingston, Jamaica - Tinson ",
        "YGK" => "Kingston, ON - Norman Rogers Airport ",
        "XEG" => "Kingston, ON - Rail service ",
        "FIH" => "Kinshasa, Congo ",
        "KPN" => "Kipnuk, AK ",
        "IRA" => "Kirakira, Solomon Islands ",
        "IRK" => "Kirksville, MO ",
        "KTD" => "Kitadaito, Japan ",
        "YKF" => "Kitchener, ON ",
        "KTT" => "Kittila, Finland ",
        "UNG" => "Kiunga, Papua New Guinea ",
        "KVL" => "Kivalina, AK ",
        "KWY" => "Kiwayu, Kenya ",
        "LMT" => "Klamath Falls, OR ",
        "KLW" => "Klawock, AK ",
        "YKT" => "Klemtu, BC ",
        "NOC" => "Knock, Ireland ",
        "TYS" => "Knoxville, TN ",
        "OBU" => "Kobuk, AK ",
        "KCZ" => "Kochi, Japan ",
        "ADQ" => "Kodiak, AK ",
        "USM" => "Koh Samui, Thailand ",
        "QJY" => "Kolobrzeg, Poland ",
        "KXK" => "Komsomolsk Na Amure, Russia ",
        "KOA" => "Kona, HI - Island of Hawaii ",
        "KKH" => "Kongiganak, AK ",
        "KYA" => "Konya, Turkey ",
        "ROR" => "Koror, Palau ",
        "OSZ" => "Koszalin, Poland ",
        "KBR" => "Kota Bharu, Malaysia ",
        "BKI" => "Kota Kinabalu, Malaysia ",
        "KOT" => "Kotlik, AK ",
        "OTZ" => "Kotzebue, AK ",
        "KWM" => "Kowanyama, QL, Australia ",
        "KYU" => "Koyukuk, AK ",
        "CCJ" => "Kozhikode, India ",
        "KBV" => "Krabi, Thailand ",
        "KRK" => "Krakow, Poland ",
        "KWG" => "Krivoy Rog, Ukraine ",
        "KUL" => "Kuala Lumpur, Malaysia ",
        "TGG" => "Kuala Terengganu, Malaysia ",
        "KUA" => "Kuantan, Malaysia ",
        "KUG" => "Kubin Island, QL, Australia ",
        "KCH" => "Kuching, Malaysia ",
        "KUD" => "Kudat, Malaysia ",
        "AKF" => "Kufrah, Libya ",
        "YBB" => "Kugaaruk, NU ",
        "YCO" => "Kugluktuk/Coppermine, NU ",
        "KUS" => "Kulusuk, Greenland ",
        "UEO" => "Kumejima, Japan ",
        "CMU" => "Kundiawa, Papau New Guinea ",
        "KUO" => "Kuopio, Finland ",
        "KUQ" => "Kuri, Papua New Guinea ",
        "KUH" => "Kushiro, Japan ",
        "KUT" => "Kutaisi, Georgia ",
        "YVP" => "Kuujjuaq, QC ",
        "YGW" => "Kuujjuarapik, QC ",
        "KAO" => "Kuusamo, Finland ",
        "KWI" => "Kuwait, Kuwait ",
        "KWA" => "Kwajalein, Marshall Islands ",
        "KWT" => "Kwethluk, AK ",
        "KWK" => "Kwigillingok, AK ",
        "KYZ" => "Kyzyl, Russia ",
        "LCE" => "La Ceiba, Honduras ",
        "LCG" => "La Coruna, Spain ",
        "LSE" => "La Crosse, WI ",
        "YGL" => "La Grande, QC ",
        "PLP" => "La Palma, Panama ",
        "LPB" => "La Paz, Bolivia ",
        "LAP" => "La Paz, Mexico ",
        "IRJ" => "La Rioja, Argentina ",
        "LRM" => "La Romana, Dominican Republic ",
        "YVC" => "La Ronge, SK ",
        "LSC" => "La Serena, Chile ",
        "ZLT" => "La Tabatiere, QC ",
        "YLQ" => "La Tuque, QC ",
        "EUN" => "Laayoune, Morocco ",
        "LBS" => "Labasa, Fiji ",
        "LAB" => "Lablab, Papua New Guinea ",
        "LBJ" => "Labuan Bajo, Indonesia ",
        "LBU" => "Labuan, Malaysia ",
        "XLB" => "Lac Brochet, MB ",
        "XEE" => "Lac Edouard, QC - Rail service ",
        "XEH" => "Ladysmith, BC - Rail service ",
        "LML" => "Lae Island, Marshall Islands ",
        "LAE" => "Lae, Papua New Guinea ",
        "LAF" => "Lafayette, IN ",
        "LFT" => "Lafayette, LA ",
        "LAJ" => "Lages, SC, Brazil ",
        "LGQ" => "Lago Agrio, Ecuador ",
        "ING" => "Lago Argentina, Argentina ",
        "LOS" => "Lagos, Nigeria ",
        "LDU" => "Lahadbatu, Malaysia ",
        "LHE" => "Lahore, Pakistan ",
        "LCH" => "Lake Charles, LA ",
        "HII" => "Lake Havasu City, AZ ",
        "LMA" => "Lake Minchumina, AK ",
        "LKB" => "Lakeba, Fiji ",
        "LKL" => "Lakselv, Norway ",
        "LLI" => "Lalibela, Ethiopia ",
        "LPM" => "Lamap, Vanuatu ",
        "LNB" => "Lamen Bay, Vanuatu ",
        "SUF" => "Lamezia-Terme, Italy ",
        "LPI" => "Linkoping, Sweden ",
        "LMP" => "Lampedusa, Italy ",
        "LAU" => "Lamu, Kenya ",
        "LNY" => "Lanai City, HI - Island of Lanai ",
        "LNS" => "Lancaster, PA ",
        "LEQ" => "Lands End, United Kingdom ",
        "XEJ" => "Langford, BC - Rail service ",
        "LUV" => "Langguri, Indonesia ",
        "LGK" => "Langkawi, Malaysia ",
        "LAI" => "Lannion, France ",
        "YLH" => "Lansdowne House, ON ",
        "LAN" => "Lansing, MI ",
        "ACE" => "Lanzarote, Spain ",
        "LHW" => "Lanzhau, Guinea ",
        "ZGC" => "Lanzhou, China ",
        "LAO" => "Laoag, Philippines ",
        "LPP" => "Lappeenranta, Finland ",
        "LAR" => "Laramie, WY ",
        "LKA" => "Larantuka, Indonesia ",
        "LRD" => "Laredo, TX ",
        "LCA" => "Larnaca, Cyprus ",
        "LRH" => "Larochelle, France ",
        "LPA" => "Las Palmas, Spain ",
        "LSP" => "Las Piedras, Venezuela ",
        "LAS" => "Las Vegas, NV ",
        "LTK" => "Latakia, Syria ",
        "LBE" => "Latrobe, PA ",
        "LUC" => "Laucala Island, Fiji ",
        "LST" => "Launceston, TS, Australia ",
        "PIB" => "Laurel, MS ",
        "LVO" => "Laverton, WA, Australia ",
        "LWY" => "Lawas, Malaysia ",
        "LAW" => "Lawton, OK ",
        "LEH" => "Le Havre, France ",
        "ZLN" => "Le Mans, France ",
        "LPY" => "Le Puy, France ",
        "LTQ" => "Le Touquet, France ",
        "YLR" => "Leaf Rapids, MB ",
        "LEA" => "Learmonth, WA, Australia ",
        "LBA" => "Leeds, United Kingdom ",
        "LGP" => "Legaspi, Philippines ",
        "IXL" => "Leh, India ",
        "LER" => "Leinster, WA, Australia ",
        "LEJ" => "Leipzig, Germany ",
        "LKN" => "Leknes, Norway ",
        "LXS" => "Lemnos, Greece ",
        "LNO" => "Leonora, WA, Australia ",
        "YQL" => "Lethbridge, AB ",
        "LET" => "Leticia, Colombia ",
        "KLL" => "Levelock, AK ",
        "LWB" => "Lewisburg, WV ",
        "LWS" => "Lewiston, ID ",
        "LWT" => "Lewistown, MT ",
        "LEX" => "Lexington, KY ",
        "LXA" => "Lhasa, China ",
        "LYG" => "Lianyungang, China ",
        "LBL" => "Liberal, KS ",
        "LIR" => "Liberia, Costa Rica ",
        "LBV" => "Libreville, Gabon ",
        "VXC" => "Lichinga, Mozambique ",
        "LGG" => "Liege, Belgium ",
        "LIF" => "Lifa, New Caledonia ",
        "LHG" => "Lightning Ridge, NS, Australia ",
        "LNV" => "Lihir Island, Papua New Guinea ",
        "LJG" => "Lijiang City, China ",
        "LIK" => "Likiep Island, Marshall Islands ",
        "LIL" => "Lille, France - Lesquin ",
        "XDB" => "Lille, France - Rail service ",
        "LLW" => "Lilongwe, Malawi ",
        "LIM" => "Lima, Peru ",
        "LMN" => "Limbang, Malaysia ",
        "LIG" => "Limoges, France ",
        "LNK" => "Lincoln, NE ",
        "LDC" => "Lindeman Island, QL, Australia ",
        "LDI" => "Lindi, Tanzania ",
        "LYI" => "Linyi, China ",
        "LNZ" => "Linz, Austria ",
        "LIS" => "Lisbon, Portugal ",
        "LSY" => "Lismore, NS, Australia ",
        "LIT" => "Little Rock, AR ",
        "LZH" => "Liuzhou, China ",
        "LPL" => "Liverpool, United Kingdom ",
        "LVI" => "Livingstone, Zambia ",
        "LZR" => "Lizard Island, QL, Australia ",
        "LJU" => "Ljubliana, Slovenia ",
        "YLL" => "Lloydminister, AB ",
        "IRG" => "Lockhart River, Australia ",
        "LOF" => "Loen, Marshall Islands ",
        "LOH" => "Loja, Ecuador ",
        "LFW" => "Lome, Togo ",
        "YXU" => "London, ON - Municipal Airport ",
        "XDQ" => "London, ON - Rail service ",
        "LON" => "London, United Kingdom - All airports ",
        "BQH" => "London, United Kingdom - Biggin Hill ",
        "LGW" => "London, United Kingdom - Gatwick ",
        "LHR" => "London, United Kingdom - Heathrow ",
        "LCY" => "London, United Kingdom - London City ",
        "LTN" => "London, United Kingdom - Luton ",
        "STN" => "London, United Kingdom - Stansted ",
        "LDY" => "Londonderry, United Kingdom ",
        "QQP" => "London-Paddington, United Kingdom - Rail service ",
        "LDB" => "Londrina, PR, Brazil ",
        "LPU" => "Long Apung, Indonesia ",
        "LBP" => "Long Banga, Malaysia ",
        "LBW" => "Long Bawan, Indonesia ",
        "LGB" => "Long Beach, CA ",
        "HAP" => "Long Island, Australia ",
        "LGI" => "Long Island/Deadmans Cay, Bahamas ",
        "LGL" => "Long Lellang, Malaysia ",
        "ODN" => "Long Seridan, Malaysia ",
        "LOD" => "Longana, Vanuatu ",
        "LRE" => "Longreach, QL, Australia ",
        "LYR" => "Longyearbyen, Svalbard & Jan Mayen Island ",
        "LNE" => "Lonorore, Vanuatu ",
        "LPS" => "Lopez Island, WA ",
        "LDH" => "Lord Howe Island, NS, Australia ",
        "LTO" => "Loreto, Mexico ",
        "LRT" => "Lorient, France ",
        "LAX" => "Los Angeles, CA ",
        "LMM" => "Los Mochis, Mexico ",
        "LSA" => "Losuia, Papua New Guinea ",
        "SDF" => "Louisville, KY ",
        "LDE" => "Lourdes/Tarbes, France ",
        "LZC" => "Lozaro Cardenas, Mexico ",
        "LAD" => "Luanda, Angola ",
        "LXG" => "Luang Namtha, Laos ",
        "LPQ" => "Luang Prabang, Laos ",
        "LBB" => "Lubbock, TX ",
        "LKO" => "Lucknow, India ",
        "LUD" => "Luderitz, Namibia ",
        "LUG" => "Lugano, Switzerland ",
        "VSG" => "Lugansk, Uganda ",
        "LUA" => "Lukla, Nepal ",
        "LLA" => "Lulea, Sweden ",
        "FBM" => "Lumbashi, Congo ",
        "LYA" => "Luoyang, China ",
        "LUN" => "Lusaka, Zambia ",
        "YSG" => "Lutselke/Snowdrift, NT ",
        "LUW" => "Luwuk, Indonesia ",
        "LUX" => "Luxembourg, Luxembourg ",
        "LUM" => "Luxi, China ",
        "LXR" => "Luxor, Egypt ",
        "LZO" => "Luzhou, China ",
        "LWO" => "Lvov, Ukraine ",
        "LYC" => "Lyoksele, Sweden ",
        "XYD" => "Lyon, France - Lyon Part-Dieu Rail service ",
        "LYS" => "Lyon, France - Satolas ",
        "MST" => "Maastricht, Netherlands ",
        "UBB" => "Mabuiag Island, QL, Australia ",
        "MCP" => "Macapa, AP, Brazil ",
        "XMS" => "Macas, Ecuador ",
        "MFM" => "Macau, China ",
        "MCZ" => "Maceio, AL, Brazil ",
        "MCH" => "Machala, Ecuador ",
        "MKY" => "Mackay, QL, Australia ",
        "MCN" => "Macon, GA ",
        "MAG" => "Madang, Papua New Guinea ",
        "MED" => "Madinah, Saudi Arabia ",
        "MSN" => "Madison, WI ",
        "MDJ" => "Madras, OR ",
        "MAD" => "Madrid, Spain ",
        "IXM" => "Madurai, India ",
        "HGN" => "Mae Hong Son, Thailand ",
        "MAQ" => "Mae Sot, Thailand ",
        "MWF" => "Maewo, Vanuatu ",
        "MFA" => "Mafia, Tanzania ",
        "GDX" => "Magadan, Russia ",
        "MQF" => "Magnitogorsk, Russia ",
        "VVB" => "Mahanoro, Madagascar ",
        "SEZ" => "Mahe Island, Seychelles ",
        "MXT" => "Maintirano, Madagascar ",
        "MMO" => "Maio, Cape Verde ",
        "MJE" => "Majkin, Marshall Islands ",
        "MJN" => "Majunga, Madagascar ",
        "MAJ" => "Majuro, Marshall Islands ",
        "MQX" => "Makale, Ethiopia ",
        "MCX" => "Makhachkala, Russia ",
        "MKU" => "Makokou, Gabon ",
        "MZG" => "Makung, Taiwan ",
        "SSG" => "Malabo, Equatorial Guinea ",
        "MKZ" => "Malacca, Malaysia ",
        "AGP" => "Malaga, Spain ",
        "MAK" => "Malakai, Sudan ",
        "LGS" => "Malargue, MD, Argentina ",
        "MLX" => "Malatya, Turkey ",
        "MLE" => "Male, Maldives ",
        "MYD" => "Malindi, Kenya ",
        "MMX" => "Malmo, Sweden ",
        "MAV" => "Maloelap Island, Marshall Islands ",
        "PTF" => "Malololailai, Fiji ",
        "MLA" => "Malta, Malta ",
        "WMP" => "Mampikony, Madagascar ",
        "MNF" => "Mana Island, Fiji ",
        "MDC" => "Manado, Indonesia ",
        "MGA" => "Managua, Nicaragua ",
        "WVK" => "Manakara, Madagascar ",
        "WMR" => "Mananara, Madagascar ",
        "NGX" => "Manang, Nepal ",
        "MNJ" => "Mananjary, Madagascar ",
        "MAO" => "Manaus, AM, Brazil ",
        "MHT" => "Manchester, NH ",
        "MAN" => "Manchester, United Kingdom ",
        "MDL" => "Mandalay, Myanmar ",
        "WMA" => "Mandritsara, Madagascar ",
        "MGS" => "Mangaia Island, Cook Islands ",
        "IXE" => "Mangalore, India ",
        "MAY" => "Mangrove Cay, Bahamas ",
        "MNR" => "Mangu, Zambia ",
        "MFO" => "Manguna, Papua New Guinea ",
        "MHK" => "Manhattan, KS ",
        "XMH" => "Manihi, French Polynesia ",
        "MHX" => "Manihiki Island, Cook Islands ",
        "MNL" => "Manila, Philippines ",
        "MNG" => "Maningrioa, NT, Australia ",
        "MBL" => "Manistee, MI ",
        "MZL" => "Manizales, Colombia ",
        "MJA" => "Manja, Madagascar ",
        "MKT" => "Mankato, MN ",
        "MLY" => "Manley Hot Springs, AK ",
        "MHG" => "Mannheim, Germany ",
        "KMO" => "Manokotak, AK ",
        "MKW" => "Manokwari, Indonesia ",
        "MSE" => "Manston, United Kingdom ",
        "MEC" => "Manta, Ecuador ",
        "MAS" => "Manus Island, Papua New Guinea ",
        "MZO" => "Manzanillo, Cuba ",
        "ZLO" => "Manzanillo, Mexico ",
        "MTS" => "Manzini, Swaziland ",
        "AMO" => "Mao, Chad ",
        "MXS" => "Maota Savaii Is, Western Samoa ",
        "MPM" => "Maputo, Mozambique ",
        "MDQ" => "Mar Del Plata, BA, Argentina ",
        "MRE" => "Mara Lodges, Kenya ",
        "MAB" => "Maraba, PA, Brazil ",
        "MAR" => "Maracaibo, Venezuela ",
        "MYC" => "Maracay, Venezuela ",
        "MEE" => "Mare, New Caledonia ",
        "MGH" => "Margate, South Africa ",
        "MBX" => "Maribor, Slovenia ",
        "MHQ" => "Mariehamn, Finland ",
        "PKB" => "Parkersburg, WV/Marietta, OH ",
        "JSU" => "Mariitsoq, Greenland ",
        "MII" => "Marilia, SP, Brazil ",
        "MGF" => "Maringa, PR, Brazil ",
        "MWA" => "Marion, IL ",
        "MPW" => "Mariupol, Ukraine ",
        "WMN" => "Maroantsetra, Madagascar ",
        "MVR" => "Marova, Cameroon ",
        "MQT" => "Marquette, MI ",
        "MRS" => "Marseille, France ",
        "MHH" => "Marsh Harbour, Bahamas ",
        "MLL" => "Marshall, AK ",
        "MVY" => "Martha's Vineyard, MA ",
        "MUR" => "Marudi, Malaysia ",
        "MBH" => "Maryborough, QL, Australia ",
        "YMH" => "Mary's Harbour, NL ",
        "MBT" => "Masbate, Philippines ",
        "MSU" => "Maseru, Lesotho ",
        "MHD" => "Mashad, Iran ",
        "MCW" => "Mason City, IA ",
        "MSS" => "Massena, NY ",
        "MAM" => "Matamoros, Mexico ",
        "AMI" => "Mataram, Indonesia ",
        "MMJ" => "Matsumoto, Japan ",
        "MYJ" => "Matsuyama, Japan ",
        "MUN" => "Maturin, Venezuela ",
        "MUK" => "Mauke Island, Cook Islands ",
        "MNU" => "Maulmyine, Myanmar ",
        "MOF" => "Maumere, Indonesia ",
        "MUB" => "Maun, Botswana ",
        "MAU" => "Maupiti, French Polynesia ",
        "MRU" => "Mauritius, Mauritius ",
        "XID" => "Maxville, ON - Rail service ",
        "MYG" => "Mayaguana, Bahamas ",
        "MAZ" => "Mayaguez, Puerto Rico ",
        "MZT" => "Mazatlan, Mexico ",
        "MDK" => "Mbandaka, Congo ",
        "MJM" => "Mbuji Mayi, Congo ",
        "MFE" => "Mission, TX ",
        "MCV" => "Mcarthur River, NT, Australia ",
        "MCK" => "Mccook, NE ",
        "MCG" => "Mcgrath, AK ",
        "MES" => "Medan, Indonesia ",
        "EOH" => "Medellin, Colombia - Enrique Olaya Herrera ",
        "MDE" => "Medellin, Colombia - Jose Marie Cordova ",
        "MFR" => "Medford, OR ",
        "YXH" => "Medicine Hat, AB ",
        "MKR" => "Meekatharra, WA, Australia ",
        "MEH" => "Mehamn, Norway ",
        "MXZ" => "Meixian, China ",
        "MJB" => "Mejit Island, Marshall Islands ",
        "MKS" => "Mekane Selam, Ethiopia ",
        "MYU" => "Mekoryuk, AK ",
        "MLB" => "Melbourne, FL ",
        "MEL" => "Melbourne, Victoria, Australia ",
        "MLN" => "Melilla, Spain ",
        "XEK" => "Melville, SK - Rail service ",
        "MMB" => "Memanbetsu, Japan ",
        "MEM" => "Memphis, TN ",
        "NDM" => "Mendi, Ethiopia ",
        "MDU" => "Mendi, Papua New Guinea ",
        "MDZ" => "Mendoza, MD, Argentina ",
        "MAH" => "Menorca, Spain ",
        "MYX" => "Menyamya, Papua New Guinea ",
        "MKQ" => "Merauke, Indonesia ",
        "MWE" => "Merave, Sudan ",
        "MCE" => "Merced, CA ",
        "RDE" => "Merdey, Indonesia ",
        "MID" => "Merida, Mexico ",
        "MRD" => "Merida, Venezuela ",
        "MEI" => "Meridian, MS ",
        "MIM" => "Merimbula, NS, Australia ",
        "MUH" => "Mersa Matruh, Egypt ",
        "MTM" => "Metlakatla, AK ",
        "ETZ" => "Metz/Nancy, France ",
        "MXL" => "Mexicali, Mexico ",
        "MEX" => "Mexico City, Mexico ",
        "WMK" => "Meyers Chuck, AK ",
        "MFU" => "Mfume, Zambia ",
        "MIA" => "Miami, FL - International ",
        "MPB" => "Miami, FL - Sea Plane Base ",
        "ZVA" => "Miandrivazo, Madagascar ",
        "MDS" => "Middle Caicos, Turks and Caicos ",
        "MAF" => "Odessa/Midland, TX ",
        "MDY" => "Midway Island, US Minor Outlying Islands ",
        "MIK" => "Mikkeli, Finland ",
        "JMK" => "Mikonos, Greece ",
        "LIN" => "Milan, Italy - Linate ",
        "MXP" => "Milan, Italy - Malpensa ",
        "BGY" => "Milan, Italy - Orio Al Serio ",
        "PMF" => "Parma/Milan, Italy ",
        "MQL" => "Mildura, VI, Australia ",
        "MLS" => "Miles City, MT ",
        "MIJ" => "Mili Island, Marshall Islands ",
        "MGT" => "Milingimbi, NT, Australia ",
        "MKE" => "Milwaukee, WI ",
        "MMD" => "Minami Daito, Japan ",
        "MTT" => "Minatitla, Mexico ",
        "MDP" => "Mindiptana, Indonesia ",
        "MRV" => "Mineralnye Vody, Russia ",
        "MSP" => "Minneapolis, MN ",
        "MOT" => "Minot, ND ",
        "MHP" => "Minsk, Belarus - Minsk International 1 ",
        "MSQ" => "Minsk, Belarus - Minsk International 2 ",
        "MNT" => "Minto, AK ",
        "XEY" => "Miramichi, NB - Rail service ",
        "MYY" => "Miri, Malaysia ",
        "MJZ" => "Mirnyj, Russia ",
        "MSJ" => "Misawa, Japan ",
        "MIS" => "Misima Island, Papua New Guinea ",
        "MSO" => "Missoula, MT ",
        "MRA" => "Misurata, Libya ",
        "MOI" => "Mitiaro Island, Cook Islands ",
        "MYE" => "MiyakeJima, Japan ",
        "MMY" => "Miyako Jima, Japan ",
        "MTF" => "Mizan Teferi, Ethiopia ",
        "MQN" => "Mo I Rana, Norway ",
        "MOA" => "Moa, Cuba ",
        "CNY" => "Moab, UT ",
        "MFJ" => "Moala, Fiji ",
        "ONI" => "Moanamani, Indonesia ",
        "MNB" => "Moanda, Congo ",
        "MFF" => "Moanda, Gabon ",
        "MOB" => "Mobile, AL ",
        "MOD" => "Modesto, CA ",
        "MGQ" => "Mogadishu, Somalia ",
        "MJD" => "Mohenjodaro, Denmark ",
        "MPK" => "Mokpo, South Korea ",
        "OKU" => "Mokuti Lodge, Namibia ",
        "MOL" => "Molde, Norway ",
        "MLI" => "Moline, IL ",
        "MBA" => "Mombasa, Kenya ",
        "MIR" => "Monastir, Tunisia ",
        "MBE" => "Monbetsu, Japan ",
        "LOV" => "Monclova, Mexico ",
        "YQM" => "Moncton, NB - Airport ",
        "XDP" => "Moncton, NB - Rail service ",
        "MJK" => "Monkey Mia, WA, Australia ",
        "MNY" => "Mono, Solomon Islands ",
        "MLU" => "Monroe, LA ",
        "ROB" => "Monrovia, Liberia ",
        "YYY" => "Mont Joli, QC ",
        "MCM" => "Monte Carlo, Monaco ",
        "MEU" => "Monte Dourado, PA, Brazil ",
        "MBJ" => "Montego Bay, Jamaica ",
        "MTR" => "Monteria, Colombia ",
        "MTY" => "Monterrey, Mexico ",
        "MOC" => "Montes Claros, MG, Brazil ",
        "MVD" => "Montevideo, Uruguay ",
        "MGM" => "Montgomery, AL ",
        "MPL" => "Montpellier, France ",
        "YMQ" => "Montreal, QC - all airports ",
        "YUL" => "Montreal, QC - Dorval ",
        "XAX" => "Montreal, QC - Dorval Rail service ",
        "YMY" => "Montreal, QC - Downtown Rail service ",
        "YMX" => "Montreal, QC - Mirabel ",
        "XLM" => "Montreal, QC - St Lambert Rail service ",
        "MTJ" => "Montrose, CO ",
        "MNI" => "Montserrat, Montserrat ",
        "MOZ" => "Moorea, French Polynesia ",
        "YMO" => "Moosonee, ON ",
        "MZI" => "Mopti, Mali ",
        "MXX" => "Mora, Sweden ",
        "TVA" => "Morafenobe, Madagascar ",
        "MXM" => "Morambe, Madagascar ",
        "MOV" => "Moranbah, QL, Australia ",
        "MRZ" => "Moree, NS, Australia ",
        "MLM" => "Morelia, Mexico ",
        "MGW" => "Morgantown, WV ",
        "HNA" => "Morioka, Japan ",
        "ONG" => "Mornington, QL, Australia ",
        "MXH" => "Moro, Papua New Guinea ",
        "MOQ" => "Morondava, Madagascar ",
        "HAH" => "Moroni, Comoros ",
        "MYA" => "Moruya, NS, Australia ",
        "MOW" => "Moscow, Russia - all locations ",
        "BKA" => "Moscow, Russia - Bykovo ",
        "DME" => "Moscow, Russia - Domodedovo ",
        "SVO" => "Moscow, Russia - Sheremetyevo ",
        "VKO" => "Moscow, Russia - Vnukovo ",
        "MWH" => "Moses Lake, WA ",
        "MJF" => "Mosjoen, Norway ",
        "OMO" => "Mostar, Bosnia and Herzegovina ",
        "MTV" => "Mota Lava, Vanuatu ",
        "MJL" => "Mouila, Gabon ",
        "MON" => "Mount Cook, New Zealand ",
        "MGB" => "Mount Gambier, SA, Australia ",
        "HGU" => "Mount Hagen, Papua New Guinea ",
        "MHU" => "Mount Hotham, VI, Australia ",
        "ISA" => "Mount Isa, Australia ",
        "WME" => "Mount Keith, WA, Australia ",
        "MMG" => "Mount Magnet, WA, Australia ",
        "MPN" => "Mount Pleasant, Falkland Islands ",
        "WMH" => "Mountain Home, AR ",
        "MOU" => "Mountain Village, AK ",
        "MPA" => "Mpacha, Namibia ",
        "MYW" => "Mtwara, Tanzania ",
        "MVS" => "Mucuri, BA, Brazil ",
        "MDG" => "Mudanjiang, China ",
        "DGE" => "Mudgee, Australia ",
        "FMO" => "Muenster, Germany ",
        "MKM" => "Mukan, Malaysia ",
        "MLH" => "Mulhouse, France ",
        "LII" => "Mulia, Indonesia ",
        "MUX" => "Multan, Pakistan ",
        "MZV" => "Mulu, Malaysia ",
        "MUA" => "Munda, Solomon Islands ",
        "MUC" => "Munich, Germany ",
        "MJV" => "Murcia, Spain ",
        "MMK" => "Murmansk, Russia ",
        "MYI" => "Murray Island, QL, Australia ",
        "MSR" => "Mus, Turkey ",
        "MCT" => "Muscat, Oman ",
        "MKG" => "Muskegon, MI ",
        "MSA" => "Muskrat Dam, ON ",
        "MUZ" => "Musoma, Tanzania ",
        "MFG" => "Muzaffarabad, Pakistan ",
        "MWZ" => "Mwanza, Tanzania ",
        "MGZ" => "Myeik, Myanmar ",
        "MYT" => "Myitkyina, Myanmar ",
        "MYR" => "Myrtle Beach, SC ",
        "MJT" => "Mytilene, Greece ",
        "ZZU" => "Mzuzu, Malawi ",
        "NBX" => "Nabire, Indonesia ",
        "MNC" => "Nacala, Mozambique ",
        "NDR" => "Nadar, Morocco ",
        "NAN" => "Nadi, Fiji ",
        "NYM" => "Nadym, Russia ",
        "WNP" => "Naga, Philippines ",
        "NGS" => "Nagasaki, Japan ",
        "NGO" => "Nagoya, Japan - Nagoya Komaki APT ",
        "NAG" => "Nagpur, India ",
        "YDP" => "Nain, NL ",
        "NBO" => "Nairobi, Kenya - Jomo Kenyatta Intl ",
        "WIL" => "Nairobi, Kenya - Wilson ",
        "NAJ" => "Nakhichevan, Azerbaijan ",
        "NAK" => "Nakhon Ratchosima, Thailand ",
        "NST" => "Nakhon Si Thammarat, Thailand ",
        "YQN" => "Nakina, ON ",
        "NAL" => "Nalchik, Russia ",
        "NMA" => "Namangan, Uzbekistan ",
        "ATN" => "Namatana, Papua New Guinea ",
        "NDK" => "Namorik Island, Marshall Islands ",
        "APL" => "Nampula, Mozambique ",
        "OSY" => "Namsos, Norway ",
        "NDI" => "Namudi, Papua New Guinea ",
        "NNT" => "Nan, Thailand ",
        "YCD" => "Nanaimo, BC - Cassidy Airport ",
        "ZNA" => "Nanaimo, BC - Harbour Airport ",
        "NAO" => "Nanchong, China ",
        "YSR" => "Nanisivik, NU ",
        "NKG" => "Nanking/Nanjing, China ",
        "NNG" => "Nanning, China ",
        "JNN" => "Nanortalik, Greenland ",
        "NTE" => "Nantes, France - Nantes Atlantique ",
        "QJZ" => "Nantes, France - Rail service ",
        "NTG" => "Nantong, China ",
        "ACK" => "Nantucket, MA ",
        "NNY" => "Nanyang, China ",
        "NYK" => "Nanyuki, Kenya ",
        "WNA" => "Napakiak, AK ",
        "XIF" => "Napanee, ON - Rail service ",
        "PKA" => "Napaskiak, AK ",
        "NPE" => "Napier-Hastings, New Zealand ",
        "APF" => "Naples, FL ",
        "NAP" => "Naples, Italy ",
        "NAW" => "Narathiwat, Thailand ",
        "NAA" => "Narrabri, NS, Australia ",
        "JNS" => "Narsaq, Greenland ",
        "UAK" => "Narsarsuaq, Greenland ",
        "NVK" => "Narvik, Norway ",
        "NNM" => "Naryan-Mar, Russia ",
        "BNA" => "Nashville, TN ",
        "NAS" => "Nassau, Bahamas - Paradise Island ",
        "MAT" => "Natadi, Congo ",
        "NAT" => "Natal, RN, Brazil ",
        "YNA" => "Natashquan, QC ",
        "NKI" => "Naukiti, AK ",
        "INU" => "Nauru Island, Nauru ",
        "NVT" => "Navegantes, SC, Brazil ",
        "WNS" => "Nawabshah, Pakistan ",
        "NUX" => "NayUrengoy, Russia ",
        "NDJ" => "Ndjamena, Chad ",
        "NLA" => "Ndola, Zambia ",
        "CNP" => "Neerlerit Inaat, Greenland ",
        "NFG" => "Nefteyugansk, Russia ",
        "EGL" => "Neghelli, Ethiopia ",
        "NEG" => "Negril, Jamaica ",
        "NVA" => "Neiva, Colombia ",
        "EAM" => "Nejran, Saudi Arabia ",
        "NSN" => "Nelso, New Zealand ",
        "NLG" => "Nelson Lagoon, AK ",
        "NLP" => "Nelspruit, South Africa ",
        "EMN" => "Nema, Mauritania ",
        "YNS" => "Nemiscau, QC ",
        "NER" => "Neryjungri, Russia ",
        "NQN" => "Neuquen, NE, Argentina ",
        "NEV" => "Nevis, St. Kitts and Nevis ",
        "XEL" => "New Carlisle, QC - Rail service ",
        "NCN" => "New Chenega, AK ",
        "HVN" => "New Haven, CT ",
        "KGK" => "New Koliganek, AK ",
        "MSY" => "New Orleans, LA ",
        "NPL" => "New Plymouth, New Zealand ",
        "XEM" => "New Richmond, QC - Rail service ",
        "KNW" => "New Stuyahok, AK ",
        "NYC" => "New York, NY - All airports ",
        "JFK" => "New York, NY - Kennedy ",
        "LGA" => "New York, NY - La Guardia ",
        "EWR" => "Newark, NJ ",
        "SWF" => "Stewart Field/Newburgh, NY ",
        "NTL" => "Newcastle, New South Wales, Australia ",
        "BEO" => "Newcastle, New South Wales, Australia - Belmont ",
        "NCL" => "Newcastle, United Kingdom ",
        "ZNE" => "Newman, WA, Australia ",
        "NQY" => "Newquay, United Kingdom ",
        "WWT" => "Newtok, AK ",
        "NGE" => "Ngaoundere, Cameroon ",
        "NGI" => "Ngau Island, Fiji ",
        "RPM" => "Ngukurr, NT, Australia ",
        "NHA" => "Nha Trang, Viet Nam ",
        "XLV" => "Niagara Falls, ON - Rail service ",
        "NIM" => "Niamey, Niger ",
        "NCE" => "Nice, France ",
        "NIC" => "Nicosia, Cyprus ",
        "NME" => "Nightmute, AK ",
        "NIB" => "Nikolai, AK ",
        "IKO" => "Nikolski, AK ",
        "FNI" => "Nimes, France ",
        "NGB" => "Ningbo, China ",
        "NIX" => "Nioko, Congo ",
        "NFO" => "Niuafo'ou, Tonga ",
        "NTT" => "Niuatoputapu, Tonga ",
        "IUE" => "Niue Island, Niue ",
        "NJC" => "Nizhnevartovsk, Russia ",
        "GOJ" => "Nizhniy Novgorod, Russia ",
        "WTK" => "Noatak, AK ",
        "NOJ" => "Nojabrxsk, Russia ",
        "OME" => "Nome, AK ",
        "NNL" => "Nondalton, AK ",
        "ORV" => "Noorvik, AK ",
        "YUY" => "Rouyn/Noranda, QC ",
        "NRD" => "Norderney, Germany ",
        "NDZ" => "Nordholz-Speika, Germany ",
        "NLK" => "Norfolk Island, Norfolk Island ",
        "OFK" => "Norfolk, NE ",
        "ORF" => "Norfolk, VA ",
        "NSK" => "Noril'sk, Russia ",
        "YVQ" => "Norman Wells, NT ",
        "NTN" => "Normanton, QL, Australia ",
        "NRK" => "Norrkoping, Sweden ",
        "NUS" => "Norsup, Vanuatu ",
        "YYB" => "North Bay, ON ",
        "OTH" => "North Bend, OR ",
        "NCA" => "North Caicos, Turks and Caicos Islands ",
        "LBF" => "North Platte, NE ",
        "NRL" => "North Ronaldsay, United Kingdom ",
        "YNO" => "North Spirit Lake, ON ",
        "ORT" => "Northway, AK ",
        "YNE" => "Norway House, MB ",
        "NWI" => "Norwich, United Kingdom ",
        "NOB" => "Nosara Beach, Costa Rica ",
        "NOS" => "Nossi-be, Madagascar ",
        "EMA" => "Nottingham, United Kingdom ",
        "NDB" => "Nouadhiba, Mauritania ",
        "NKC" => "Nouakchott, Mauritania ",
        "GEA" => "Noumea, New Caledonia - Magenta ",
        "NOU" => "Noumea, New Caledonia - Tontouta ",
        "NVR" => "Novgorod, Russia ",
        "NOZ" => "Novokuznetsk, Russia ",
        "OVB" => "Novosibirsk, Russia - Tolmachevo ",
        "GER" => "Nueva Gerona, Cuba ",
        "NLD" => "Nuevo Laredo, Mexico ",
        "NUI" => "Nuiqsut, AK ",
        "NHV" => "Nuku Hiva, French Polynesia ",
        "TBU" => "Nuku'Alofa, Tonga ",
        "NCU" => "Nukus, Uzbekistan ",
        "NUL" => "Nulato, AK ",
        "NUB" => "Numbulwar, NT, Australia ",
        "NUP" => "Nunapitchuk, AK ",
        "NNX" => "Nunukan, Indonesia ",
        "NUE" => "Nuremberg, Germany ",
        "ZAQ" => "Nuremberg, Germany - Rail service ",
        "GOH" => "Nuuk, Greenland ",
        "UYL" => "Nyala, Sudan ",
        "NYU" => "Nyaung, Myanmar ",
        "NYN" => "Nyngan, NS, Australia ",
        "OAK" => "Oakland, CA ",
        "OAX" => "Oaxaca, Mexico ",
        "OBD" => "Obano, Indonesia ",
        "OBO" => "Obihiro, Japan ",
        "OCJ" => "Ocho Rios, Jamaica ",
        "ZBQ" => "Odense, Denmark ",
        "ODS" => "Odessa, Ukraine ",
        "OGS" => "Ogdensburg, NY ",
        "YOG" => "Ogoki, ON ",
        "OHD" => "Ohrid, Macedonia ",
        "OIT" => "Oita, Japan ",
        "OKQ" => "Okaba, Indonedia ",
        "OKJ" => "Okayama, Japan ",
        "OHO" => "Okhotsk, Russia ",
        "OKA" => "Okinawa, Japan ",
        "OKC" => "Oklahoma City, OK ",
        "OIR" => "Okoshiri, Japan ",
        "OKL" => "Oksibil, Indonesia ",
        "OLB" => "Olbia, Italy ",
        "YOC" => "Old Crow, YT ",
        "OLJ" => "Olpoi, Vanuatu ",
        "OLP" => "Olympic Dam, SA, Australia ",
        "OMA" => "Omaha, NE ",
        "OMB" => "Omboue, Gabon ",
        "OMS" => "Omsk, Russia ",
        "OND" => "Ondangwa, Namibia ",
        "ONT" => "Ontario, CA ",
        "YBS" => "Opapamiska Lake, ON ",
        "OMR" => "Oradea, Romania ",
        "ORN" => "Oran, Algeria ",
        "SNA" => "Santa Ana, CA ",
        "OAG" => "Orange, New South Wales, Australia ",
        "OMD" => "Oranjemund, Namibia ",
        "ORB" => "Orebro, Sweeden - Obrebro-Bofors ",
        "REN" => "Orenburg, Russia ",
        "ORL" => "Orlando, FL - Herndon ",
        "MCO" => "Orlando, FL - International ",
        "ORW" => "Ormara, Pakistan ",
        "OER" => "Ornskoldsvik, Sweden ",
        "OSW" => "Orsk, Russia ",
        "HOV" => "Orsta-Volda, Norway ",
        "OSA" => "Osaka, Japan - all airports ",
        "ITM" => "Osaka, Japan - Itami ",
        "KIX" => "Osaka, Japan - Kansai Intl ",
        "YOO" => "Oshawa, ON ",
        "OIM" => "Oshima, Japan ",
        "OSH" => "Oshkosh, WI ",
        "OSI" => "Osijek, Croatia ",
        "OSK" => "Oskarshamn, Sweden ",
        "OSL" => "Oslo, Norway ",
        "OSD" => "Ostersund, Sweden ",
        "OSR" => "Ostrava, Czech Republic ",
        "YOW" => "Ottawa, ON - International ",
        "XDS" => "Ottawa, ON - Rail service ",
        "OTM" => "Ottumwa, IA ",
        "OTU" => "Otu, Colombia ",
        "OUA" => "Ouagadougou, Burkina Faso ",
        "OGX" => "Ouargla, Algeria ",
        "OZZ" => "Ouarzazate, Morocco ",
        "ODY" => "Oudomxay, Laos ",
        "OUD" => "Oujda, Morocco ",
        "OUL" => "Oulu, Finland ",
        "UVE" => "Ouvea, New Caledonia ",
        "VDA" => "Ovda, Israel ",
        "OWB" => "Owensboro, KY ",
        "YOH" => "Oxford House, MB ",
        "OXR" => "Ventura/Oxnard, CA ",
        "OYE" => "Oyem, Gabon ",
        "PBJ" => "Paama, Vanuatu ",
        "JFR" => "Paamiut, Greenland ",
        "PDG" => "Padang, Indonesia ",
        "PAD" => "Paderborn, Germany ",
        "PAH" => "Paducah, KY ",
        "PGA" => "Page, AZ ",
        "PPG" => "Pago Pago, American Samoa ",
        "PKZ" => "Pakse, Laos ",
        "YIF" => "Pakuashipi, QC ",
        "PAF" => "Pakuba, Uganda ",
        "PCH" => "Palacios, Honduras ",
        "PKY" => "Palang Karaya, Indonesia ",
        "PLQ" => "Palanga, Lithuania ",
        "PLM" => "Palembang, Indonesia ",
        "PQM" => "Palenque, Mexico ",
        "PMO" => "Palermo, Sicily, Italy ",
        "PSP" => "Palm Springs, CA ",
        "PMI" => "Palma Mallorca, Spain and Canary Islands ",
        "PMZ" => "Palmar, Costa Rica ",
        "PMW" => "Palmas, TO, Brazil ",
        "PMR" => "Palmerston North, New Zealand ",
        "PLW" => "Palu, Indonesia ",
        "PNA" => "Pamplona, Spain ",
        "PFN" => "Panama City, FL ",
        "PAC" => "Panama City, Panama - Paitilla ",
        "PTY" => "Panama City, Panama - Tocumen Intl ",
        "PKN" => "Pangkalanboun, Indonesia ",
        "PGK" => "Pangkalpinang, Indonesia ",
        "YXP" => "Pangnirtung, NU ",
        "PJG" => "Panjger, Pakistan ",
        "PNL" => "Pantelleria, Italy ",
        "PPW" => "Papa Westray, United Kingdom ",
        "PPT" => "Papeete, French Polynesia ",
        "PFO" => "Paphos, Cyprus ",
        "PAJ" => "Para Chinar, Pakistan ",
        "PBO" => "Paraburdoo, WA, Australia ",
        "PID" => "Paradise Island, Bahamas ",
        "PBM" => "Paramaribo, Suriname - Zanderij Intl ",
        "ORG" => "Paramaribo, Suriname - Zorg En Hoop ",
        "PRA" => "Parana, ER, Argentina ",
        "PRS" => "Parasi, Solomon Islands ",
        "XFE" => "Parent, QC - Rail service ",
        "PAR" => "Paris, France - All airports ",
        "BVA" => "Paris, France - Beauvais-Tille ",
        "CDG" => "Paris, France - Charles Degaulle ",
        "ORY" => "Paris, France - Orly ",
        "XPB" => "Parksville, BC - Rail service ",
        "PHB" => "Parnaiba, PI, Brazil ",
        "PBH" => "Paro, Bhutan ",
        "PSC" => "Pasco, WA ",
        "PSI" => "Pasni, Pakistan ",
        "AOL" => "Paso de Los Libres, Argentina ",
        "PFB" => "Passo Fundo, RS, Brazil ",
        "PSO" => "Pasto, Colombia ",
        "PAT" => "Patna, India ",
        "GPA" => "Patras, Greece ",
        "PUF" => "Pau, France ",
        "YPC" => "Paulatuk, NT ",
        "PWQ" => "Pavlodar, Kazakhstan ",
        "YPE" => "Peace River, AB ",
        "YPO" => "Peawanuck, ON ",
        "PEX" => "Pechora, Russia ",
        "PDB" => "Pedro Bay, AK ",
        "PKU" => "Pekanbaru, Indonesia ",
        "PEC" => "Pelican, AK ",
        "PLN" => "Pellston, MI ",
        "PET" => "Pelotas, RS, Brazil ",
        "POL" => "Pemba, Mozambique ",
        "PMA" => "Pemba, Tanzania - Wawi ",
        "YTA" => "Pembroke, ON ",
        "PEN" => "Penang, Malaysia ",
        "PDT" => "Pendleton, OR ",
        "PYE" => "Penrhyn Island, Cook Islands ",
        "PNS" => "Pensacola, FL ",
        "YYF" => "Penticton, BC ",
        "PZE" => "Penzance, United Kingdom ",
        "PIA" => "Peoria, IL ",
        "XFG" => "Perce, QC - Rail service ",
        "PEI" => "Pereira, Colombia ",
        "PGX" => "Perigueux, France ",
        "PMQ" => "Perito Moreno, SC, Argentina ",
        "PEE" => "Perm, Russia ",
        "PGF" => "Perpignan, France ",
        "KPV" => "Perryville, AK ",
        "PER" => "Perth, Western Australia, Australia ",
        "PEG" => "Perugia, Italy ",
        "PSR" => "Pescara, Italy ",
        "PEW" => "Peshawar, Pakistan ",
        "PSG" => "Petersburg, AK ",
        "PNZ" => "Petrolina, PE, Brazil ",
        "PKC" => "Petropaulousk-Kamchats, Russia ",
        "PES" => "Petrozavodsk, Russia ",
        "PHW" => "Phalaborwa, South Africa ",
        "PHL" => "Philadelphia, PA - International ",
        "TTN" => "Trenton/Mercer, NJ ",
        "PHS" => "Phitsanulok, Thailand ",
        "PNH" => "Phnom Penh, Cambodia ",
        "PHX" => "Phoenix, AZ ",
        "PRH" => "Phrae, Thailand ",
        "PQC" => "Phu Quoc, Viet Nam - Duong Dang ",
        "HKT" => "Phuket, Thailand ",
        "YPL" => "Pickle Lake, ON ",
        "PIX" => "Pico Island, Portugal ",
        "PDS" => "Piedras Negras, Mexico ",
        "PIR" => "Pierre, SD ",
        "PZB" => "Pietermaritzburg, South Africa ",
        "PTG" => "Pietersburb, South Africa ",
        "YPM" => "Pikangikum, ON ",
        "PIP" => "Pilot Point, AK ",
        "UGB" => "Pilot Point, AK - Ugashnik Bay ",
        "PQS" => "Pilot Station, AK ",
        "PIF" => "Pingtung, Taiwan ",
        "PIT" => "Pittsburgh, PA ",
        "THU" => "Pituffik, Greenland ",
        "PIU" => "Piura, Peru ",
        "PLJ" => "Placencia, Belize ",
        "PTU" => "Platinum, AK ",
        "PLB" => "Plattsburgh, NY ",
        "PXU" => "Pleiku, Viet Nam ",
        "PBZ" => "Plettenburg Bay, South Africa ",
        "PLH" => "Plymouth, United Kingdom ",
        "PIH" => "Pocatello, ID ",
        "TGD" => "Podgoriea, Serbia and Montenegro - Golubovci ",
        "PNI" => "Pohnpei, Micronesia ",
        "KPB" => "Point Baker, AK ",
        "PHO" => "Point Hope, AK ",
        "PIZ" => "Point Lay, AK ",
        "PNR" => "Pointe Noire, Congo ",
        "PTP" => "Pointe-a-Pitre, Guadeloupe ",
        "XPX" => "Pointe-aux-Trembles, QC - Rail service ",
        "YNL" => "Points North Landing, SK ",
        "PIS" => "Poitiers, France - Biard ",
        "XOP" => "Poitiers, France - Rail service ",
        "PKR" => "Pokhara, Nepal ",
        "PLV" => "Poltava, Ukraine ",
        "PYJ" => "Polyarnyj, Russia ",
        "PNC" => "Ponca City, OK ",
        "PSE" => "Ponce, Puerto Rico ",
        "YIO" => "Pond Inlet, NU ",
        "PDL" => "Ponta Delgada, Portugal ",
        "PMG" => "Ponta Pora, MS, Brazil ",
        "PNK" => "Pontianak, Indonesia ",
        "YHP" => "Poplar Hill, ON ",
        "PNP" => "Popondetta, Papua New Guinea ",
        "TAT" => "Popraol/Tatry, Slovakia ",
        "PBD" => "Porbandar, India ",
        "POR" => "Pori, Finland ",
        "PMV" => "Porlamar, Venezuela ",
        "YPB" => "Port Alberni, BC ",
        "PTA" => "Port Alsworth, AK ",
        "CLM" => "Port Angeles, WA ",
        "POT" => "Port Antonio, Jamaica ",
        "PAP" => "Port Au Prince, Haiti ",
        "PUG" => "Port Augusta, SA, Australia ",
        "WPB" => "Port Berge, Madagascar ",
        "IXZ" => "Port Blair, India ",
        "KPC" => "Port Clarence, AK ",
        "PLZ" => "Port Elizabeth, South Africa ",
        "POG" => "Port Gentil, Gabon ",
        "PHC" => "Port Harcourt, Nigeria ",
        "YZT" => "Port Hardy, BC ",
        "PHE" => "Port Headland, WA, Australia ",
        "PTH" => "Port Heiden, AK ",
        "YHA" => "Port Hope Simpson, NL ",
        "PLO" => "Port Lincoln, SA, Australia ",
        "PQQ" => "Port Macquarie, NS, Australia ",
        "YPN" => "Port Meiner, QC ",
        "PML" => "Port Moller, AK ",
        "POM" => "Port Moresby, Papua New Guinea ",
        "POS" => "Trinidad, Trinidad and Tobago ",
        "PPV" => "Port Protection, AK ",
        "PSY" => "Port Stanley, Falkland Islands ",
        "PZU" => "Port Sudan, Sudan ",
        "VLI" => "Port Vila, Vanuatu ",
        "PCA" => "Portage Creek, AK ",
        "PWM" => "Portland, ME ",
        "PDX" => "Portland, OR ",
        "PTJ" => "Portland, VI, Australia ",
        "POA" => "Porto Alegre, RS, Brazil ",
        "PXO" => "Porto Santo, Portugal ",
        "BPS" => "Porto Seguro, Brazil ",
        "PVH" => "Porto Velho, RO, Brazil ",
        "OPO" => "Porto, Portugal ",
        "PVO" => "Portoviejo, Ecuador ",
        "PSM" => "Portsmouth, NH ",
        "PSS" => "Posadas, MI, Argentina ",
        "YSO" => "Postville, NL ",
        "POU" => "Poughkeepsie, NY ",
        "YPX" => "Povungnituk, QC ",
        "YPW" => "Powell River, BC ",
        "PAZ" => "Poza Rico, Mexico ",
        "POZ" => "Poznan, Poland ",
        "PRG" => "Prague, Czech Republic ",
        "RAI" => "Praia, Cape Verde ",
        "PRQ" => "Pres. Roque Saenz Pena, CH, Argentina ",
        "PRC" => "Prescott, AZ ",
        "XII" => "Prescott, ON - Rail service ",
        "PPB" => "Presidente Prudente, SP, Brazil ",
        "PQI" => "Presque Isle, ME ",
        "PVK" => "Preveza/Lefkas, Greece ",
        "YPA" => "Prince Albert, SK ",
        "YXS" => "Prince George, BC ",
        "XDV" => "Prince George, BC - Rail service ",
        "YPR" => "Prince Rupert, BC - Digby Island Airport ",
        "XDW" => "Prince Rupert, BC - Rail service ",
        "PRN" => "Pristina, Serbia and Montenegro ",
        "PPP" => "Proserpine, QL, Australia ",
        "PVD" => "Providence, RI ",
        "PLS" => "Providenciales, Turks and Caicos Islands ",
        "PVC" => "Provincetown, MA ",
        "SCC" => "Prudhoe Bay/Deadhorse, AK ",
        "PCL" => "Pucallpa, Peru ",
        "PBC" => "Puebla, Mexico ",
        "PUB" => "Pueblo, CO ",
        "PYH" => "Puerto Ayacucha, Venezuela ",
        "PBE" => "Puerto Berria, Colombia ",
        "PUD" => "Puerto Deseado, SC, Argentina ",
        "PXM" => "Puerto Escondido, Mexico ",
        "PJM" => "Puerto Jimenez, Costa Rica ",
        "PEU" => "Puerto Lempira, Honduras ",
        "PMY" => "Puerto Madryn, CB, Argentina ",
        "PEM" => "Puerto Maldonado, Peru ",
        "PMC" => "Puerto Montt, Chile ",
        "PZO" => "Puerto Ordaz, Venezuela ",
        "POP" => "Puerto Plata, Dominican Republic ",
        "PPS" => "Puerto Princesa, Philippines ",
        "PSZ" => "Puerto Suarez, Bolivia ",
        "PVR" => "Puerto Vallarta, Mexico ",
        "XPK" => "Pukatawagan, MB - ",
        "PUY" => "Pula, Croatia ",
        "PUW" => "Pullman, WA ",
        "PNQ" => "Pune, India ",
        "PUQ" => "Punta Arenas, Chile ",
        "PUJ" => "Punta Cana, Dominican Republic ",
        "PDP" => "Punta Del Este, Uruguay ",
        "PND" => "Punta Gorda, Belize ",
        "PBP" => "Punta Islita, Costa Rica ",
        "PUT" => "Puttaparthi, India ",
        "PSU" => "Putussibau, Indonesia ",
        "FNJ" => "Pyongyang, North Korea ",
        "AQI" => "Qaisumah, Saudi Arabia ",
        "JJU" => "Qaqortoq, Greenland ",
        "IQM" => "Qiemo, China ",
        "YVM" => "Qikiqtarjuaq, NU ",
        "TAO" => "Qingdao, China ",
        "NDG" => "Qiqihar, China ",
        "XQU" => "Qualicum, BC ",
        "YQC" => "Quaqtaq, QC ",
        "YFZ" => "Quebec, QC - Charny Rail service ",
        "YQB" => "Quebec, QC - International Airport ",
        "XLK" => "Quebec, QC - Levis Rail service ",
        "XLJ" => "Quebec, QC - Quebec Station Rail service ",
        "XFY" => "Quebec, QC - Sainte-Foy Rail service ",
        "ZQN" => "Queenstown, New Zealand ",
        "UEL" => "Quelimane, Mozambique ",
        "XQP" => "Quepos, Costa Rica ",
        "QRO" => "Queretaro, Mexico ",
        "YQZ" => "Quesnel, BC ",
        "UET" => "Quetta, Pakistan ",
        "UIH" => "Qui Nhon, Viet Nam ",
        "UIB" => "Quibdo, Colombia ",
        "UIP" => "Quimper, France ",
        "UIN" => "Quincy, IL ",
        "KWN" => "Quinhagak, AK ",
        "ULP" => "Quipi, QL, Australia ",
        "UIO" => "Quito, Ecuador ",
        "RBP" => "Rabaraba, Papua New Guinea ",
        "RBA" => "Rabat, Morocco ",
        "RAB" => "Rabaul, Papua New Guinea ",
        "VKG" => "Roch Gia, Viet Nam ",
        "RAT" => "Raduzhnyi, Russia ",
        "YRA" => "Rae Lakes, NT ",
        "RAH" => "Rafha, Saudi Arabia ",
        "RJN" => "Rafsanjan, Iran ",
        "RFP" => "Raiatea, French Polynesia ",
        "YOP" => "Rainbow Lake, AB ",
        "RAJ" => "Rajkot, India ",
        "RJH" => "Rajshahi, Bangladesh ",
        "RBV" => "Ramato, Solomon Islands ",
        "RAM" => "Ramingining, NT, Australia ",
        "RMP" => "Rampart, AK ",
        "IXR" => "Ranchi, India ",
        "YRT" => "Rankin Inlet, NU ",
        "UNN" => "Ranong, Thailand ",
        "RAP" => "Rapid City, SD ",
        "RAR" => "Rarotonga, Cook Islands ",
        "RKT" => "Ras Al Khaimah, United Arab Emirates ",
        "RAS" => "Rasht, Iran ",
        "RBE" => "Ratanakiri, Cambodia ",
        "RAZ" => "Rawala Kot, Pakistan ",
        "RDG" => "Reading, PA ",
        "RBJ" => "Rebun, Japan ",
        "REC" => "Recife, PE, Brazil ",
        "RCQ" => "Reconquista, SF, Argentina ",
        "RDV" => "Red Devil, AK ",
        "YRL" => "Red Lake, ON ",
        "YRS" => "Red Sucker Lake, MB ",
        "RDD" => "Redding, CA ",
        "RDM" => "Redmond, OR ",
        "REG" => "Reggio Calabria, Italy ",
        "YQR" => "Regina, SK ",
        "RNL" => "Rennell, Solomon Islands ",
        "RNS" => "Rennes, France ",
        "RNO" => "Reno, NV ",
        "YUT" => "Repulse Bay, NU ",
        "RES" => "Resistencia, CW, Argentina ",
        "YRB" => "Resolute, NU ",
        "REU" => "Reus, Spain and Canary Islands ",
        "KEF" => "Reykjavik, Iceland ",
        "REX" => "Reynossa, Mexico ",
        "RHI" => "Rhinelander, WI, ",
        "RHO" => "Rhodes, Gabon ",
        "RAO" => "Ribeirao Preto, SP, Brazil ",
        "RIB" => "Riberalta, Bolivia ",
        "RCB" => "Richards Bay, South Africa ",
        "RCM" => "Richmond, QL, Australia ",
        "RIC" => "Richmond, VA ",
        "RIX" => "Riga, Latvia ",
        "YRG" => "Rigolet, NL ",
        "RJK" => "Rijeka, Croatia ",
        "RMI" => "Rimini, Italy ",
        "YXK" => "Rimouski, QC ",
        "RBR" => "Rio Branco, AC, Brazil ",
        "RCU" => "Rio Cuarto, CD, Argentina ",
        "GIG" => "Rio De Janeiro, RJ, Brazil ",
        "RGL" => "Rio Gallegos, Argentina - Internacional ",
        "RIG" => "Rio Grande, RS, Brazil ",
        "RGA" => "Rio Grande, TF, Argentina ",
        "ROY" => "Rio Mayo, CB, Argentina ",
        "RVD" => "Rio Verde, GO, Brazil ",
        "RCH" => "Riohacha, Colombia ",
        "RIS" => "Rishiri, Japan ",
        "RIW" => "Riverton, WY ",
        "XRP" => "Riviere-a-Pierre, QC - Rail service ",
        "RUH" => "Riyadh, Saudi Arabia ",
        "RIY" => "Riyan Mukalla, Yemen ",
        "RNE" => "Roane, France ",
        "ROA" => "Roanoke, VA ",
        "RTB" => "Roatan, Honduras ",
        "YRJ" => "Roberval, QC ",
        "RCE" => "Roche Harbor, WA ",
        "RST" => "Rochester, MN ",
        "ROC" => "Rochester, NY ",
        "RSD" => "Rock Sound, Bahamas ",
        "RKS" => "Rock Springs, WY ",
        "ZRF" => "Rockford, IL - Park&Ride Bus ",
        "ZRK" => "Rockford, IL - Van Galder Bus ",
        "ROK" => "Rockhampton, QL, Australia ",
        "RKD" => "Rockland, ME ",
        "RDZ" => "Rodez, France ",
        "RRG" => "Rodrigues Island, Mauritius ",
        "RVK" => "Roervik, Norway ",
        "ZXM" => "Rognan, Norway ",
        "RMA" => "Roma, QL, Australia ",
        "ROM" => "Rome, Italy - All airports ",
        "CIA" => "Rome, Italy - Ciampino ",
        "FCO" => "Rome, Italy - Leonardo Da Vinci/Fiumicino ",
        "RNP" => "Rongelap Island, Marshall Islands ",
        "RNB" => "Ronneby, Sweden ",
        "RRS" => "Roros, Norway ",
        "RUR" => "Rorutu, French Polynesia ",
        "ROS" => "Rosario, SF, Argentina ",
        "RSJ" => "Rosario, WA ",
        "RPN" => "Rosh Pina, Iceland ",
        "RET" => "Rost, Norway ",
        "RLG" => "Rostock-Laage, Germany ",
        "ROV" => "Rostov, Russia ",
        "ROW" => "Roswell, NM ",
        "ROP" => "Rota, Northern Mariana Islands ",
        "ROT" => "Rotorua, New Zealand ",
        "RTM" => "Rotterdam, Netherlands ",
        "URO" => "Rouen, France ",
        "ZRJ" => "Round Lake, ON ",
        "RVN" => "Rovaniemi, Finland ",
        "RBY" => "Ruby, AK ",
        "RSH" => "Russian Mission, AK ",
        "RUT" => "Rutland, VT ",
        "SZT" => "S. Cristobal del Casas, Mexico ",
        "QFZ" => "Saarbruecken, Germany ",
        "ZPB" => "Sachigo Lake, ON ",
        "YSY" => "Sachs Harbour, NT ",
        "XKV" => "Sackville, NB - Rail service ",
        "SMF" => "Sacramento, CA ",
        "STC" => "Saint Cloud, MN ",
        "STX" => "St. Croix Island, U.S. Virgin Islands ",
        "STG" => "Saint George Island, AK ",
        "SGU" => "Saint George, UT ",
        "XIM" => "Saint Hyacinthe, QC - Rail service ",
        "YSJ" => "Saint John, NB ",
        "YYT" => "Saint Johns, NL ",
        "YSL" => "Saint Leonard, NB ",
        "STL" => "Saint Louis, MO ",
        "SLU" => "Saint Lucia, ST. LUCIA ",
        "SXM" => "Saint Maarten, Netherlands Antilles ",
        "KSM" => "Saint Mary's, AK ",
        "SMK" => "Saint Michael, AK ",
        "SNP" => "Saint Paul Island, AK ",
        "LED" => "St. Petersburg, Russia ",
        "STT" => "St. Thomas Island, U.S. Virgin Islands ",
        "XPZ" => "Saint Tropez, France ",
        "LTT" => "Saint Tropez, France - La Mole ",
        "SPN" => "Saipan, Northern Mariana Islands ",
        "SNO" => "Sakon Nakhon, Thailand ",
        "SLY" => "Salehard, Russia ",
        "SLE" => "Salem, OR ",
        "SLN" => "Salina, KS ",
        "SBY" => "Salisbury-Ocean City, MD ",
        "YZG" => "Salluit, QC ",
        "SLX" => "Salt Cay, Turks and Caicos Islands ",
        "SLC" => "Salt Lake City, UT ",
        "SLW" => "Saltillo, Mexico ",
        "SSA" => "Salvadore, BA, Brazil ",
        "SZG" => "Salzburg, Austria ",
        "KUF" => "Samara, Russia ",
        "SVB" => "Sambaua, Madagascar ",
        "UAS" => "Samburu, Kenya ",
        "SMI" => "Samos, Greece ",
        "ADZ" => "San Andres Island, Colombia ",
        "SJT" => "San Angelo, TX ",
        "OES" => "San Antonio Oesta, RN, Argentina ",
        "SAT" => "San Antonio, TX ",
        "SVZ" => "San Antonio, Venezuela ",
        "BRC" => "San Carlos, Argentina ",
        "SAN" => "San Diego, CA ",
        "SFO" => "San Francisco, CA ",
        "SJC" => "San Jose, CA ",
        "SJO" => "San Jose, Costa Rica - Juan Santa Maria ",
        "SYQ" => "San Jose, Costa Rica - Tobias Bolanos Int'l ",
        "SJU" => "San Juan, Puerto Rico ",
        "UAQ" => "San Juan, SJ, Argentina ",
        "ULA" => "San Julian, SC, Argentina ",
        "SBP" => "San Luis Obispo, CA ",
        "SLP" => "San Luis Potosi, Mexico ",
        "LUQ" => "San Luis, SL, Argentina ",
        "SAI" => "San Marino, San Marino ",
        "CPC" => "San Martin De Los Andes, Argentina ",
        "NMG" => "San Miguel, Panama ",
        "SAP" => "San Pedro Sula, Honduras ",
        "AFA" => "San Rafael, Argentina ",
        "ZSA" => "San Salvador, Bahamas ",
        "SAL" => "San Salvador, El Salvador ",
        "EAS" => "San Sebastian, Spain ",
        "SAH" => "Sana'a, Yemen ",
        "SDP" => "Sand Point, AK ",
        "NDY" => "Sanday, United Kingdom ",
        "ZSJ" => "Sandy Lake, ON ",
        "SFB" => "Sanford, FL ",
        "YSK" => "Sanikiluaq, NU ",
        "NNB" => "Santa Ana, Solomon Islands ",
        "SBA" => "Santa Barbara, CA ",
        "STB" => "Santa Barbara, Ed, Venezuela ",
        "SPC" => "Santa Cruz De La Palma, Spain and Canary Islands - La Palma ",
        "VVI" => "Santa Cruz, Bolivia ",
        "SAF" => "Santa Fe, NM ",
        "RIA" => "Santa Maria, Brazil ",
        "SMX" => "Santa Maria, CA ",
        "SMA" => "Santa Maria, Portugal ",
        "SMR" => "Santa Marta, Colombia ",
        "STS" => "Santa Rosa, CA ",
        "RSA" => "Santa Rosa, LP, Argentina ",
        "STM" => "Santarem, PA, Brazil ",
        "SMS" => "Sante Marie, Madagascar ",
        "SCL" => "Santiago, Chile ",
        "STI" => "Santiago, Dominican Republic ",
        "GEL" => "Santo Angelo, Brazil ",
        "NTO" => "Santo Antao, Cape Verde ",
        "HEX" => "Santo Domingo, Dominican Republic - Herrera ",
        "SDQ" => "Santo Domingo, Dominican Republic - Las Americas ",
        "STD" => "Santo Domingo, Venezuela ",
        "SYX" => "Sanya, China ",
        "SNE" => "Sao Nicolau, Cape Verde ",
        "CGH" => "Sao Paulo, Brazil - Congonhas ",
        "GRU" => "Sao Paulo, Brazil - Guarulhos Intl ",
        "VCP" => "Sao Paulo, Brazil - Viracopos ",
        "TMS" => "Sao Tome Is., Sao Tome and Principe ",
        "VXE" => "Sao Vicente, Cape Verde ",
        "CTS" => "Sapporo, Japan - Chitose ",
        "OKD" => "Sapporo, Japan - Okadama ",
        "SSR" => "Sara, Vanuatu ",
        "SLK" => "Saranac Lake, NY ",
        "SRQ" => "Sarasota, FL ",
        "RTW" => "Saratov, Russia ",
        "ZRM" => "Sarmi, Indonesia ",
        "YZR" => "Sarnia, ON ",
        "XDX" => "Sarnia, ON - Rail service ",
        "YXE" => "Saskatoon, SK ",
        "SUJ" => "Satu Mare, Romania ",
        "SWG" => "Satwag, Papua New Guinea ",
        "SLZ" => "Sau Luiz, MA, Brazil ",
        "CIU" => "Sault Ste Marie, MI, ",
        "YAM" => "Sault Ste-Marie, ON ",
        "SXK" => "Saumlaki, Indonesia ",
        "SAV" => "Savannah, GA ",
        "ZVK" => "Savannakhet, Laos ",
        "SVL" => "Savonlinna, Finland ",
        "SVA" => "Savoonga, AK ",
        "SVU" => "Savusavu, Fiji ",
        "ZBY" => "Sayaboury, Laos ",
        "SCM" => "Scammon Bay, AK ",
        "YKL" => "Schefferville, QC ",
        "BFF" => "Scottsbluff, NE ",
        "AVP" => "Wilkes Barre, PA ",
        "LKE" => "Seattle, WA - Lake Union SPB ",
        "SEA" => "Seattle, WA - Seattle/Tacoma International ",
        "EGM" => "Sege, Solomon Islands ",
        "GXF" => "Seiyun, Yemen ",
        "WLK" => "Selawik, AK ",
        "QFK" => "Selje, Norway ",
        "SRG" => "Semarang, Indonesia ",
        "ZEG" => "Senggo, Indonesia ",
        "XFK" => "Senneterre, QC - Rail service ",
        "SEL" => "Seoul, South Korea - All Airports ",
        "GMP" => "Seoul, South Korea - Gimpo International ",
        "ICN" => "Seoul, South Korea - Incheon International ",
        "YZV" => "Sept-Iles, QC ",
        "ZRI" => "Servi, Indonesia ",
        "SZM" => "Sesriem, Namibia ",
        "SVQ" => "Sevilla, Spain and Canary Islands ",
        "SWD" => "Seward, AK ",
        "SHX" => "Shageluk, AK ",
        "SKK" => "Shaktoolik, AK ",
        "ZTM" => "Shamattawa, MB ",
        "PVG" => "Shanghai, China ",
        "SNN" => "Shannon, Ireland ",
        "SWA" => "Shantou, China ",
        "SSH" => "Sharm El Sheikh, Egypt ",
        "XFL" => "Shawinigan, QC - Rail service ",
        "XFM" => "Shawnigan, BC - Rail service ",
        "SZD" => "Sheffield, United Kingdom ",
        "SXP" => "Sheldon Point, AK ",
        "SZX" => "Shenzhen, China ",
        "SHR" => "Sheridan, WY ",
        "LWK" => "Shetland Islands, United Kingdom - Lerwick/Tingwall ",
        "LSI" => "Shetland Islands, United Kingdom - Sumburgh ",
        "HIL" => "Shillavo, Ethiopia ",
        "CIT" => "Shimkent, Kazakhstan ",
        "SYZ" => "Shiraz, Iran ",
        "SHH" => "Shishmaref, AK ",
        "SYO" => "Shonai, Japan ",
        "SHV" => "Shreveport, LA ",
        "SHG" => "Shungnak, AK ",
        "JHQ" => "Shute Harbor, Australia ",
        "REP" => "Siem Reap, Cambodia ",
        "IXS" => "Silchar, India ",
        "SVC" => "Silver City, NM ",
        "SYM" => "Simao, China ",
        "NKD" => "Sinak, Indonesia ",
        "SIN" => "Singapore, Singapore - Changi ",
        "XSP" => "Singapore, Singapore - Seletar ",
        "SQG" => "Sintang, Indonesia ",
        "SUX" => "Sioux City, IA ",
        "FSD" => "Sioux Falls, SD ",
        "YXL" => "Sioux Lookout, ON ",
        "JHS" => "Sisimiut, Greenland ",
        "SIT" => "Sitka, AK ",
        "AKY" => "Sittwe, Myanmar ",
        "VAS" => "Sivas, Turkey ",
        "SGY" => "Skagway, AK ",
        "JSI" => "Skiathos, Greece ",
        "FYR" => "Skopie, Macedonia ",
        "KVB" => "Skovde, Sweden ",
        "SZK" => "Skukuza, South Africa ",
        "SLQ" => "Sleetmore, AK ",
        "SXL" => "Sligo, Ireland ",
        "YSH" => "Smith Falls, ON ",
        "YYD" => "Smithers, BC ",
        "YFJ" => "Snare Lake, NT ",
        "DWB" => "Soalala, Madagascar ",
        "SOO" => "Soderham, Sweden ",
        "SOF" => "Sofia, Bulgaria ",
        "SOG" => "Sognolal, Norway ",
        "SOC" => "Solo City, Indonesia ",
        "SQH" => "Son-La, Viet Nam - Na-San ",
        "SOJ" => "Sorkjosen, Norway ",
        "SOD" => "Sorocaba, Brazil ",
        "TZN" => "South Andros, Bahamas ",
        "SBN" => "South Bend, IN ",
        "XSC" => "South Caicos, Turks and Caicos Islands ",
        "SOU" => "South Hampton, United Kingdom ",
        "XSI" => "South Indian Lake, MB ",
        "SOI" => "South Molle Island, QL, Australia ",
        "WSN" => "South Naknek, AK ",
        "SOP" => "Southern Pines, NC ",
        "SPU" => "Split, Croatia ",
        "GEG" => "Spokane, WA ",
        "AXP" => "Spring Point, Bahamas ",
        "SPI" => "Springfield, IL ",
        "SGF" => "Springfield, MO ",
        "SXR" => "Srinagar, India ",
        "YAY" => "St Anthony, NL ",
        "YCM" => "St Catharines, ON ",
        "RUN" => "St Denis de la Reunion, Reunion ",
        "SKB" => "St Kitts, St Kitts and Nevis ",
        "XIO" => "St Marys, ON - Rail service ",
        "FSP" => "St Pierre, St Pierre and Miquelon ",
        "SVD" => "St Vincent, Saint Vincent and the Grenadines ",
        "EBU" => "St. Etienne, France ",
        "EUX" => "St. Eustatius, Netherlands Antilles ",
        "UVF" => "St. Lucia, St. Lucia - Hawnorra ",
        "ZSE" => "St. Pierre de la Reunion, Reunion ",
        "SCE" => "State College/University Park, PA ",
        "SHD" => "Staunton, VA ",
        "SVG" => "Stavanger, Norway ",
        "STW" => "Stavropol, Russia ",
        "YST" => "Ste Therese Point, MB ",
        "SBS" => "Steamboat Springs, CO ",
        "WBB" => "Stebbins, AK ",
        "SML" => "Stella Maris, Bahamas ",
        "YJT" => "Stephenville, NL ",
        "CWA" => "Wausau/Stevens Point, WI ",
        "SVS" => "Stevens Village, AK ",
        "STO" => "Stockholm, Sweden - All airports ",
        "ARN" => "Stockholm, Sweden - Arlanda ",
        "BMA" => "Stockholm, Sweden - Bromma ",
        "SCK" => "Stockton, CA ",
        "YSF" => "Stony Rapids, SK ",
        "SRV" => "Stony River, AK ",
        "SYY" => "Stornoway, United Kingdom ",
        "SQO" => "Storuman, Sweden ",
        "XER" => "Strasbourg, France - Bus service ",
        "SXB" => "Strasbourg, France - Entzheim ",
        "XTY" => "Strathroy, ON - Rail service ",
        "SOY" => "Stronsay, United Kingdom ",
        "TNX" => "Stung Treng, Cambodia ",
        "STR" => "Stuttgart, Germany - Echterdingen ",
        "ZWS" => "Stuttgart, Germany - Rail service ",
        "VAO" => "Suavanao, Solomon Islands ",
        "SRE" => "Sucre, Bolivia ",
        "YSB" => "Sudbury, ON ",
        "XDY" => "Sudbury, ON - Rail service ",
        "SYU" => "Sue Island, QL, Australia ",
        "SUL" => "Sui, Pakistan ",
        "THS" => "Sukhotthai, Thailand ",
        "SUR" => "Summer Beaver, ON ",
        "NTY" => "Sun City, South Africa ",
        "MCY" => "Sunshine Coast, QL, Australia ",
        "SUB" => "Surabaya, Indonesia ",
        "URT" => "Surat Thani, Thailand ",
        "SUV" => "Suva, Fiji ",
        "EVG" => "Sveg, Sweden ",
        "SVJ" => "Svolvaer, Norway ",
        "ZJN" => "Swan River, MB ",
        "SYD" => "Sydney, New South Wales, Australia ",
        "YQY" => "Sydney, NS ",
        "ZYL" => "Sylhet, Bangladesh ",
        "SYR" => "Syracuse, NY ",
        "SZZ" => "Szczecin, Poland ",
        "TCP" => "Taba, Egypt ",
        "TBJ" => "Tabarka, Tunisia ",
        "TBT" => "Tabatinga, AM, Brazil ",
        "TBO" => "Tabora, Tanzania ",
        "TBZ" => "Tabriz, Iran ",
        "TBG" => "Tabubil, Papua New Guinea ",
        "TUU" => "Tabuk, Saudi Arabia ",
        "TCG" => "Tacheng, China ",
        "THL" => "Tachilek, Myanmar ",
        "TAC" => "Tacloban, Philippines ",
        "TCQ" => "Tacna, Peru ",
        "XTL" => "Tadoule Lake, MB ",
        "TXG" => "Taichung, Taiwan ",
        "TIF" => "Taif, Saudi Arabia ",
        "TNN" => "Tainan, Taiwan ",
        "TPE" => "Taipei, Taiwan - Chiang Kai Shek ",
        "TSA" => "Taipei, Taiwan - Sung Shan ",
        "TTT" => "Taitung, Taiwan ",
        "TYN" => "Taiyuan, China ",
        "TAI" => "Taiz, Yemen ",
        "TCT" => "Takotna, AK ",
        "TKA" => "Talkeetna, AK ",
        "TLH" => "Tallahassee, FL ",
        "TLL" => "Tallinn, Estonia ",
        "YYH" => "Taloyoak, NU ",
        "TMR" => "Tamanrasset, Algeria ",
        "TNO" => "Tamarindo, Costa Rica ",
        "TMM" => "Tamatave, Madagascar ",
        "WTA" => "Tambohorano, Madagascar",
        "TMC" => "Tambolaka, Indonesia ",
        "TMU" => "Tambor, Costa Rica ",
        "TPA" => "Tampa, FL ",
        "TMP" => "Tampere, Finland ",
        "TAM" => "Tampico, Mexico ",
        "TMW" => "Tamworth, NS, Australia ",
        "TMH" => "Tanahmerah, Indonesia ",
        "TAL" => "Tanana, AK ",
        "TNG" => "Tangier, Morocco ",
        "TJQ" => "Tanjung Pandan, Indonesia ",
        "TJS" => "Tanjung Selor, Indonesia ",
        "TAH" => "Tanna, Vanuatu ",
        "TSM" => "Taos, NM ",
        "TAP" => "Tapachula, Mexico ",
        "TRK" => "Tarakan, Indonesia ",
        "TRA" => "Taramajma, Japan ",
        "TAR" => "Taranto, Italy ",
        "TPP" => "Tarapoto, Peru ",
        "TRW" => "Tarawa, Kiribati ",
        "TRO" => "Taree, NS, Australia ",
        "TIZ" => "Tari, Papua New Guinea ",
        "TJA" => "Tarija, Bolivia ",
        "TAS" => "Tashkent, Uzbekistan ",
        "YTQ" => "Tasiujuaq, QC ",
        "TEK" => "Tatitlek, AK ",
        "TUO" => "Taupo, New Zealand ",
        "TRG" => "Tauranga, New Zealand ",
        "TVU" => "Taveuni, Fiji ",
        "TWU" => "Tawau, Malaysia ",
        "TEE" => "Tbessa, Algeria ",
        "TBS" => "Tbilisi, Georgia ",
        "TCH" => "Tchibanga, Gabon ",
        "TEU" => "Te Anau, New Zealand ",
        "MME" => "Teesside, United Kingdom ",
        "TFF" => "Tefe, AM, Brazil ",
        "TGU" => "Tegucigalpa, Honduras ",
        "THR" => "Tehran, Iran - Mehrabad ",
        "TKB" => "Tekadu, Papua New Guinea ",
        "TLV" => "Tel Aviv, Israel ",
        "KTS" => "Teller Mission, AK ",
        "TEX" => "Telluride, CO ",
        "TIM" => "Tembagapura, Indonesia ",
        "TXM" => "Teminabuan, Indonesia ",
        "ZCO" => "Temuco, Chile ",
        "TKE" => "Tenakee Springs, AK ",
        "TFN" => "Tenerife, Spain and Canary Islands - Norte Los Rodeos ",
        "TFS" => "Tenerife, Spain and the Canary Islands - Sur Reina Sofia ",
        "TCA" => "Tennant Creek, NT, Australia ",
        "TPQ" => "Tepic, Mexico ",
        "TER" => "Terceira Island, Portugal ",
        "THE" => "Teresina, PI, Brazil ",
        "TMJ" => "Termez, Uzbekistan ",
        "TTE" => "Ternate, Indonesia ",
        "YXT" => "Terrace, BC ",
        "HUF" => "Terre Haute, IN ",
        "TDB" => "Tetabedi, Papua New Guinea ",
        "TET" => "Tete, Mozambique ",
        "ZTB" => "Tete-a-La Baleine, QC ",
        "TEH" => "Tetlin, AK ",
        "TTU" => "Tetuan, Morocco ",
        "TXK" => "Texarkana, AR ",
        "TEZ" => "Tezpur, India ",
        "SNW" => "Thandwe, Myanmar ",
        "THG" => "Thangool, QL, Australia ",
        "XTG" => "Thargomindah, QL, Australia ",
        "TBI" => "The Bight, Bahamas ",
        "YQD" => "The Pas, MB ",
        "XDZ" => "The Pas, MB - Rail service ",
        "SKG" => "Thessaloniki, Greece ",
        "YTD" => "Thicket Portage, MB ",
        "TVF" => "Thief River Falls, MN ",
        "JTR" => "Thira, Greece ",
        "TRV" => "Thiruvananthapuram, India ",
        "YTH" => "Thompson, MB ",
        "KTB" => "Thorne Bay, AK ",
        "THO" => "Thorshofn, Iceland ",
        "YQT" => "Thunder Bay, ON ",
        "TIS" => "Thursday Island, QL, Australia ",
        "TSN" => "Tianjn, China ",
        "TID" => "Tiaret, Algeria ",
        "TGJ" => "Tiga, New Caledonia ",
        "TIJ" => "Tijuana, Mexico ",
        "TIH" => "Tikehau Atoll, French Polynesia ",
        "IKS" => "Tiksi, Russia ",
        "TIU" => "Timaru, New Zealand ",
        "TMX" => "Timimoun, Algeria ",
        "YTS" => "Timmins, ON ",
        "TSR" => "Timosoara, Romania ",
        "TNC" => "Tin City, AK ",
        "TIN" => "Tindouf, Algeria ",
        "TIQ" => "Tinian, Northern Mariana Islands ",
        "TIY" => "Tioljikja, Mauritania ",
        "TOD" => "Tioman, Malaysia ",
        "TIE" => "Tippi, Ethiopia ",
        "TIA" => "Tirana, Albania ",
        "TRE" => "Tiree, United Kingdom ",
        "TGM" => "Tirgu Mures, Romania ",
        "TRZ" => "Tiruchirapally, India ",
        "TIR" => "Tirupati, India ",
        "TIV" => "Tivat, Serbia and Montenegro ",
        "TLM" => "Tlemcen, Algeria ",
        "TAB" => "Tobago, Trinidad and Tobago ",
        "TOB" => "Tobruk, Libya ",
        "YAZ" => "Tofino, BC, ",
        "TOG" => "Togiak Village, AK ",
        "TKJ" => "Tok, AK ",
        "OOK" => "Toksook Bay, AK ",
        "TKN" => "Tokunoshima, Japan ",
        "TKS" => "Tokushima, Japan ",
        "TYO" => "Tokyo, Japan - All airports ",
        "HND" => "Tokyo, Japan - Haneda ",
        "NRT" => "Tokyo, Japan - Narita ",
        "TOL" => "Toledo, OH ",
        "TOW" => "Toledo, PR, Brazil ",
        "TLC" => "Toluco, Mexico ",
        "TPR" => "Tom Price, WA, Australia ",
        "TMG" => "Tomanggong, Malaysia ",
        "TOM" => "Tombouctou, Mali ",
        "TOF" => "Tomsk, Russia ",
        "TGO" => "Tongliao, China ",
        "TGH" => "Tongoa, Vanuatu ",
        "TWB" => "Toowoomba, QL, Australia ",
        "FOE" => "Topeka, KS ",
        "YBZ" => "Toronto, ON - Downtown Rail service ",
        "XLQ" => "Toronto, ON - Guildwood Rail service ",
        "YYZ" => "Toronto, ON - International ",
        "YTZ" => "Toronto, ON - Toronto Island Airport ",
        "TRF" => "Torp Airport Oslo, Norway - Sandefjord ",
        "TRC" => "Torreon, Mexico ",
        "TOH" => "Torres, Vanuatu ",
        "TYF" => "Torsby, Sweden ",
        "TOV" => "Tortola, British Virgin Islands ",
        "TTB" => "Tortoli, Italy ",
        "TTQ" => "Tortuquero, Costa Rica ",
        "TTJ" => "Tottori, Japan ",
        "TOU" => "Touho, New Caledonia ",
        "TLN" => "Toulon, France ",
        "TLS" => "Toulouse, France ",
        "TUF" => "Tours, France ",
        "XSH" => "Tours, France - Rail service ",
        "TSV" => "Townsville, QL, Australia ",
        "TOY" => "Toyama, Japan ",
        "TOE" => "Tozeur, Tunisia ",
        "TZX" => "Trabzon, Turkey ",
        "TST" => "Trang, Thailand ",
        "TPS" => "Trapani, Italy ",
        "TGN" => "Traralgon, VI, Australia ",
        "TVC" => "Traverse City, MI ",
        "TCB" => "Treasure Cay, Bahamas ",
        "REL" => "Trelew, CB, Argentina ",
        "TRS" => "Trieste, Italy ",
        "TDD" => "Trinidad, Bolivia ",
        "TIP" => "Tripoli, Libya ",
        "THN" => "Trollhattan, Sweden ",
        "TMT" => "Trombetas, PA, Brazil ",
        "TOS" => "Tromso, Norway ",
        "TRD" => "Trondheim, Norway ",
        "TJI" => "Trujillo, Honduras ",
        "TRU" => "Trujillo, Peru ",
        "TKK" => "Truk, Micronesia ",
        "XLZ" => "Truro, NS - Rail service ",
        "TTS" => "Tsaratanana, Madagascar ",
        "WTS" => "Tsiroanomandidy, Madagascar ",
        "TSB" => "Tsumeb, Namibia ",
        "TSJ" => "Tsushima, Japan ",
        "TUB" => "Tubuai, French Polynesia ",
        "TUS" => "Tucson, AZ ",
        "TUC" => "Tucuman, TU, Argentina ",
        "TUV" => "Tucupita, Venezuela ",
        "TUR" => "Tucurui, PA, Brazil ",
        "TFI" => "Tufi, Papua New Guinea ",
        "TUG" => "Tuguegarao, Philippines ",
        "YUB" => "Tuktoyaktuk, NT ",
        "TUA" => "Tulcan, Ecuador ",
        "TLE" => "Tulear, Madagascar ",
        "ZFN" => "Tulita/Fort Norman, NT ",
        "TUL" => "Tulsa, OK ",
        "TLT" => "Tuluksak, AK ",
        "TUJ" => "Tum, Ethiopia ",
        "TCO" => "Tumaco, Colombia ",
        "TBP" => "Tumbes, Peru ",
        "TUN" => "Tunis, Tunisia ",
        "WTL" => "Tuntutuliak, AK ",
        "TNK" => "Tununak, AK ",
        "TXN" => "Tunxi, China ",
        "TUP" => "Tupelo, MS ",
        "TUI" => "Turaif, Saudi Arabia ",
        "TUK" => "Turbat, Pakistan ",
        "TRN" => "Turin, Italy ",
        "TKU" => "Turku, Finland ",
        "TCL" => "Tuscaloosa, AL ",
        "TGZ" => "Tuxtla Gutierrez, Mexico ",
        "TWF" => "Twin Falls, ID ",
        "TWA" => "Twin Hills, AK ",
        "TYR" => "Tyler, TX ",
        "TJM" => "Tyumen, Russia ",
        "UBJ" => "Ube, Japan ",
        "UBA" => "Uberaba, MG, Brazil ",
        "UDI" => "Uberlandia, MG, Brazil ",
        "UBP" => "Ubon Ratchathani, Thailand ",
        "UDR" => "Udaipur, India ",
        "UTH" => "Udon Thani, Thailand ",
        "UFA" => "Ufa, Russia ",
        "UJE" => "Ujae Island, Marshall Islands ",
        "UPG" => "Ujung Pandang, Indonesia ",
        "UCT" => "Ukhta, Russia ",
        "ULN" => "Ulaanbaatar, Mongolia ",
        "HLH" => "Ulanhot, China ",
        "UUD" => "Ulan-Ude, Russia ",
        "ULB" => "Ulei, Vanuatu ",
        "ULY" => "Uliastai, Mongolia ",
        "ULI" => "Ulithi, Micronesia ",
        "USN" => "Ulsan, South Korea ",
        "ULD" => "Ulundi, South Africa ",
        "UME" => "Umea, Sweden ",
        "YUD" => "Umiujag, QC ",
        "UTT" => "Umtata, South Africa ",
        "UNK" => "Unalakleet, AK ",
        "JUV" => "Upernavik, Greenland ",
        "UTN" => "Upington, South Africa ",
        "URJ" => "Uraj, Russia ",
        "URA" => "Uralsk, Kazakhstan ",
        "YBE" => "Uranium City, SK ",
        "UGC" => "Urgench, Uzbekistan ",
        "OMH" => "Urmieh, Iran ",
        "URU" => "Uroubi, Papua New Guinea ",
        "UPN" => "Uruapan, Mexico ",
        "URG" => "Uruguaiana, RS, Brazil ",
        "URC" => "Urumqi, China ",
        "USL" => "Useless Loop, WA, Australia ",
        "USH" => "Ushuaia, TF, Argentina ",
        "USK" => "Usinsk, Russia ",
        "UIK" => "Ust-Ilimsk, Russia ",
        "UKK" => "Ust-Kamenogorsk, Kazakhstan ",
        "UTP" => "Utapao, Thailand ",
        "UCA" => "Utica, NY ",
        "UII" => "Utila, Honduras ",
        "UTK" => "Utirik Island, Marshall Islands ",
        "UTO" => "Utopia Creek, AK ",
        "UMD" => "Uummannaq, Greeland ",
        "UDJ" => "Uzhgorod, Ukraine ",
        "VAA" => "Vaasa, Finland ",
        "BDQ" => "Vadodara, India ",
        "VDS" => "Vadso, Norway ",
        "EGE" => "Vail, CO - Eagle County Airport ",
        "QBF" => "Vail, CO - Van service ",
        "VDZ" => "Valdez, AK ",
        "ZAL" => "Valdivia, Chile ",
        "YVO" => "Val-d'Or, QC ",
        "VLD" => "Valdosta, GA ",
        "VLC" => "Valencia, Spain ",
        "VLN" => "Valencia, Venezuela ",
        "XVS" => "Valenciennes, France ",
        "VLV" => "Valera, Venezuela ",
        "VLS" => "Valesdir, Vanuatu ",
        "VLL" => "Valladolid, Spain and Canary Islands ",
        "VUP" => "Valledupar, Colombia ",
        "VDE" => "Valverde, Spain and Canary Islands - Hierro ",
        "VAN" => "Van, Turkey ",
        "YVR" => "Vancouver, BC - International ",
        "CXH" => "Vancouver, BC - Coal Harbour ",
        "XEA" => "Vancouver, BC - Rail service ",
        "VAI" => "Vanimo, Papua New Guinea ",
        "VBV" => "Vanuabalavu, Fiji ",
        "VRA" => "Varadero, Cuba ",
        "VNS" => "Varanasi, India ",
        "VAW" => "Vardoe, Norway ",
        "VAG" => "Varginha, MG, Brazil ",
        "VRK" => "Varkaus, Finland ",
        "VAR" => "Varna, Bulgaria ",
        "VST" => "Vasteras, Sweden ",
        "VAT" => "Vatomatry, Madagascar ",
        "VAV" => "Vava'u, Tonga ",
        "VXO" => "Vaxjo, Sweden ",
        "VEE" => "Venetie, AK ",
        "VCE" => "Venice, Italy - Marco Polo ",
        "TSF" => "Venice, Italy - Treviso ",
        "VER" => "Veracruz, Mexico ",
        "VEL" => "Vernal, UT ",
        "VRN" => "Verona, Italy ",
        "VEY" => "Vestmannaeyjar, Iceland ",
        "VFA" => "Victoria Falls, Zimbabwe ",
        "VCD" => "Victoria River Downs, NT, Australia ",
        "YWH" => "Victoria, BC - Inner Harbor ",
        "YYJ" => "Victoria, BC - International ",
        "VCT" => "Victoria, TX ",
        "VDM" => "Viedma, RN, Argentina ",
        "VIE" => "Vienna, Austria ",
        "VTE" => "Vientiane, Laos - Wattay ",
        "VQS" => "Vieques, Puerto Rico ",
        "VGO" => "Vigo, Spain ",
        "VNX" => "Vilanculos, Mozambique ",
        "VHM" => "Vilhelmina, Sweden ",
        "BVH" => "Vilhena, Brazil ",
        "VLG" => "Villa Gesell, BA, Argentina ",
        "VME" => "Villa Mercedes, SL, Argentina ",
        "VSA" => "Villahermosa, Mexico ",
        "VNO" => "Vilnius, Lithuania ",
        "VII" => "Vinh City, Viet Nam ",
        "VIJ" => "Virgin Gorda, British Virgin Islands ",
        "VIS" => "Visalia, CA ",
        "VBY" => "Visby, Sweden ",
        "VTZ" => "Vishakhapatnam, India ",
        "VTB" => "Vitebsk, Belarus ",
        "VDC" => "Vitoria da Conquista, BA, Brazil ",
        "VIT" => "Vitoria, Spain and Canary Islands ",
        "VIV" => "Vivigani, Papua New Guinea ",
        "OGZ" => "Vladikavkaz, Russia ",
        "VVO" => "Vladivostok, Russia ",
        "VOH" => "Vohemar, Madagascar ",
        "VLK" => "Volgodonsk, Russia ",
        "VOG" => "Volgograd, Russia ",
        "VPN" => "Vopnafjordur, Iceland ",
        "VKT" => "Vorkuta, Russia ",
        "VOZ" => "Voronezh, Russia ",
        "YWK" => "Wabush, NL ",
        "ACT" => "Waco, TX ",
        "WAE" => "Wadi Ad Dawasir, Saudi Arabia ",
        "WHF" => "Wadi Halfa, Sudan ",
        "WET" => "Wagethe, Indonesia ",
        "WGA" => "Wagga Wagga, NS, Australia ",
        "WBA" => "Wahai, Indonesia ",
        "WGP" => "Waingapo, Indonesia ",
        "AIN" => "Wainwright, AK ",
        "WKJ" => "Wakkanai, Japan ",
        "WLH" => "Walaha, Vanuatu ",
        "WAA" => "Wales, AK ",
        "WGE" => "Walgett, NS, Australia ",
        "ALW" => "Walla Walla, WA ",
        "WLS" => "Wallis Island, Wallis and Futuna Islands ",
        "WVB" => "Walvis Bay, Namibia ",
        "WMX" => "Wamena, Indonesia ",
        "WKA" => "Wanaka, New Zealand ",
        "WAG" => "Wanganui, New Zealand ",
        "AGE" => "Wangerooge, Germany ",
        "AGL" => "Wanigela, Papua New Guinea ",
        "WXN" => "Wanxian, China ",
        "WAW" => "Warsaw, Poland ",
        "WAS" => "Washington DC - All airports ",
        "IAD" => "Washington DC - Dulles ",
        "DCA" => "Washington DC - National ",
        "WSR" => "Wasior, Indonesia ",
        "YKQ" => "Waskaganish, QC ",
        "WSU" => "Wasu, Papua New Guinea ",
        "KWF" => "Waterfall, AK ",
        "WAT" => "Waterford, Ireland ",
        "ALO" => "Waterloo, IA ",
        "ART" => "Watertown, NY ",
        "ATY" => "Watertown, SD ",
        "XWA" => "Watford, ON - Rail service ",
        "WUG" => "Wau, Papua New Guinea ",
        "WUU" => "Wau, Sudan ",
        "YWP" => "Webequie, ON ",
        "WED" => "Wedau, Papua New Guinea ",
        "EJH" => "Wedjh, Saudi Arabia ",
        "WEH" => "Weihai, China ",
        "WEI" => "Weipa, QL, Australia ",
        "WLG" => "Wellington, New Zealand ",
        "YNC" => "Wemindji, QC ",
        "EAT" => "Wenatchee, WA ",
        "WNZ" => "Wenzhou, China ",
        "PBI" => "West Palm Beach, FL ",
        "WYS" => "West Yellowstone, MT ",
        "HPN" => "Westchester County, NY ",
        "GWT" => "Westerland, Germany ",
        "WST" => "Westerly, RI ",
        "WSZ" => "Westport, New Zealand ",
        "WRY" => "Westray, United Kingdom ",
        "WSX" => "Westsound, WA ",
        "WWK" => "Wewak, Papua New Guinea ",
        "XFQ" => "Weymont, QC - Rail service ",
        "YLE" => "Wha Ti/Lac La Martre, NT ",
        "WHK" => "Whakatane, New Zealand ",
        "YXN" => "Whale Cove, NU ",
        "WWP" => "Whale Pass, AK ",
        "WRE" => "Whangarei, New Zealand ",
        "WMO" => "White Mountain, AK ",
        "YWR" => "White River, ON ",
        "YXY" => "Whitehorse, YT ",
        "WYA" => "Whyalla, SA, Australia ",
        "SPS" => "Wichita Falls, TX ",
        "ICT" => "Wichita, KS ",
        "WIC" => "Wick, United Kingdom ",
        "WVN" => "Wilhelmshaven, Germany ",
        "YWM" => "Williams Harbour, NL ",
        "YWL" => "Williams Lake, BC ",
        "IPT" => "Williamsport, PA ",
        "ISN" => "Williston, ND ",
        "ILM" => "Wilmington, NC ",
        "WUN" => "Wiluna, WA, Australia ",
        "WNR" => "Windarra, QL, Australia ",
        "YQG" => "Windsor, ON ",
        "XEC" => "Windsor, ON - Rail service ",
        "YWG" => "Winnipeg, MB - International ",
        "XEF" => "Winnipeg, MB - Rail service ",
        "WIN" => "Winton, QL, Australia ",
        "WJA" => "Woja, Marshall Islands ",
        "ZWL" => "Wollaston Lake, SK ",
        "WOT" => "Wonan, Taiwan ",
        "WJU" => "WonJu, South Korea ",
        "XIP" => "Woodstock, ON - Rail service ",
        "ORH" => "Worcester, MA ",
        "WRL" => "Worland, WY ",
        "WTO" => "Wotho Island, Marshall Islands ",
        "WTE" => "Wotje Island, Marshall Islands ",
        "WRG" => "Wrangell, AK ",
        "WRO" => "Wroclaw, Poland ",
        "WUD" => "Wudinna, SA, Australia ",
        "WUH" => "Wuhan, China ",
        "WNN" => "Wunnummin Lake, ON ",
        "WUS" => "Wuyishan, China ",
        "WYN" => "Wyndham, WA, Australia ",
        "XWY" => "Wyoming, ON - Rail service ",
        "XIY" => "Xi An, China - Xianyang ",
        "XMN" => "Xiamen, China ",
        "XFN" => "Xiangfan, China ",
        "XIC" => "Xichang, China ",
        "XKH" => "Xieng Khouang, Laos ",
        "XIL" => "Xilinhot, China ",
        "XNN" => "Xining, China ",
        "XUZ" => "Xuzhou, China ",
        "YKM" => "Yakima, WA ",
        "YAK" => "Yakutat, AK ",
        "YKS" => "Yakutsk, Russia ",
        "KYX" => "Yalumet, Papua New Guinea ",
        "XMY" => "Yam Island, QL, Australia ",
        "GAJ" => "Yamagata, Japan ",
        "ENY" => "Yan'an, China ",
        "YNB" => "Yanbu, Saudi Arabia ",
        "YNZ" => "Yancheng, China ",
        "XYA" => "Yandina, Solomon Islands ",
        "RGN" => "Yangon, Myanmar ",
        "YNJ" => "Yanji, China ",
        "YNT" => "Yantai, China ",
        "YAO" => "Yaounde, Cameroon ",
        "YAP" => "Yap, Micronesia ",
        "YQI" => "Yarmouth, NS ",
        "IAR" => "Yaroslavl, Russia ",
        "AZD" => "Yazd, Iran ",
        "EYL" => "Yelimane, Mali ",
        "YZF" => "Yellowknife, NT ",
        "RSU" => "Yeosu, South Korea ",
        "EVN" => "Yerevan, Armenia ",
        "YBP" => "Yibin, China ",
        "YIH" => "Yichang, China ",
        "INC" => "Yinchuan, China ",
        "YIN" => "Yining, China ",
        "YIW" => "Yiwu, China ",
        "JOG" => "Yogyakarta, Indonesia ",
        "YGJ" => "Yonago, Japan ",
        "OGN" => "Yonaguni Jima, Japan ",
        "ZAC" => "York Landing, MB ",
        "OKR" => "Yorke Island, QL, Australia ",
        "RNJ" => "Yoronjima, Japan ",
        "YNG" => "Youngstown, OH ",
        "UYN" => "Yulin, China ",
        "YUM" => "Yuma, AZ ",
        "UUS" => "Yuzhno-Sakhalinsk, Russia ",
        "ZAD" => "Zadar, Croatia ",
        "ZAG" => "Zagreb, Croatia ",
        "ZAH" => "Zahedan, Iran ",
        "ZTH" => "Zakinthos Island, Greece ",
        "ZAM" => "Zamboanga, Philippines ",
        "ZNZ" => "Zanzibar, Tanzania - Kisauni ",
        "OZH" => "Zaporozhye, Ukraine ",
        "ZAZ" => "Zaragoza, Spain and Canary Islands ",
        "ZHA" => "Zhanjiang, China ",
        "ZAT" => "Zhaotong, China ",
        "CGO" => "Zhengzha, China ",
        "PZH" => "Zhob, Pakistan ",
        "HSN" => "Zhoushan, China ",
        "ZUH" => "Zhuhai, China ",
        "IEG" => "Zielana, Poland ",
        "OUZ" => "Zouerate, Mauritania ",
        "UGU" => "Zugapa, Indonesia ",
        "ZRH" => "Zurich, Switzerland ",
        "SAW" => "Sabiha Gokcen International Airport ",
    );
    if (array_key_exists("$code", $airport_name)) {
        return $airport_name[$code];
    } else {
        return $code;
    }
}

function airline_name($code) {

    $airport_name = Array
                (
                "0A" => "Amber Air",
                "0D" => "Darwin Airline",
                "0J" => "Jetclub",
                "0K" => "Kokshetau",
                "0W" => "West Caribbean",
                "1I" => "Sky Trek International Airlines",
                "2A" => "Deutsche Bahn",
                "2B" => "ATA Aerocondor",
                "2D" => "Aero VIP",
                "2F" => "Payam/IPTAS",
                "2G" => "San Juan Airlines",
                "2I" => "Star Up",
                "2J" => "Air Burkina",
                "2K" => "Aerogal Aerolineas Galapagos",
                "2L" => "Helvetic Airways",
                "2M" => "Moldavian Airlines",
                "2N" => "Yuzmashavia",
                "2O" => "Air Salone",
                "2P" => "Air Philippines",
                "2Q" => "Air Cargo Carriers",
                "2R" => "VIA Rail Canada",
                "2S" => "Island Express",
                "2T" => "Tulpar Air Service",
                "2U" => "Air Guinee Express",
                "2W" => "Welcome Air",
                "3A" => "Al iance Airlines",
                "3B" => "Job Air",
                "3C" => "Corporate Airlines",
                "3D" => "Denim Air",
                "3E" => "East Asia Airlines",
                "3G" => "Atlant Soyuz Airlines",
                "3H" => "Air Inuit",
                "3K" => "Jetstar Asia Airways",
                "3L" => "Intersky",
                "3M" => "Gulfstream International Airlines",
                "3N" => "Air Urga",
                "3O" => "Jubba Airways",
                "3P" => "Thai Pacific Airlines",
                "3T" => "Turan Air",
                "3U" => "Sichuan Airlines",
                "3V" => "TNT Airways",
                "3W" => "Wanair",
                "3X" => "Japan Air Commuter",
                "3Y" => "Baseops International",
                "3Z" => "Necon Air",
                "4A" => "F.S. Air Service",
                "4B" => "Aviastar Tu Co.",
                "4D" => "Air Sinai",
                "4E" => "Tanana Air Service",
                "4F" => "Air City",
                "4G" => "Shenzhen Airlines",
                "4H" => "Fly Linhas Aereas",
                "4I" => "Knighthawk Air Express",
                "4J" => "Nationwide Airlines",
                "4K" => "Kenn Borek Air",
                "4L" => "Air Astana",
                "4M" => "Lan Argentina",
                "4N" => "Air North Charter",
                "4O" => "Ocean Airlines",
                "4R" => "Orient Eagle Airways",
                "4S" => "Sol Air Honduras",
                "4T" => "Belair Airlines",
                "4U" => "Tavaj Transportes Aereos Regulares",
                "4W" => "Warbelow's Air Ventures",
                "4X" => "Air Mercury",
                "4Y" => "Yute Air Alaska",
                "4Z" => "South African Airlink",
                "5A" => "Alpine Aviation",
                "5B" => "Euro Asia International",
                "5C" => "CAL Cargo Airlines",
                "5D" => "Dombass Airlines",
                "5E" => "Equaflight Service",
                "5F" => "Arctic Circle Air Service",
                "5G" => "Skyservice Airlines",
                "5I" => "Trans Air Georgia",
                "5J" => "Cebu Pacific Air",
                "5K" => "Odessa Airlines",
                "5L" => "Aerosur",
                "5M" => "SIAT Sibaviatrans",
                "5N" => "Aeroflot Nord",
                "5O" => "Europe Airport",
                "5P" => "Sky Europe Airlines",
                "5Q" => "Keenair Charter",
                "5R" => "Karthago Airlines",
                "5S" => "Servicios Aereos Profesionales",
                "5T" => "Transair Sweden",
                "5U" => "LADE Lineas Aereas del Estado",
                "5V" => "Lviv Airlines/Lvov Airlines",
                "5W" => "Itapermirim Transportes Aereos",
                "5X" => "United Parcel Service",
                "5Y" => "Isles of Scil y Skybus",
                "5Z" => "Bismil ah Airlines",
                "6A" => "Aviacsa",
                "6B" => "Britannia Airways Sweden",
                "6C" => "Cape Smythe Air Services",
                "6D" => "Pelita Air",
                "6E" => "City Air Germany",
                "6F" => "Falcon Air Express",
                "6G" => "Air Wales",
                "6H" => "Israir",
                "6I" => "Aviast",
                "6J" => "Skynet Asia Airways",
                "6K" => "Inter Express Hava Tasimacilik",
                "6L" => "Aklak Air",
                "6M" => "Majestic Air",
                "6N" => "Trans Travel Airlines",
                "6O" => "Air Satellite",
                "6P" => "Clubair Sixgo",
                "6Q" => "Slovak Airlines",
                "6R" => "Aero Union",
                "6S" => "Kato Air",
                "6T" => "Almaty Aviation",
                "6V" => "Axis Airways",
                "6W" => "Saravia Saratov Airlines",
                "6Y" => "Latcharter",
                "6Z" => "Ukrainian Cargo Airways",
                "7A" => "Air Next",
                "7B" => "Kras Air/Krasnojarsky Airlines",
                "7C" => "Coyne Airways",
                "7D" => "Utility Enterprise DonbassAero Airline",
                "7E" => "Aeroline Sylt Air ",
                "7F" => "First Air",
                "7G" => "MK Airlines",
                "7H" => "ERA Aviation",
                "7I" => "Coastal Travel Limited",
                "7J" => "Tajik Air",
                "7K" => "Kolavia/Kogalymavia Airlines",
                "7L" => "Sun d'Or International Airlines",
                "7M" => "Tyumen Airlines",
                "7P" => "Metro Batavia",
                "7Q" => "Tibesti Air Libya",
                "7S" => "Arctic Transportation Services",
                "7T" => "Tobruk Air Transport and Cargo",
                "7U" => "Aviaenergo",
                "7V" => "Pelican Air Services",
                "7Y" => "Mid Airlines",
                "7Z" => "Laker Airways Bahamas",
                "8A" => "Atlas Blue",
                "8B" => "Caribbean Star Airlines",
                "8C" => "Air Transport International ATI",
                "8D" => "Expo Aviation",
                "8E" => "Bering Air",
                "8F" => "Fischer Air",
                "8G" => "Gestion Aerea Ejecutiva",
                "8H" => "Highland Airways",
                "8J" => "Komi nteravia",
                "8K" => "Angel Airlines",
                "8L" => "Redhil Aviation",
                "8M" => "Myanmar Airways International",
                "8N" => "Nordkalot flyg",
                "8P" => "Pacific Coastal Airlines",
                "8Q" => "Onur Air",
                "8R" => "Edelweiss Air",
                "8S" => "AAW Afriqiyah Airways",
                "8V" => "Wright Air Service",
                "8Y" => "China Postal Airlines",
                "8Z" => "LASER Linea Aerea",
                "9E" => "Pinnacle Airlines",
                "9J" => "Pacific Island Aviation",
                "9K" => "Cape Air",
                "9L" => "Colgan Air",
                "9M" => "Central Mountain Air",
                "9N" => "SATENA",
                "9Q" => "Pee Bee Air",
                "9R" => "Phuket Airlines",
                "9S" => "Spring Airlines",
                "9T" => "Athabaska Airlines",
                "9U" => "Air Moldova",
                "9V" => "Avior Express",
                "9W" => "Jet Airways",
                "9X" => "Itali Airlines",
                "A2" => "Cielos del Peru",
                "A3" => "Aegean Cronus Airlines",
                "A4" => "Southern Winds ",
                "A5" => "Airlinair",
                "A6" => "Air Alps",
                "A7" => "Air Comet",
                "A8" => "Benin Golf Air",
                "A9" => "Georgian Airlines",
                "AA" => "American Airlines",
                "AB" => "Air Berlin",
                "AC" => "Air Canada",
                "AD" => "Air Paradise International",
                "AE" => "Mandarin Airlines",
                "AF" => "Air France",
                "AG" => "Air Contractors",
                "AH" => "Air Algerie",
                "AI" => "Air India",
                "AJ" => "Aero Contractors",
                "AK" => "Air Asia",
                "AL" => "Transaviaexport Cargo Airline",
                "AM" => "Aeromexico",
                "AO" => "Australian Airlines",
                "AP" => "Air One",
                "AQ" => "Aloha Airlines",
                "AR" => "Aerolineas Argentinas",
                "AS" => "Alaska Airlines",
                "AT" => "Royal Air Maroc",
                "AU" => "Austral/Cielos del Sur",
                "AV" => "Avianca",
                "AW" => "Dirgantara Air Services",
                "AX" => "Trans States Airlines",
                "AY" => "Finnair",
                "AZ" => "Alitalia",
                "B2" => "Belavia",
                "B3" => "Bel view Airlines",
                "B4" => "Globespan Airways",
                "B5" => "FlightLine",
                "B6" => "JetBlue Airways",
                "B7" => "UNI Airways Corporation",
                "B8" => "Eritrean Airlines",
                "B9" => "Iran Air Tours",
                "BA" => "British Airways",
                "BC" => "Skymark Airlines",
                "BD" => "bmi British Midland",
                "BE" => "Flybe/British European Airways",
                "BF" => "Bluebird Cargo",
                "BG" => "Biman Bangladesh",
                "BI" => "Royal Brunei Airlines",
                "BJ" => "Nouvelair Tunisie",
                "BK" => "Potomac Air/US Airways Express",
                "BL" => "Pacific Airlines",
                "BM" => "Bayu Indonesia Air",
                "BN" => "Horizon Airlines",
                "BO" => "Bouraq Indonesia",
                "BP" => "Air Botswana",
                "BQ" => "Aeromar",
                "BR" => "EVA Airways Corporation",
                "BS" => "British International Helicopters",
                "BT" => "Air Baltic",
                "BU" => "SAS Braathens",
                "BV" => "Blue Panorama Airlines",
                "BW" => "BWIA West Indies Airways",
                "BX" => "Coast Air",
                "BY" => "Thomsonfly Britannia Airways",
                "BZ" => "Keystone Air Service",
                "C2" => "Air Luxor",
                "C3" => "Icar Airlines",
                "C4" => "Zimex Aviation",
                "C5" => "Commutair",
                "C6" => "CanJet",
                "C7" => "Rico Linhas Aereas",
                "C9" => "Cirrus Airways",
                "CB" => "Scot Airways",
                "CC" => "Macair",
                "CD" => "Al iance Air",
                "CE" => "Nationwide Airlines",
                "CF" => "City Airline",
                "CH" => "Bemidji Airlines",
                "CI" => "China Airlines",
                "CK" => "China Cargo Airlines",
                "CL" => "Lufthansa Cityline",
                "CM" => "Copa Airlines",
                "CO" => "Continental Express",
                "CQ" => "Sunshine Express Airlines",
                "CS" => "Continental Micronesia",
                "CT" => "Air Sofia",
                "CU" => "Cubana de Aviacion",
                "CV" => "Cargolux",
                "CW" => "Air Marshal Islands",
                "CX" => "Cathay Pacific",
                "CY" => "Cyprus Airways",
                "CZ" => "China Southern Airlines",
                "D3" => "Daal o Airlines",
                "D4" => "Alidauni",
                "D5" => "DHL Aero Expreso",
                "D6" => "Inter Air",
                "D8" => "Djibouti Airlines",
                "D9" => "Aeroflot Don",
                "DB" => "Brit Air",
                "DC" => "Golden Air",
                "DE" => "Condor Flugdienst",
                "DF" => "Aebal Aerolineas de Baleares",
                "DG" => "South East Asian Airlines",
                "DH" => "Independence Air",
                "DI" => "Dba Deutsche BA",
                "DJ" => "Virgin Blue Airways",
                "DK" => "Viking Airways",
                "DL" => "Delta Air Lines",
                "DM" => "Maersk Air",
                "DN" => "Air Deccan",
                "DO" => "Air Val ee",
                "DP" => "First Choice Airways",
                "DQ" => "Coastal Air Transport",
                "DS" => "EasyJet Switzerland",
                "DT" => "TAAG Angola Airlines",
                "DU" => "Hemus Air",
                "DV" => "Luft axi",
                "DW" => "Aero Charter Ukraine",
                "DX" => "DAT Danish Air Transport",
                "DY" => "Norwegian Air",
                "DZ" => "Transcaraibes Air International",
                "E2" => "Rio Grande Air",
                "E3" => "Domodedovo Airlines",
                "E5" => "Samara Airlines",
                "E7" => "European Aviation Air Charter",
                "E8" => "Alpi Eagles",
                "E9" => "Boston Maine Airways",
                "EA" => "European Air Express",
                "EC" => "Avialeasing Aviation Company",
                "ED" => "CCAir/US Airways Express",
                "EE" => "Aero Airlines",
                "EF" => "Far Eastern Air Transport",
                "EG" => "Japan Asia Airways",
                "EH" => "SAETA Sociedad Ecuatoriana",
                "EI" => "Aer Lingus",
                "EJ" => "New England Airlines",
                "EK" => "Emirates",
                "EL" => "Air Nippon",
                "EM" => "Empire Airlines",
                "EN" => "Air Dolomiti",
                "EO" => "Hewa Bora Airlines",
                "EP" => "Iran Asseman Airlines",
                "EQ" => "TAME",
                "ER" => "ASTAR Air Cargo",
                "ES" => "DHL Aviation",
                "ET" => "Ethiopian Airlines",
                "EU" => "Ecuatoriana",
                "EV" => "Atlantic Southeast Airlines",
                "EW" => "Eurowings/Lufthansa Regional",
                "EX" => "Air Santo Domingo",
                "EY" => "Etihad Airways",
                "EZ" => "Sun Air of Scandinavia",
                "F2" => "Fly Havayolu Tasimacilik",
                "F3" => "Faso Airways",
                "F4" => "Albarka Air",
                "F5" => "Cosmic Air",
                "F6" => "Air China Zhejiang Company",
                "F7" => "Flybaboo",
                "F8" => "Freedom Airlines",
                "F9" => "Frontier Airlines",
                "FA" => "Safair",
                "FB" => "Bulgaria Air",
                "FG" => "Ariana Afghan Airlines",
                "FH" => "Futura International Airlines",
                "FI" => "Icelandair",
                "FJ" => "Air Pacific",
                "FK" => "Africa West",
                "FL" => "Airtran",
                "FM" => "Shanghai Airlines",
                "FN" => "Regional Airlines",
                "FO" => "Airlines of Tasmania",
                "FP" => "TAG Aviation",
                "FQ" => "Thomas Cook Airlines",
                "FR" => "Ryanair",
                "FS" => "STAF Airlines",
                "FT" => "Siem Reap Airways International",
                "FV" => "Pulkovo Aviation Enterprise",
                "FW" => "Fair",
                "FX" => "FedEx Express",
                "FY" => "Northwest Regional Airlines",
                "G2" => "Dobrolet Airlines",
                "G3" => "GOL Transportes Aereos",
                "G4" => "Al egiant Air",
                "G6" => "Guine Bissau Airlines",
                "G8" => "Gujarat Airways",
                "G9" => "Air Arabia",
                "GA" => "Garuda ",
                "GB" => "ABX Air",
                "GC" => "Gambia International",
                "GD" => "Air Alpha Greenland",
                "GE" => "Trans Asia Airways",
                "GF" => "Gulf Air",
                "GG" => "Air Guyane",
                "GI" => "Air Company Itek Air",
                "GJ" => "Mexicargo",
                "GL" => "Miami Air",
                "GM" => "Air Slovakia",
                "GN" => "Air Gabon",
                "GO" => "Kuzu Airlines Cargo Transport",
                "GP" => "China General Aviation",
                "GQ" => "Big Sky Airlines",
                "GR" => "Gemini Air Cargo",
                "GS" => "Airfoyle",
                "GT" => "GB Airways",
                "GV" => "Aero Flight",
                "GW" => "Kuban Airlines",
                "GX" => "Pacific Airways Corporation",
                "GY" => "Tri MG Intra Asia Airlines",
                "H1" => "Hooters Air",
                "H2" => "Sky Service",
                "H4" => "Inter Islands Airlines",
                "H5" => "MAVIAL Magadan Airlines",
                "H6" => "Hageland Aviation Services",
                "H7" => "Eagle Air",
                "H8" => "Dalavia Far East Airways",
                "H9" => "TAM Express",
                "HA" => "Hawai an Airlines",
                "HB" => "Harbor Airlines",
                "HC" => "Aero Tropics Air Services",
                "HD" => "Hokkaido International Airlines",
                "HE" => "LGW Luftfahrtgesel schaft Walter",
                "HF" => "Hapagfly/Hapag Lloyd Flug",
                "HG" => "Haiti International Airline",
                "HH" => "Icebird/Islandsflug",
                "HJ" => "Asian Express Airlines",
                "HK" => "Four Star Aviation",
                "HM" => "Air Seychel",
                "HN" => "Proteus Helicopters",
                "HO" => "Antinea Airlines",
                "HP" => "America West Airlines",
                "HQ" => "Harmony Airways",
                "HR" => "Hahn Air Lines",
                "HS" => "Svenska Direktflyg",
                "HT" => "Aeromist/Kharkiv",
                "HU" => "Hainan Airlines",
                "HV" => "Transavia Airlines",
                "HW" => "North Wright Airways",
                "HY" => "Uzbekistan Airways",
                "HZ" => "SAT Sakhalin Airlines",
                "I6" => "Sky Eyes",
                "I9" => "Indigo Blue",
                "IA" => "Iraqi Airways",
                "IC" => "Indian Airlines",
                "ID" => "Interlink Airlines",
                "IE" => "Solomon Airlines",
                "IF" => "Islas Airways",
                "IG" => "Meridiana",
                "IH" => "Falcon Aviation",
                "II" => "IBC Airways",
                "IK" => "IMAIR",
                "IL" => "Lankair Private",
                "IM" => "Menajet",
                "IN" => "MAT Macedonian Airlines",
                "IO" => "Indonesian Airlines Aviapatria",
                "IP" => "AAW Atyrau Air Ways",
                "IQ" => "Augsburg Airways",
                "IR" => "Iran Air",
                "IS" => "Island Airlines",
                "IT" => "Irtysh Avia",
                "IV" => "Wind Jet",
                "IY" => "Yemenia",
                "IZ" => "Arkia",
                "J2" => "AZAL Azerbaijan Airlines",
                "J3" => "Northwestern Air Lease",
                "J4" => "Buf alo Airways",
                "J6" => "Avcom Aviation",
                "J7" => "Centre Avia Airlines",
                "J8" => "Berjaya Air",
                "J9" => "Guinee Airlines",
                "JB" => "Helijet Airways",
                "JC" => "JAL Express",
                "JF" => "LAB Flying Service",
                "JH" => "Nordeste Linhas Aereas Regionais",
                "JJ" => "TAM Transportes Aereos Meridionais",
                "JK" => "Spanair",
                "JL" => "Japan Airlines",
                "JM" => "Air Jamaica",
                "JN" => "Excel Airways",
                "JO" => "JALWays",
                "JP" => "Adria Airways",
                "JQ" => "Jetstar Airways",
                "JR" => "Aero California",
                "JS" => "Air Koryo",
                "JT" => "Lion Airlines",
                "JU" => "JAT Yugoslav Airlines",
                "JV" => "Bearskin Airlines",
                "JW" => "Arrow Air",
                "JY" => "Air Turks and Caicos",
                "JZ" => "Skyways",
                "K4" => "Kalit a Air",
                "K5" => "Wings of Alaska",
                "K7" => "Sakha Airlines",
                "K9" => "Skyward Aviation",
                "KA" => "Dragonair Airlines",
                "KB" => "Druk Air",
                "KD" => "Kaliningradavia",
                "KE" => "Korean Air",
                "KF" => "Blue1",
                "KG" => "Linea Aerea IAACA LAI",
                "KJ" => "BMED/British Mediterranean Airways",
                "KK" => "AtlasJet",
                "KL" => "KLM Royal Dutch Airlines",
                "KM" => "Air Malta",
                "KN" => "China United Airlines",
                "KO" => "Alaska Central Express",
                "KQ" => "Kenya Airways",
                "KR" => "Kit y Hawk Air Cargo",
                "KS" => "Penair/Peninsula Airways",
                "KU" => "Kuwait Airways",
                "KV" => "Kavminvodyavia",
                "KW" => "Kas Air",
                "KX" => "Cayman Airways",
                "KY" => "Air Sao Tome e Principe",
                "KZ" => "Nippon Cargo Airlines",
                "L2" => "Lynden Air Cargo",
                "L3" => "LTU Bil",
                "L4" => "Lauda Air Italy",
                "L5" => "Luft ransport",
                "L6" => "Tbilaviamsheni",
                "L7" => "Linea Aerea SAPSA",
                "L8" => "Air Luxor",
                "L9" => "Teamline Air Luftfahrt",
                "LA" => "LAN Airlines",
                "LB" => "LAB Airlines",
                "LC" => "Varig Log",
                "LD" => "Linea Turistica Aereotuy",
                "LF" => "Nordic East Airlink",
                "LG" => "Luxair",
                "LH" => "Lufthansa",
                "LI" => "LiatLJ SLA Sierra National Airlines",
                "LK" => "Air Luxor",
                "LL" => "Al egro Air",
                "LM" => "Livingston",
                "LN" => "Lybian Arab Airlines",
                "LO" => "LOT Polish Airlines",
                "LP" => "LAN Peru",
                "LQ" => "Lebanese Air Transport",
                "LR" => "Lacsa/Lineas Aereas Costarricenses",
                "LS" => "Channel Express",
                "LT" => "LTU International Airways",
                "LV" => "Albanian Airlines",
                "LW" => "Pacific Wings",
                "LX" => "Swiss International Air Lines",
                "M2" => "Mahfooz Aviation",
                "M3" => "North Flying",
                "M4" => "Avioimpex",
                "M6" => "Amerijet International",
                "M7" => "Tropical Airlines",
                "M8" => "Mekong Airlines",
                "M9" => "Motor Sich Aviakompania",
                "MA" => "Malev",
                "MB" => "MNG Cargo Airlines",
                "MC" => "Macedonian Airlines",
                "MD" => "Air Madagascar",
                "ME" => "MEA Middle East Airlines Airliban",
                "MF" => "Xiamen Airlines",
                "MG" => "Champion Air",
                "MH" => "Malaysia Airlines",
                "MI" => "Silk Air",
                "MJ" => "LAPA ",
                "MK" => "Air Mauritius",
                "ML" => "ATTICO African Transport Trading and Investment Co.",
                "MM" => "SAM Sociedad Aeronautica",
                "MN" => "Comair Commercial Airlines",
                "MO" => "Calm Air",
                "MP" => "Martinair",
                "MQ" => "American Eagle",
                "MR" => "Air Mauritanie",
                "MS" => "EgyptAir",
                "MT" => "Thomas Cook Airlines",
                "MU" => "China Eastern Airlines",
                "MW" => "Maya Island Air",
                "MX" => "Mexicana Airlines",
                "MY" => "Midwest Airlines",
                "MZ" => "Merpati Nusantara Airlines",
                "N2" => "Kabo Airlines",
                "N3" => "Omskavia Airline",
                "N4" => "Minerva Airlines",
                "N5" => "Skagway Air Service",
                "N7" => "Lagun Air",
                "N8" => "Salaam Express Air Services",
                "N9" => "North Coast Aviation",
                "NA" => "North American Airlines",
                "NB" => "Sterling European Airlines",
                "NC" => "Northern Air Cargo",
                "NE" => "SkyEurope Airlines",
                "NF" => "Air Vanuatu",
                "NG" => "Lauda Air",
                "NH" => "ANA Al  Nippon Airways",
                "NI" => "Portugalia",
                "NK" => "Spirit Airlines",
                "NL" => "Shaheen Air",
                "NM" => "Mount Cook Airlines (New Zealand) Mount Cook ",
                "NN" => "Vim Airlines (Russian Federation) Mov Air ",
                "NO" => "Neos (Italy) Moonflower ",
                "NQ" => "Air Japan (Japan) Air Japan",
                "NR" => "Pamir Air (Afghanistan) Pamir ",
                "NS" => "Caucasus Airlines (Georgia) Caucasus ",
                "NT" => "Binter Canarias (Spain) ",
                "NU" => "Japan Transocean Air (Japan) Jay Ocean",
                "NW" => "Northwest Airlines (USA) Northwest",
                "NX" => "Air Macau (Macao) Air Macau",
                "NY" => "Air Iceland/Flugfelag Islands (Iceland) ",
                "NZ" => "Air New Zealand (New Zealand) New Zealand",
                "O2" => "Oceanic Airlines (Guinea) ",
                "O3" => "Bel view Airlines (Sierra Leone) ",
                "OA" => "Olympic Airlines (Greece) Olympic ",
                "OB" => "Astrakhan Airlines (Russian Federation) Air Astrakhan",
                "OC" => "Omni Aviacao e Tecnologia/PGA Express (Portugal) Omni",
                "OE" => "Asia Overnight Express Corporation (Philippines) Asia Overnight",
                "OF" => "Transports et Travaux Aeriens de Madagascar (Madagascar) Tam Airline",
                "OH" => "Comair/Delta Connection (USA) Comair ",
                "OJ" => "Overland Airways (Nigeria) OverlandOK CSA CSA Czech Airlines (Czech Rep.) CSA Lines ",
                "OL" => "OLT Ostfriesische Luft ransport (Germany) Oltra",
                "OM" => "MIAT Mongolian Airlines (Mongolia) Mongol Air ",
                "ON" => "Air Nauru (Nauru) Air Nauru",
                "OO" => "Skywest Airlines/Delta Connection/United Express (USA) Skywest",
                "OP" => "Chalk's International Airlines/Flying Boat (USA) Chalk's ",
                "OQ" => "Aeronaves Queen/Queen Air (Dominican Rep.) ",
                "OR" => "TUI Airlines Nederlands (Netherlands) ",
                "OS" => "Austrian Airlines (Austria) Austrian",
                "OT" => "Aeropelican Air Services (Australia) ",
                "OU" => "Croatia Airlines (Croatia) Croatia",
                "OV" => "Estonian Air (Estonia) Estonian",
                "OW" => "Executive Airlines (USA) Echo Line",
                "OX" => "Orient Thai Airlines (Thailand) Orient Thai",
                "OY" => "Soder Air (Finland) Soder ",
                "OZ" => "Asiana Airlines (Korea  Rep. of) Asiana",
                "P2" => "Utair/Tyumenaviatrans (Russian Federation) Tjumavi",
                "P5" => "Aerorepublica (Colombia) Aerorepublica",
                "P6" => "Trans Air (USA) Maui",
                "P8" => "Pantanal Linhas Aereas Sul Matogrossenses (Brazil) Pantanal",
                "P9" => "Perm State Air Enterprise (Russian Federation) Perm Air ",
                "PA" => "Florida Coastal Airlines (USA) Florida Coastal",
                "PC" => "Pace Airlines/Piedmont Aviation (USA) Pace",
                "PD" => "PemAir (Canada) Pem Air ",
                "PE" => "Air Europe Italy (Italy) Air Europe",
                "PF" => "Palestinian Airlines (Palestinian Territory) Palestinian",
                "PG" => "Bangkok Airlines (Thailand) Bangkok Air ",
                "PH" => "Polynesian Airline of Samoa (Samoa) Polynesian",
                "PI" => "Sun Air/Sunflower Airlines (Fiji) Sunflower ",
                "PJ" => "Air Saint Pierre (France) ",
                "PK" => "Pakistan International Airlines",
                "PL" => "Airstars (Russian Federation) Morozov ",
                "PM" => "Tropic Air (Belize) Tropiser ",
                "PN" => "Pan American Airways (USA) Clipper ",
                "PO" => "Polar Air Cargo (USA) Polar ",
                "PP" => "Jet Aviation Business Jets (Switzerland) Jet Aviation",
                "PQ" => "PTL Luftfahrunternehmen (Germany) King Star ",
                "PR" => "Philippine Airlines (Philippines) Philippine",
                "PS" => "Ukraine International Airlines (Ukraine) Ukraine International",
                "PT" => "West Air Sweden (Sweden) Air Sweden",
                "PU" => "Pluna (Uruguay) Pluna",
                "PV" => "Pan Air Lineas Aereas (Spain) Skyjet",
                "PW" => "Precision Air Services (Tanzania) Precision Air ",
                "PX" => "Air Niugini (Papua New Guinea) NiuginiPY SLM Surinam Airways (Suriname) Surinam ",
                "PZ" => "TAM Paraguay/Transportes Aereos del Mercosur (Paraguay) Paraguaya",
                "Q3" => "Zambian Airways (Zambia) Mines ",
                "Q4" => "Swazi Express Airways (Swaziland) Swazi Express ",
                "Q5" => "40 Mile Air (USA) Mile Air ",
                "Q6" => "Aero Condor (Peru) Condor Peru",
                "Q8" => "Trans Air Congo (Congo) Trans Congo",
                "Q9" => "Afrinat International Airlines (Ghana) Afrinat",
                "QA" => "Aerocaribe (Mexico) Aero Caribe",
                "QC" => "Air Corridor (Mozambique) Air Corridor ",
                "QD" => "Air Class Lineas Aereas/AeroVIP (Uruguay) Aola",
                "QE" => "Crossair Europe/ECA Europe Continental Airways (Switzerland) Cigogne",
                "QF" => "Qantas Airways (Australia) Qantas ",
                "QH" => "Altyn Air Airlines (Kyrgyzstan) Altyn Avia",
                "QI" => "Cimber Air/Team Lufthansa (Denmark) Cimber ",
                "QJ" => "Latpass Airlines (Latvia) Latpass ",
                "QK" => "Air Canada Jazz (Canada) Jazz ",
                "QL" => "Aero Lanka/Serendib Express (Sri Lanka) Serendib",
                "QM" => "Air Malawi (Malawi) Malawi",
                "QO" => "Origin Pacific Airways (New Zealand) Origin",
                "QQ" => "Al iance Airlines (Australia) Al i",
                "QR" => "Qatar Airways (Qatar) Qatari",
                "QS" => "Travel Service (Czech Rep.) Skytravel",
                "QT" => "TAMPA Transportes Aereos Mercantiles Panamericanos (Colombia) Tampa",
                "QU" => "East African Airlines (Uganda) Crane",
                "QV" => "Lao Aviation (Laos) Lao",
                "QW" => "Turks and Caicos Airways (Turks and Caicos Islands) Turk National",
                "QX" => "Horizon Air (USA) Horizon Air ",
                "QY" => "European Air Transport (Belgium) Eurotrans ",
                "R2" => "Orenburg Airlines (Russian Federation) Orenburg",
                "R4" => "Russia State Transport Company (Russian Federation) Russia",
                "R5" => "Malta Air Charter (Malta) Malta Charter ",
                "R6" => "Air Sprska (Bosnia Herzegovina) Air Sprska",
                "R7" => "Aserca Airlines (Venezuela) Arosca",
                "R8" => "Kyrghyzstan Airlines (Kyrgyzstan) Kyrgyz ",
                "R9" => "Camai Air/Vil age Aviation (USA) Air Camai",
                "RA" => "Royal Nepal Airlines (Nepal) Royal Nepal",
                "RB" => "Syrian Arab Airlines (Syrian Arab Rep.) Syrian Air ",
                "RC" => "Atlantic Airways (Faroe Islands) Faroeline",
                "RD" => "Ryan International Airlines (USA) Ryan International",
                "RE" => "Aer Arann Express (Ireland) Aer Arann",
                "RF" => "Florida West International Airways (USA) Flo West",
                "RG" => "Varig (Brazil) Varig",
                "RH" => "Republic Express Airlines (Indonesia) Public Express ",
                "RI" => "Mandala Airlines (Indonesia) Mandala",
                "RJ" => "Royal Jordanian (Jordan) Jordanian",
                "RK" => "Royal Khmer Airlines (Cambodia) Khymer AirRL PPW Royal Phnom Penh Airways (Cambodia) Phnom Penh Air ",
                "RN" => "Euralair Horizons (France) ",
                "RO" => "Tarom (Romania) Tarom ",
                "RP" => "Chautauqua Airlines/US Airways Express (USA) Chautauqua",
                "RQ" => "Kam Air (Afghanistan) Kamgar ",
                "RR" => "Royal Air Force (UK) Rafair (+ various others) ",
                "RS" => "Oman Royal Flight (Oman) Oman",
                "RT" => "Airlines of South Australia (Australia) ",
                "RU" => "TCI Skyking (Turks and Caicos Islands) Skyking",
                "RV" => "Redair (Gambia) Africa World",
                "RW" => "Republic Airlines (USA) Brickyard",
                "RX" => "Aviaexpress (Hungary) Avex ",
                "RY" => "Euro Exec Express (Sweden) Echo Express ",
                "RZ" => "SANSA Servicios Aereos Nacionales SA (Costa Rica) ",
                "S2" => "Sahara Airlines (India) ",
                "S3" => "Santa Barbara Airlines (Venezuela) ",
                "S4" => "Sata International (Portugal) Air Azores ",
                "S5" => "Shut le America/US Airways Express (USA) Shut lecraft",
                "S7" => "Sibir/Siberia Airlines (Russian Federation) ",
                "S8" => "Shovkoviy Shlyah Airlines (Ukraine) ",
                "S9" => "East African Safari Air (Kenya) ",
                "SA" => "South African Airways (South Africa) Springbok ",
                "SB" => "Aircalin/Air Caledonie International (France) Aircalin",
                "SC" => "Shandong Airlines (China) Shandong",
                "SD" => "Sudan Airways (Sudan) ",
                "SE" => "Star Airlines (France) Starway ",
                "SF" => "Tassili Airlines",
                "SG" => "Sempati Air",
                "SH" => "FlyMe Sweden",
                "SI" => "Skynet Airlines",
                "SJ" => "Freedom Air International",
                "SK" => "SAS Scandinavian Airlines",
                "SL" => "Rio Sul",
                "SM" => "Swedline Express",
                "SN" => "SN Brussels Airlines",
                "SO" => "Superior Aviation",
                "SP" => "SATA Air Acores",
                "SQ" => "Singapore Airlines",
                "SS" => "Corse Air International",
                "SU" => "Aeroflot Russian Airlines",
                "SV" => "Saudi Arabian Airlines",
                "SW" => "Air Namibia",
                "SX" => "Aeroejecutivo",
                "SY" => "Sun Country Airlines",
                "SZ" => "Air China Southwest Company",
                "T2" => "Thai Air Cargo",
                "T4" => "Hel as Jet",
                "T5" => "Turkmenistan Airlines",
                "T6" => "Tavrey/Tavria Aviakompania",
                "T7" => "Twin Jet",
                "T8" => "Transportes Aereos Neuquen",
                "T9" => "Prime Air/Transmeridian Airlines",
                "TA" => "Taca Peru",
                "TC" => "Air Tanzania",
                "TD" => "Tulip Air",
                "TE" => "Lithuanian Airlines",
                "TF" => "Malmo Aviation",
                "TG" => "Thai Airways International",
                "TH" => "Transmile Air Services",
                "TI" => "TolAir Services",
                "TJ" => "TANS Aereos Selva",
                "TK" => "Turkish Airlines",
                "TL" => "Trans Mediterranean Airlines",
                "TM" => "LAM Mozambique",
                "TN" => "Air Tahiti Nui",
                "TO" => "President Airlines",
                "TP" => "TAP Air Portugal",
                "TQ" => "Tandem Aereo",
                "TR" => "Tiger Airways",
                "TS" => "Air Transat",
                "TT" => "Air Lithuania",
                "TU" => "Tunisair",
                "TV" => "Virgin Express",
                "TX" => "Air Caraibes",
                "TY" => "Iberworld",
                "TZ" => "ATA/American Trans Air",
                "U2" => "easyJet",
                "U3" => "Avies",
                "U4" => "Progress Multitrade",
                "U5" => "USA 3000 Airlines",
                "U6" => "Ural Airlines",
                "U7" => "Northern Dene Airways",
                "U8" => "Armavia",
                "U9" => "Tatarstan Joint Stock Aircompany",
                "UA" => "United Airlines",
                "UB" => "Myanmar Airlines",
                "UC" => "Lan Express",
                "UE" => "Transeuropean Airlines",
                "UF" => "Ukrainian Mediterranean Airlines",
                "UG" => "Tuninter",
                "UH" => "Eurasia Aircompany",
                "UI" => "Eurocypria",
                "UL" => "SriLankan Airlines",
                "UM" => "Air Zimbabwe",
                "UN" => "Transaero Airlines",
                "UO" => "Helicopters Hong Kong",
                "UP" => "Bahamas Air",
                "UQ" => "O'Connor Mt. Gambier's Airline",
                "US" => "US Airways",
                "UT" => "Air Orient",
                "UU" => "Air Austral",
                "UV" => "Universal Airways",
                "UW" => "Universal Airlines",
                "UX" => "Air Europa",
                "UY" => "Cameroon Airlines",
                "UZ" => "Tesis",
                "V2" => "Karat",
                "V3" => "Carpatair",
                "V4" => "Reem Air",
                "V5" => "Royal Aruban Airlines",
                "V6" => "Voyager Airline",
                "V7" => "Air Senegal International",
                "V8" => "TAPSA Transportes Aereos Petroleros",
                "V9" => "BAL Bashkirian Airlines",
                "VA" => "Volare Airlines",
                "VC" => "Voyageur Airways",
                "VD" => "Swedjet Airways",
                "VE" => "Avensa",
                "VF" => "Valuair",
                "VG" => "VLM",
                "VH" => "Aeropostal",
                "VI" => "Volga Dnepr Airlines",
                "VJ" => "Jatayu Gelang Sejahtera",
                "VK" => "Zantop International Airlines",
                "VL" => "Air VIA Bulgarian Airways",
                "VM" => "Viaggio Air",
                "VN" => "Vietnam Airlines",
                "VO" => "Tyrolean Airways",
                "VR" => "TACV Cabo Verde Airlines",
                "VT" => "Air Tahiti",
                "VU" => "Societe Nouvel e Air Ivoire",
                "VV" => "Aerosvit/Aerosweet Airlines",
                "VW" => "Transportes Aeromar",
                "VY" => "Vueling Airlines",
                "VZ" => "MyTravel Lite",
                "W2" => "White Eagle Aviation",
                "W3" => "Danu Oro Transportas",
                "W4" => "Aero Services Executive",
                "W5" => "Mahan Air",
                "W6" => "Wizz Air",
                "W7" => "Sayakhat",
                "W8" => "Winnport Air Cargo",
                "WA" => "KLM CityHopper",
                "WB" => "Servicios Aereos Nacionales",
                "WC" => "Islena Airlines",
                "WD" => "DAS Air Limited",
                "WE" => "Centurion Air Cargo",
                "WF" => "Wideroe",
                "WG" => "Wasaya Airways",
                "WJ" => "Air Labrador",
                "WK" => "American Falcon",
                "WL" => "AeroPerlas",
                "WM" => "Winair/Windward Islands Airways",
                "WN" => "Southwest Airlines",
                "WO" => "World Airways",
                "WP" => "Aloha Island Air",
                "WQ" => "Romavia/Romanian Aviation Company",
                "WS" => "WestJet",
                "WT" => "Nigeria Airways",
                "WU" => "Tikal Jets",
                "WV" => "Swe Fly",
                "WX" => "CityJet",
                "WY" => "Oman Aviation Services",
                "WZ" => "West African Airlines",
                "X3" => "Hapag Lloyd Express",
                "X5" => "Afrique Airlines",
                "X9" => "City Star Airlines",
                "XA" => "ARINC/Aeronautical Radio Inc.",
                "XC" => "KD Air",
                "XF" => "Vladivostok Air",
                "XG" => "Air Mali International",
                "XI" => "AEROTEL/Aeronautical Telecommunications",
                "XJ" => "Mesaba Airlines",
                "XK" => "CCM Compagnie Corse Mediterrannee",
                "XM" => "Australian Air Express",
                "XO" => "LTE International Airways",
                "XP" => "Casino Express",
                "XQ" => "Sun Express",
                "XR" => "SkyWest Airlines",
                "XU" => "African Express Airways",
                "XV" => "Ivoire Airways",
                "YW" => "Air Nostrum",
                "Y4" => "Eagle Aviation",
                "Y6" => "Cambodia Airlines",
                "Y7" => "Trans Air Benin",
                "Y8" => "Yangtze River Express Airlines",
                "Y9" => "Kish Air",
                "YB" => "South African Express",
                "YD" => "Gomelavia",
                "YF" => "Canadian Armed Forces",
                "YG" => "South Airlines",
                "YH" => "West Caribbean Airways",
                "YI" => "Air Sunshine",
                "YJ" => "National Airlines",
                "YK" => "TKHY Kibris Turkish Airlines",
                "YL" => "Yamal Airlines",
                "YM" => "Montenegro Airlines",
                "YN" => "Air Creebec",
                "YO" => "Heli Air Monaco",
                "YQ" => "Helikopterservice Euro Air",
                "YR" => "Eagle Canyon Airlines",
                "YS" => "Regional Compagnie Aerienne Europeenne",
                "YT" => "Air Togo",
                "YV" => "Mesa Airlines",
                "YX" => "Midwest Airlines",
                "Z2" => "Styrian Airways",
                "Z4" => "Zoom Airlines",
                "Z6" => "Dnieproavia",
                "Z7" => "Aviation Development Company",
                "Z8" => "Mali Airways",
                "ZB" => "Monarch Airlines",
                "ZC" => "Royal Swazi National Airways",
                "ZE" => "Lineas Aereas Azteca",
                "ZF" => "Atlantic Airlines",
                "ZG" => "Air Express Limited",
                "ZI" => "Aigle Azur",
                "ZJ" => "Air Routing International",
                "ZK" => "Great Lakes Aviation",
                "ZL" => "Regional Express/Hazelton Airlines",
                "ZO" => "Great Plans Airlines",
                "ZP" => "Silk Way Airlines",
                "ZR" => "Aviacon Zitotrans",
                "ZT" => "Voronezhavia",
                "ZU" => "Helios Airways",
                "ZV" => "Air Midwest",
                "ZW" => "Air Wisconsin",
                "ZX" => "Air Georgian",
                "ZY" => "ADA Air",
                "OD" => "Malindo Air",
                "ZY" => "Jazeera Airways",
    );

    if (array_key_exists("$code", $airport_name)) {
        return $airport_name[$code];
    } else {
        return $code;
    }
}

?>